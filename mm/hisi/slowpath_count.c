...
#ifdef CONFIG_HISI_QOS_MEM_OPT
	if(unlikely(is_slowpath && test_vip_alloc_thread()))
		qos_slow_path_inc(order);
#endif
...
#ifdef CONFIG_HISI_QOS_MEM_OPT
atomic_long_t qos_alloc_count = ATOMIC_LONG_INIT(0);
atomic_long_t qos_slow_path_count[ORDER_LIMIT] = {ATOMIC_LONG_INIT(0)};

void qos_slow_path_inc(int order)
{
	if (order < ORDER_LIMIT)
		atomic_long_inc(&qos_slow_path_count[order]);
}
EXPORT_SYMBOL(qos_slow_path_inc);

void qos_alloc_count_inc(void)
{
	atomic_long_inc(&qos_alloc_count);
}
EXPORT_SYMBOL(qos_alloc_count_inc);

static int qos_slowpath_count_show(struct seq_file *s, void *data)
{
	int i;
	long val, total = 0;

	val = atomic_long_read(&qos_alloc_count);
	seq_printf(s, "Qos memory malloc ok count:%ld\n", val);

	seq_puts(s, "Qos thread slow path count:[");
	for (i = 0; i < ORDER_LIMIT; i++) {
		val = atomic_long_read(&qos_slow_path_count[i]);
		total += val;
		if (i == ORDER_LIMIT - 1)
			seq_printf(s, "%ld", val);
		else
...
#ifdef CONFIG_HISI_QOS_MEM_OPT
	qos_slowpath_count_show(s, unused);
#endif
	return 0;
}
