...
static long pipe_set_size(struct pipe_inode_info *pipe, unsigned long arg)
{
	...
	if (nr_pages > pipe->buffers &&
		...
		pr_err("%s nr_pages=%u, buffers=%u, size=%u, pipe_max_size=%u",
	...
	if (nr_pages > pipe->buffers &&
		...
		pr_err("%s nr_pages=%u, buffers=%u, soft=%lu, user_bufs=%lu",
		       __func__, nr_pages, pipe->buffers, pipe_user_pages_soft,
		       user_bufs);
		ret = -EPERM;
		goto out_revert_acct;
	}
