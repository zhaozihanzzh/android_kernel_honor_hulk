...
static inline bool tcpci_is_attach_new_snk(struct tcpc_device *tcpc)
{
	return ((tcpc->typec_attach_new == TYPEC_ATTACHED_SNK) ||
		(tcpc->typec_attach_new == TYPEC_ATTACHED_NORP_SRC) ||
		(tcpc->typec_attach_new == TYPEC_ATTACHED_CUSTOM_SRC)) ?
		true : false;
}

static inline bool tcpci_is_attach_old_snk(struct tcpc_device *tcpc)
{
	return ((tcpc->typec_attach_old == TYPEC_ATTACHED_SNK) ||
		(tcpc->typec_attach_old == TYPEC_ATTACHED_NORP_SRC) ||
		(tcpc->typec_attach_old == TYPEC_ATTACHED_CUSTOM_SRC)) ?
		true : false;
}

static inline bool tcpci_is_snk_attached(struct tcpc_device *tcpc)
{
	if ((tcpc->typec_attach_old != TYPEC_UNATTACHED) ||
		(tcpc->typec_attach_old == tcpc->typec_attach_new))
		return false;

	return tcpci_is_attach_new_snk(tcpc);
}

static inline bool tcpci_is_snk_detached(struct tcpc_device *tcpc)
{
	if ((tcpc->typec_attach_new != TYPEC_UNATTACHED) ||
		(tcpc->typec_attach_old == tcpc->typec_attach_new))
		return false;

	return tcpci_is_attach_old_snk(tcpc);
}
	...
	struct tcpc_device *tcpc)
{
	...
	/* notify typec state when typec audio or pogo otg plug in */
	if (pogopin_typec_chg_ana_audio_suport() && rt_get_pogo_state(tcpc)) {
		if (((typec_state.old_state == TYPEC_UNATTACHED) &&
			(typec_state.new_state == TYPEC_ATTACHED_AUDIO)) ||
			((typec_state.old_state == TYPEC_ATTACHED_AUDIO) &&
			(typec_state.new_state == TYPEC_UNATTACHED)) ||
			((rt_get_pogo_state(tcpc) == POGO_OTG) &&
			(tcpci_is_snk_detached(tcpc) ||
			tcpci_is_snk_attached(tcpc))))
	...
	struct tcpc_device *tcpc, uint8_t connect)
{
	...
	if (pogopin_typec_chg_ana_audio_suport() && rt_get_pogo_state(tcpc)) {
		if ((!tcpci_is_attach_new_snk(tcpc) &&
			!tcpci_is_attach_old_snk(tcpc)) ||
			(rt_get_pogo_state(tcpc) != POGO_OTG))
			return 0;
	}
	...
	struct tcpc_device *tcpc, uint8_t type, int mv, int ma)
{
	...
	/* source vbus when typec audio plug in */
	if (pogopin_typec_chg_ana_audio_suport() && rt_get_pogo_state(tcpc)) {
		if (((typec_state.old_state == TYPEC_UNATTACHED) &&
			(typec_state.new_state != TYPEC_ATTACHED_AUDIO)) ||
			((typec_state.old_state != TYPEC_ATTACHED_AUDIO) &&
	...
	struct tcpc_device *tcpc, uint8_t type, int mv, int ma)
{
	...
	/* support sink vbus when pogo otg plug in for pogo otg typec charge function */
	if (pogopin_typec_chg_ana_audio_suport() && rt_get_pogo_state(tcpc)) {
		if ((rt_get_pogo_state(tcpc) != POGO_OTG) ||
			(!tcpci_is_attach_new_snk(tcpc) &&
			!tcpci_is_attach_old_snk(tcpc)))
			return 0;
	}
