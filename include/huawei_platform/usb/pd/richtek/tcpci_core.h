...
#endif	/* CONFIG_USB_PD_ALT_MODE_RTDC */
extern int tcpc_device_irq_enable(struct tcpc_device *tcpc_dev);

#ifdef CONFIG_POGO_PIN
uint8_t rt_get_pogo_state(struct tcpc_device *tcpc);
void rt_set_pogo_state(struct tcpc_device *tcpc, uint8_t state);
#else
static inline uint8_t rt_get_pogo_state(struct tcpc_device *tcpc)
{
	return POGO_NONE;
}

static inline void rt_set_pogo_state(struct tcpc_device *tcpc, uint8_t state)
{
}
#endif /* CONFIG_POGO_PIN */
