...
void upload_double_free_log(struct kmem_cache *s, const char *add_info)
{
	int ret;
	...
	ret = snprintf(
		g_work_node.data, MAX_MESSAGE_LENGTH,
		"info:%s, name:%s, pid:%d, comm:%.20s, gid:%d, gcomm:%.20s\n",
		add_info, s->name, current->pid,
		current->comm, current->tgid,
		current->group_leader->comm);
	if ((ret >= MAX_MESSAGE_LENGTH) || (ret < 0)) {
