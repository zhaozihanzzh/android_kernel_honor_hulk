...
struct hisi_pa_priv {
	struct hisi_pa_pinctrl pin_ctrl;
	hisi_pa_callback callback[CALLBACK_COUNT];
	struct hisi_pa_platform_para platform_para;
};

enum hisi_pa_pinctrl_state {
	STATE_DEFAULT = 0,
	STATE_IDLE,
};
...
static struct hisi_pa_priv *g_privdata = NULL;
...
static void audio_exec_gain_modify(struct hisi_pa_platform_para *platform_para)
{
	unsigned int i;
	...
	for (i = 0; i < PGA_BUTT; i++) {
		...
		old_val = snd_soc_read(platform_para->codec,
		...
		snd_soc_update_bits(platform_para->codec,
			platform_para->gain_reg[i],
...
static void audio_exec_gain_restore(struct hisi_pa_platform_para *platform_para)
{
	unsigned int i;
...
{
	struct hisi_pa_priv *priv = g_privdata;
	struct hisi_pa_platform_para *platform_para = NULL;
...
static void hisi_pa_read_config(struct platform_device *pdev)
{
	struct hisi_pa_priv *priv = g_privdata;
...
static int exec_codec_callback(enum hisi_pa_callback_type func_type)
{
	struct hisi_pa_priv *priv = g_privdata;
	int ret;
...
static int set_pinctrl_state(enum hisi_pa_pinctrl_state state)
{
	int ret = 0;
	struct hisi_pa_priv *priv = g_privdata;
...
/*
 * Special treatment of hisi PA ,The pinctrl process was used to handle
 * the corresponding hisi PA business.
 */
...
static int hisi_pa_probe(struct platform_device *pdev)
{
	int ret;
	...
	g_privdata = devm_kzalloc(dev, sizeof(struct hisi_pa_priv), GFP_KERNEL);
	if (g_privdata == NULL) {
	...
	hisi_pa_read_config(pdev);

	AUDIO_LOGI("end\n");
	return ret;
}

static void hisi_pa_shutdown(struct platform_device *pdev)
{
	struct hisi_pa_priv *priv = g_privdata;
...
static const struct of_device_id hisi_pa_match[] = {
	{ .compatible = "hisilicon,hisi-pa", },
	{},
};

static struct platform_driver hisi_pa_driver = {
	.driver = {
		.name  = "hisi-pa",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(hisi_pa_match),
	},
	.probe = hisi_pa_probe,
	.shutdown = hisi_pa_shutdown,
};

static int __init hisi_pa_init(void)
{
	AUDIO_LOGI("begin\n");
	return platform_driver_register(&hisi_pa_driver);
}
fs_initcall_sync(hisi_pa_init);

static void __exit hisi_pa_exit(void)
{
	AUDIO_LOGI("begin\n");
	platform_driver_unregister(&hisi_pa_driver);
	AUDIO_LOGI("bnd\n");
}
module_exit(hisi_pa_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("ASoC hisi pa driver");
