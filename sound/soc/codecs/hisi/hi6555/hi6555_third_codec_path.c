/*
 * hi6555_third_codec_poath.c codec driver.
 *
...
#define LOG_TAG "hi6555_third_codec"
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_update_bits(codec, CODEC_CLK_EN1_REG, BIT(CODEC3_L_DN_AFIFO_CLKEN_OFFSET), 0);
		snd_soc_update_bits(codec, CODEC_CLK_EN1_REG, BIT(CODEC3_R_DN_AFIFO_CLKEN_OFFSET), 0);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_update_bits(codec, CODEC_CLK_EN1_REG, BIT(CODEC3_L_DN_AFIFO_CLKEN_OFFSET), 0);
		/* disable pga */
		snd_soc_update_bits(codec, CODEC_CLK_EN3_REG, BIT(CODEC3_L_DN_PGA_CLKEN_OFFSET), 0);
		snd_soc_update_bits(codec, CODEC_CLK_EN3_REG, BIT(CODEC3_R_DN_PGA_CLKEN_OFFSET), 0);
		break;
	default:
		AUDIO_LOGE("3rdCodec_dlr_fifo_i2s3_tx_switch event err: %d", event);
		break;
	}
...
int add_third_codec(struct snd_soc_codec *codec)
{
	...
	return hi6555_add_codec_resource(codec, &third_codec_resource);
}
