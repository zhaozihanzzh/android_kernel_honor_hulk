	...
	const struct reg_seq_config *reg_seq_array, size_t len)
{
	...
	for (i = 0; i < len; i++) {
		if (reg_seq_array[i].cfg.update_bits)
			snd_soc_update_bits(codec, reg_seq_array[i].cfg.addr,
...
int hi6555_asp_clk_enable(struct snd_soc_codec *codec)
{
	...
	if (ret) {
		AUDIO_LOGE("asp_codec_clkclk en fail, ret = %d", ret);
...
int hi6555_asp_resource_enable(struct snd_soc_codec *codec)
{
	...
	ret = hi6555_asp_regulator_enable(codec);
	if (ret) {
		AUDIO_LOGE("couldn't enable asp regulator, ret = %d", ret);
		goto end;
	}

	ret = hi6555_asp_clk_enable(codec);
	if (ret) {
...
clk_enable_err:
	hi6555_asp_regulator_disable(codec);
...
void hi6555_asp_clk_disable(struct snd_soc_codec *codec)
{
	...
	if (!IS_ERR_OR_NULL(priv->resource.asp_codec_clk)) {
		...
		AUDIO_LOGI("asp_codec_clk disable ok");
	}
...
int hi6555_add_codec_resource(struct snd_soc_codec *codec, const struct snd_codec_resource *resource)
{
	...
	ret = snd_soc_add_codec_controls(codec, resource->controls, resource->num_controls);
	if (ret != 0) {
