/*
 * hi6555_core_path.c codec driver.
 *
...
static void ibias_work_enable(struct snd_soc_codec *codec, bool enable)
{
...
#define CORE_MUX_WIDGET \
	SND_SOC_DAPM_MUX("DAC_DL MUX", CODEC_CLK_EN0_REG, DACL_SRCUP_CLKEN_OFFSET, 0, &dacl_mux), \
...
int add_core(struct snd_soc_codec *codec)
{
	if (codec == NULL) {
	...
	return hi6555_add_codec_resource(codec, &core_resource);
}
