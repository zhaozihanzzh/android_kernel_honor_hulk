/*
 * hi6555_audio_path.c codec driver.
 *
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		val_l = snd_soc_read(codec, DACL_MIXER4_CTRL0_REG);
		val_l = val_l & BIT(DACL_MIXER4_IN3_MUTE_OFFSET);
		val_r = snd_soc_read(codec, DACR_MIXER4_CTRL0_REG);
		val_r = val_r & BIT(DACR_MIXER4_IN3_MUTE_OFFSET);

		if (!val_l)
			snd_soc_update_bits(codec, DACL_MIXER4_CTRL0_REG,
				BIT(DACL_MIXER4_IN3_MUTE_OFFSET),
				BIT(DACL_MIXER4_IN3_MUTE_OFFSET));

		if (!val_r)
			snd_soc_update_bits(codec, DACR_MIXER4_CTRL0_REG,
		...
		hi6555_reg_seq_write_array(codec, hplr_pga_up_regs,
		...
		if (priv->ops.codec_customized_func)
			priv->ops.codec_customized_func(priv, HI6555_AUDIO_PATH,
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		hi6555_reg_seq_write_array(codec, hplr_pga_down_regs,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_update_bits(codec, CLASSD_MIN_TN_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		snd_soc_update_bits(codec, CODEC_ANA_RW7_REG, MICBISA2_SET_MASK, 0x0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		snd_soc_update_bits(codec, CODEC_ANA_RW7_REG, MICBISA2_SET_MASK, 0xff);
		break;
	default:
		AUDIO_LOGE("power mode event err: %d", event);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		irq_mask = snd_soc_read(codec, ANA_IRQM_REG0_REG);
		...
		snd_soc_write(codec, ANA_IRQ_REG0_REG, IRQ_MSK_COMP);
		snd_soc_update_bits(codec, ANA_IRQM_REG0_REG, IRQ_MSK_BTN_NOR, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
...
#define AUDIO_MUX_WIDGET \
	...
	SND_SOC_DAPM_MUX("ADC3 MUX", DAC_FILTER_CTRL_REG, SIF_MST_ADC_MIC3_EN_OFFSET, 0, &adc3_mux), \
	...
	SND_SOC_DAPM_MUX("ADC_ULTR MUX", DAC_FILTER_CTRL_REG, SIF_MST_ADC_MIC3_EN_OFFSET, 0, &ultr_adc_mux), \
...
int add_audio(struct snd_soc_codec *codec)
{
	...
	return hi6555_add_codec_resource(codec, &audio_resource);
}
