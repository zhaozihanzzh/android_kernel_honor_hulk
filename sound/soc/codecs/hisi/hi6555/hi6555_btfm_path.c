/*
 * hi6555_btfm_path.c codec driver.
 *
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* enable i2s2 rx */
		snd_soc_update_bits(codec, I2S2_PCM_CTRL_REG,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		hi6555_i2s_set_pinctrl_default_for_i2s2();
		break;
	case SND_SOC_DAPM_POST_PMD:
		hi6555_i2s_set_pinctrl_idle_for_i2s2();
		break;
	default:
...
int add_btfm(struct snd_soc_codec *codec)
{
	...
	return hi6555_add_codec_resource(codec, &btfm_resource);
}
