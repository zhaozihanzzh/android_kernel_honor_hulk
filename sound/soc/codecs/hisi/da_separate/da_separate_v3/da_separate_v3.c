...
struct da_separate_v3_customize {
...
static struct da_separate_v3_customize codec_customize;
...
static const struct snd_kcontrol_new da_separate_v3_kcontrols[] = {
	CUSTOMER_KCONTROLS
};
...
static void classd_pop_off(struct snd_soc_component *codec)
{
	...
	snd_soc_component_update_bits(codec, CTRL_REG_CLASSD_CTRL0_REG,
...
static void da_separate_v3_enable_hsd(struct snd_soc_component *codec)
{
	snd_soc_component_write(codec, CODEC_ANA_RW32_REG, 0x1);
}

static void da_separate_v3_set_ibias_hsmicbias(struct snd_soc_component *codec, bool enable)
{
...
static void asp_pmu_reg_init(void)
{
	...
	snd_soc_component_update_bits(soc_codec, CTRL_REG_CLASSD_CTRL0_REG,
...
static void da_separate_v3_asp_reg_init(struct snd_soc_component *codec)
{
	IN_FUNCTION;
...
void audio_codec_mute_pga(bool mute)
{
	...
	struct platform_data *priv = NULL;
	struct da_separate_v3_customize *customize = NULL;
...
static void da_separate_v3_shutdown(struct platform_device *dev)
{
	AUDIO_LOGI("Begin");
...
static int asp_resource_init(struct snd_soc_component *codec)
{
	...
	if (IS_ERR_OR_NULL(priv->resource.asp_codec_clk)) {
		AUDIO_LOGE("get clk asp err: %pK", priv->resource.asp_codec_clk);
...
{
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
	struct da_separate_v3_customize *customize = priv->customized_data;
...
static void da_separate_v3_set_classd_supply(struct snd_soc_component *codec, bool enable)
{
...
static int pmu_resource_init(struct snd_soc_component *codec)
{
	...
	priv->resource.set_classd_supply = da_separate_v3_set_classd_supply;
...
static int pinctrl_init(struct snd_soc_component *codec)
{
	...
	if (priv->board_type == FPGA_BOARD) {
		snd_soc_component_write(codec, 0xFFF11714, 0x2); /* sif_do0 GPIO_194 */
		snd_soc_component_write(codec, 0xFFF11718, 0x2); /* sif_di0 GPIO_193 */
		snd_soc_component_write(codec, 0xFFF1171c, 0x2); /* sif_sync GPIO_204 */
		snd_soc_component_write(codec, 0xFFF1172c, 0x2); /* sif_do1 GPIO_210 */
		snd_soc_component_write(codec, 0xFFF11758, 0x3); /* dsd_clk GPIO_221, used as sif_clk */
		snd_soc_component_write(codec, 0xFFF11760, 0x2); /* sif_di1 GPIO_223 */
...
static enum classd_supply_type get_classd_supply_type(const struct snd_soc_component *codec)
{
...
static void usb_ana_ldo_resource_init(struct snd_soc_component *codec)
{
	unsigned int val = 0;
	struct platform_data *priv = snd_soc_component_get_drvdata(codec);
	struct device_node *np = codec->dev->of_node;
...
static int resource_init(struct snd_soc_component *codec)
{
	...
	da_separate_v3_asp_reg_init(codec);
...
static void customized_data_init(struct da_separate_v3_customize *data)
{
	int ret;
	...
	ret = snd_soc_add_component_controls(soc_codec, da_separate_v3_kcontrols, ARRAY_SIZE(da_separate_v3_kcontrols));
	if (ret != 0)
...
static int da_separate_v3_codec_init(struct snd_soc_component *codec)
{
	int ret;
	...
	if (ret) {
		AUDIO_LOGE("register au pa error, ret = %d", ret);
...
static void da_separate_v3_codec_deinit(struct snd_soc_component *codec)
{
	if (codec == NULL) {
		AUDIO_LOGE("parameter is null");
		return;
...
static void da_separate_v3_codec_customized_func(struct platform_data *priv,
	u32 sub_module, u32 key, u32 val)
{
	switch (sub_module) {
	case DA_SEPARATE_AUDIO_PATH:
...
static struct da_separate_mbhc_ops da_separate_v3_mbhc_ops = {
	da_separate_v3_set_ibias_hsmicbias,
	da_separate_v3_enable_hsd,
};

static struct codec_ops da_separate_v3_codec_ops = {
	da_separate_v3_codec_init,
	da_separate_v3_codec_deinit,
	da_separate_v3_shutdown,
	da_separate_v3_asp_reg_init,
	&da_separate_v3_mbhc_ops,
	da_separate_v3_codec_customized_func,
};

static int __init da_separate_v3_codec_data_init(void)
{
	da_separate_codec_driver_init(&da_separate_v3_codec_ops);

	return 0;
}

arch_initcall(da_separate_v3_codec_data_init);
