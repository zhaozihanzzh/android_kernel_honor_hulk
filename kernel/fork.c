...
#else
#ifdef CONFIG_HISI_QOS_MEM_OPT
	struct page *page = get_stack_page_from_pool(THREAD_SIZE_ORDER);
	if (!page) {
		gfp_t gfp_mask = THREADINFO_GFP | __GFP_HIGHMEM;
#ifdef CONFIG_ALLOC_MEM_SUPPORT_4GPLUS
		gfp_mask = THREADINFO_GFP;
#endif
		page = alloc_pages_node(node, gfp_mask, THREAD_SIZE_ORDER);
	}
#else
#ifdef CONFIG_ALLOC_MEM_SUPPORT_4GPLUS
...
#ifdef CONFIG_HISI_QOS_MEM_OPT
	if (!free_stack_page_to_pool(virt_to_page(tsk->stack), THREAD_SIZE_ORDER))
		return;
#endif
