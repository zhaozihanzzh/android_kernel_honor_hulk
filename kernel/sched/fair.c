...
static inline bool is_rtg_task(struct task_struct *p)
{
	...
	is_rtg = grp && grp->preferred_cluster;
	rcu_read_unlock();

	return is_rtg;
}

static inline bool rtg_task_misfit(struct task_struct *p, int cpu)
{
	struct related_thread_group *grp = NULL;
	bool misfit = false;
	...
	if (capacity_orig_of(cpumask_first(&grp->preferred_cluster->cpus)) >
...
#ifdef CONFIG_HISI_EAS_SCHED
	task_fits = is_rtg_task(p)
		? !rtg_task_misfit(p, cpu)
		: task_fits_max(p, cpu);
...
boosted_task_util(struct task_struct *task)
{
	...
	unsigned long boosted_util = util + margin;

	trace_sched_boost_task(task, util, margin);

#ifdef CONFIG_SCHED_HISI_TASK_MIN_UTIL
	boosted_util = max_t(unsigned long, boosted_util,
				task->cpu_cap_req.min_util);
#endif
...
static int find_vip_cpu(struct task_struct *p)
{
	...
	int preferred_cpu = -1;
	cpumask_t search_cpus = CPU_MASK_NONE;
	cpumask_t preferred_cluster_cpus = CPU_MASK_NONE;
	bool favor_larger_capacity = false;
	unsigned long capacity_orig, wake_util;
	int idle_idx;
	...
	grp = task_related_thread_group(p);
	if (grp && grp->preferred_cluster) {
		cpumask_copy(&preferred_cluster_cpus,
	...
	for_each_cpu(i, &search_cpus) {
		trace_sched_find_vip_cpu_each(
			...
			cpu_rq(i)->highest_hisi_vip_prio, is_reserved(i),
			walt_cpu_high_irqload(i), capacity_orig_of(i),
			cpu_util_without(i, p),
			cpumask_test_cpu(i, &preferred_cluster_cpus)
		);

		if (!list_empty(&cpu_rq(i)->hisi_vip_thread_list))
			continue;
		...
		if (cpumask_test_cpu(i, &preferred_cluster_cpus)) {
			if (preferred_cpu != -1) {
				/*
				 * In preferred cluster, favor lowest
				 * wake util cpu or shallowest idle cpu.
				 */
		    ...
		    int sibling_count_hint)
{
	...
	grp = task_related_thread_group(p);
	if (grp) {
		struct sched_cluster *prefer_cluster = grp->preferred_cluster;
		...
		if (prefer_cluster) {
			 ...
			 */
			sync_entity_load_avg(&p->se);
			rtg_cpu = find_rtg_cpu(p, &prefer_cluster->cpus);
			if (rtg_cpu != -1){
				rcu_read_unlock();
				return rtg_cpu;
			}
...
void check_for_migration(struct rq *rq, struct task_struct *p)
{
	...
	if (rq->misfit_task_load) {
		...
		grp = task_related_thread_group(p);
		if (grp && grp->preferred_cluster) {
			new_cpu = find_rtg_cpu(p, &grp->preferred_cluster->cpus);

			/* Inside rcu lock. Release it and goto the choice. */
			rcu_read_unlock();
			if (new_cpu != -1 &&
			    capacity_orig_of(new_cpu) > capacity_orig_of(cpu))
				goto do_active_balance;
