	...
	*backup_cpu = -1;
	...
	do {
		for_each_cpu_and(i, tsk_cpus_allowed(p), sched_group_cpus(sg)) {
...
#ifdef CONFIG_SCHED_HISI_DEBUG_TRACE
			trace_sched_cpu_util(i, cpu_util_wake(i, p),
					     walt_irqload(i),
			...
			/*
			 * When rotation enabled, consider only idle CPUs for
			 * active migration.
			 */
			if (walt_rotation_enabled &&
			    p->state == TASK_RUNNING && !idle_cpu(i))
				continue;
#endif
#endif
			/*
...
static inline void update_cfs_nr_heavy_running(struct rq *update_rq,
					       struct task_struct *p)
{
	bool is_heavy = update_rq->misfit_task > 0;
...
next:
		...
		trace_sched_load_balance_skip_tasks(env->src_cpu, env->dst_cpu,
				env->busiest_group_type, p->pid, load,
				task_util(p),
...
#ifdef CONFIG_HISI_EAS_SCHED
	if (((env->idle == CPU_IDLE) ||
	     (env->idle == CPU_NEWLY_IDLE && !walt_task_sleep_short(env->dst_rq->curr))) &&
