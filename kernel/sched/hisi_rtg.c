...
int sched_set_group_load_mode(struct rtg_load_mode *mode)
{
	...
	raw_spin_lock_irqsave(&grp->lock, flag);
	if (mode->freq_enabled == 0)
		grp->mode.freq_enabled = false;
	else
		grp->mode.freq_enabled = true;
	if (mode->util_enabled == 0)
		grp->mode.util_enabled = false;
	else
		grp->mode.util_enabled = true;
	raw_spin_unlock_irqrestore(&grp->lock, flag);
