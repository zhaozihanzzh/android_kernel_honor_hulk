...
int sched_set_group_load_mode(unsigned int grp_id,
		bool freq_enabled, bool util_enabled)
{
	struct related_thread_group *grp;
	unsigned long flag;
	...
	grp = lookup_related_thread_group(grp_id);
	if (!grp) {
		pr_err("set group_load_mode for group %d fail\n", grp_id);
		return -ENODEV;
	}

	raw_spin_lock_irqsave(&grp->lock, flag);
	grp->mode.freq_enabled = freq_enabled;
	grp->mode.util_enabled = util_enabled;
	raw_spin_unlock_irqrestore(&grp->lock, flag);
...
{
	struct related_thread_group *grp;
	bool need_update_prev_freq = false, need_update_next_freq = false;
	u64 now;
	unsigned long flags;
