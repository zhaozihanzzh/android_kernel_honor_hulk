				   ...
				   unsigned int flags)
{
	...
	if (flags & SCHED_CPUFREQ_IOWAIT) {
		...
		if (sched_io_is_busy != 0)
			sg_cpu->iowait_boost += (IOWAIT_BOOST_INC_STEP >> 1);
		else
			sg_cpu->iowait_boost +=
				(sg_cpu->iowait_boost != 0) ? IOWAIT_BOOST_INC_STEP : sg_cpu->sg_policy->policy->min;
...
#endif
#ifdef CONFIG_SCHED_HISI_TASK_MIN_UTIL
	util = max_t(unsigned long, util, sg_policy->min_util);
#endif
