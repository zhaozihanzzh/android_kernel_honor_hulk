...
static int find_lowest_rq(struct task_struct *task)
{
	...
	if (sysctl_sched_enable_rt_cas) {
		...
		if (grp) {
			if (unlikely(global_boost_enable) &&
				schedtune_task_boost(task) > 0) {
				grp->preferred_cluster = max_cap_cluster();
				rtg_target_cpus = &grp->preferred_cluster->cpus;
			} else if (unlikely(!grp->preferred_cluster)) {
				hisi_get_slow_cpus(&slow_cpus);
				rtg_target_cpus = &slow_cpus;
			} else
				rtg_target_cpus = &grp->preferred_cluster->cpus;
		} else if (cpumask_equal(tsk_cpus_allowed(task), cpu_all_mask)) {
			hisi_get_slow_cpus(&slow_cpus);
