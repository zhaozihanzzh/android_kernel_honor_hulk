				   ...
				   char *buf, int pwrhal_prf_idx)
{
	...
	for (i = 0; i < g_ipa_actor_num; i++) {
		rc = snprintf_s(buf + ret, PAGE_SIZE - ret,
			FREQ_BUF_LEN, "%u%s",
			powerhal_profiles[pwrhal_prf_idx][i], ",");
		if (rc < 0) {
...
static void update_debugfs(struct ipa_sensor *sensor_data)
{
	...
	rc = snprintf_s(filter_name, sizeof(filter_name),
		(sizeof(filter_name) - 1),
...
static int ipa_register_soc_cdev(struct ipa_thermal *thermal_data, struct platform_device *pdev)
{
	...
	(void)memset_s(cpu_masks,
		sizeof(struct cpumask) * CAPACITY_OF_ARRAY, 0,
			sizeof(struct cpumask) * CAPACITY_OF_ARRAY);
	...
	for (i = 0; i < (int)g_cluster_num; i++) {
		...
		ret = snprintf_s(node, sizeof(node),
			(sizeof(node) - 1), "cluster%d", i);
		if (ret < 0) {
