...
void thermal_contexthub_init(void)
{
	...
	int i, chan_table_size, ret, addrhdrlen, chublen;
	...
	addrhdrlen = CONTEXTHUB_THERMAL_DDR_MEMBERS_SIZE;
	p_chub_ddr = gtm_dev->chub->share_addr + CONTEXTHUB_THERMAL_DDR_MEMBERS_SIZE;
	chublen = CONTEXTHUB_THERMAL_DDR_TOTAL_SIZE
		- CONTEXTHUB_THERMAL_DDR_MEMBERS_SIZE;
	if (chublen < 0) {
		pr_err("[%s]chublen :%d is less than 0.\n", __func__, chublen);
		return;
	}
	...
	for(i = 0; i < gtm_dev->tsens_num_sensor; i++) {
		...
		ret = memcpy_s((void *)(p_chub_ddr +
			CONTEXTHUB_THERMAL_DDR_MEMBERS_SIZE * t_id),
				chublen, (void *)src, s_size);
		if (ret != EOK) {
			pr_err("%s:%d:memcpy_s copy failed.\n",
				__func__, __LINE__);
			return;
		}
		chublen = chublen - s_size;
		if (chublen < 0) {
			pr_err("%s:%d:chublen invalid.\n",
				__func__, chublen);
			return;
		}
	}
	...
	p_ddr_header++;
	if (addrhdrlen >= sizeof(struct hw_chan_table))
		addrhdrlen = addrhdrlen - sizeof(struct hw_chan_table);
	...
	if (addrhdrlen >= chan_table_size) {
		ret = memcpy_s((void *)p_ddr_header, addrhdrlen,
			gtm_dev->chub->adc_table, chan_table_size);
		if (ret != EOK) {
			pr_err("%s:%d:memcpy_s copy failed.\n",
				 __func__, __LINE__);
			return;
