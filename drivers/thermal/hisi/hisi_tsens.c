...
static int _tsens_register_thermal(void)
{
	...
	for (i = 0, j = 0; i < g_tmdev->tsens_num_sensor; i++, j++) {
		...
		rc = snprintf_s(name, sizeof(name),
			(sizeof(name) - 1),
				hisi_tsensor_name[i]); /*lint !e592*/
		if (rc < 0)
			dev_err(&pdev->dev, "snprintf_s fail for sensor name!\n");
		g_tmdev->sensor[i].mode = hisi_tsensor_mode[i];
		g_tmdev->sensor[i].sensor_num = (unsigned int)i;

		(void)memset_s(reg_no_name, sizeof(reg_no_name),
			0, sizeof(reg_no_name));
		rc = snprintf_s(reg_no_name, sizeof(reg_no_name),
			(sizeof(reg_no_name) - 1),
				"hisi,detect_%s_regno", name);
		if (rc < 0)
		...
		rc = snprintf_s(sensor_type_name, sizeof(sensor_type_name),
			(sizeof(sensor_type_name) - 1),
				"hisi,detect_%s_sensortype", name);
		if (rc < 0)
...
#ifdef CONFIG_HISI_THERMAL_TRIP
		(void)memset_s((void *)node_name, sizeof(node_name),
			0, sizeof(node_name));
