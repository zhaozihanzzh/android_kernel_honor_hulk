...
int gtx8_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	if (gts_test->ts->support_wake_lock_suspend ==
		...
		wake_lock(&gts_test->ts->wake_lock);
...
exit_finish:
	if (gts_test->ts->support_wake_lock_suspend ==
			GT8X_WAKE_LOCK_SUSPEND_SUPPORT)
		/* add wakelock,avoid i2c suspend */
		wake_unlock(&gts_test->ts->wake_lock);
	if (gts_test) {
		kfree(gts_test);
		gts_test = NULL;
	}
...
void gtx8_strncatint(char *dest, int src, char *format, size_t dest_size)
{
	...
	if (avaliable_len > 0)
		strncat(&dest[dest_len], src_str, avaliable_len);
}
	...
	struct ts_rawdata_info_new *info)
{
	...
	avaliable_len = sizeof(info->i2cinfo) - dest_len - 1;
	if (avaliable_len > 0)
		strncat(&info->i2cinfo[dest_len], "0F", avaliable_len);

	dest_len = strlen(info->i2cerrinfo);
	avaliable_len = sizeof(info->i2cerrinfo) - dest_len - 1;
	if (avaliable_len > 0)
		...
		struct gtx8_ts_test *ts_test)
{
	...
	if (have_bus_error) {
		...
		avaliable_len = sizeof(info->i2cinfo) - dest_len - 1;
		if (avaliable_len > 0)
			strncat(&info->i2cinfo[dest_len], "0P", avaliable_len);
	}
	/* CAP data info 1P */
	...
	avaliable_len = TS_RAWDATA_DEVINFO_MAX - dest_len - 1;
	if (avaliable_len > 0)
		strncat(&info->deviceinfo[dest_len], "-GT", avaliable_len);
	...
	avaliable_len = TS_RAWDATA_DEVINFO_MAX - dest_len - 1;
	if (avaliable_len > 0)
		strncat(&info->deviceinfo[dest_len], "-", avaliable_len);
		...
		struct gtx8_ts_test *ts_test)
{
	...
	if (have_bus_error) {
		...
		avaliable_len = TS_RAWDATA_RESULT_MAX - dest_len - 1;
		if (avaliable_len > 0)
