...
#define PROJECTID_FROM_IC "projectid_from_ic"
...
static int thp_project_init(struct thp_core_data *cd)
{
	...
	if (cd->projectid_from_ic == 1) {
	#ifndef CONFIG_LCD_KIT_DRIVER
		if(lcdkit_get_panel_version(&value))
			return ret;
	#endif
		THP_LOG_INFO("%s value is %u\n", __func__, value);
		value = value >> 4; /* Move four bits to the right */
		if (value == PROJECTID_VALUE) {
			...
			struct thp_core_data *cd)
{
	...
	rc = of_property_read_u32(spi_cfg_node, PROJECTID_FROM_IC, &value);
	if (rc == 0)
		cd->projectid_from_ic = value;
	else
		cd->projectid_from_ic = 0;
	THP_LOG_INFO("%s:projectid_from_ic %u\n", __func__, value);
