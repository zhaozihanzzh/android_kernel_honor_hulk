...
#define SYNA_TCM_RMIADDR_LEN 2
#define SYNA_TCM_SPI_BUF_LEN 4
...
static unsigned char *syna_tcm_spi_buf;
...
{
	unsigned char *rmiaddr = NULL;
	unsigned char fnnum = 0;
	int rc;

	syna_tcm_spi_buf = kzalloc(SYNA_TCM_SPI_BUF_LEN, GFP_KERNEL);
	if (syna_tcm_spi_buf == NULL) {
		THP_LOG_ERR("%s: Failed to allocate memory\n", __func__);
		return -ENOMEM;
	}

	rmiaddr = syna_tcm_spi_buf;
	...
	syna_tcm_spi_write(tdev->thp_core->sdev, rmiaddr, SYNA_TCM_RMIADDR_LEN);
...
exit:
	kfree(syna_tcm_spi_buf);
	syna_tcm_spi_buf = NULL;
	return rc;
}
...
{
	unsigned char *data = syna_tcm_spi_buf;
	...
	retval = syna_tcm_spi_read(tdev->thp_core->sdev, data,
		SYNA_TCM_SPI_BUF_LEN);
	if (retval < 0) {
	...
	thp_bus_unlock();
	memcpy(frame_buf, data, SYNA_TCM_SPI_BUF_LEN);
	return 0;
