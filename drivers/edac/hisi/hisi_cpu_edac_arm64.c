...
#else
	if (block_nr == L3) {
		bbox_diaginfo_record(L3_EDAC_ECC_UE_ERROR, NULL,
				     "L3 UE, errxstatus=0x%llx, errxmisc=0x%llx", errxstatus, errxmisc);
	} else {
		bbox_diaginfo_record((CPU0_EDAC_ECC_UE_ERROR + inst_nr), NULL,
				     "CPU%d UE, errxstatus=0x%llx, errxmisc=0x%llx", inst_nr, errxstatus, errxmisc);
	}
#endif
}
