				...
				struct arm_smmu_s1_cfg *cfg)
{
	...
	cfg->cdptr[cdptr_offset + CTXDESC_CD_1_TTB0_OFFSET] = cpu_to_le64(val);
...
static void arm_smmu_domain_free(struct iommu_domain *domain)
{
	...
	size_t cd_size = (1UL << smmu->ssid_bits) *
...
#ifdef HISI_SMMUV3_SUPPORT
	ret = of_property_read_u32(dev->of_node, "smmu_fpga", &is_fpga);
	if (!ret && (is_fpga == 1)) {
