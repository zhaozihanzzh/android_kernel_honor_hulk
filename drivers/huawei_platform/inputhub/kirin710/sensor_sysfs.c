...
static void cap_prox_calibrate_data_process(int calibrate_index)
{
	...
	memset(&cap_prox_test, 0, sizeof(cap_prox_test));
	...
	memcpy(cap_prox_test.name, CAP_PROX_TEST_CAL, sizeof(CAP_PROX_TEST_CAL));
	memcpy(cap_prox_test.result, CAP_PROX_RESULT, strlen(CAP_PROX_RESULT) + 1);
	...
	const char *buf, size_t count)
{
	...
	memcpy(spkt.para, &ps_extend_data, sizeof(ps_extend_data));
...
static DEVICE_ATTR(ps_switch_mode, 0664, attr_ps_switch_mode_show, attr_ps_switch_mode_store);
