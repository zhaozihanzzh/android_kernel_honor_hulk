	...
	unsigned int length, struct read_info *rd)
{
	...
	while (retry_count--) { /* send retry 3 times */
		if (!WAIT_FOR_MCU_RESP_DATA_AFTER_SEND(buf, unpack(buf, length), 2000,
			...
			/* parse the out data */
			if (resp_pkt.length <=
				(MAX_PKT_LENGTH + sizeof(resp_pkt.errno))) {
				/* fill errno to app */
				rd->errno = resp_pkt.errno;
				/* data_length means data lenght below */
				rd->data_length = (resp_pkt.length - sizeof(resp_pkt.errno));
				/* fill resp data to app */
				memcpy(rd->data, (char *)&resp_pkt + sizeof(pkt_header_resp_t),
					rd->data_length);
