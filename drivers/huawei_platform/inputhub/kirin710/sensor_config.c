...
int write_ps_sensor_offset_to_nv(char *temp, int length)
{
	...
	if (temp == NULL) {
		hwlog_err("write_ps_sensor_offset_to_nv fail, invalid para\n");
		return -1;
	}

	if (write_calibrate_data_to_nv(PS_CALIDATA_NV_NUM, length, "PSENSOR", (char *)temp))
		return -1;

	hwlog_info("write_ps_sensor_offset_to_nv suc len=%d\n", length);
...
int send_ps_calibrate_data_to_mcu(void)
{
	...
	if (ps_external_ir_calibrate_flag == 1) {
		if (ps_sensor_offset[0] != 0 && ps_sensor_offset[1] != 0 && ps_sensor_offset[2] != 0) {
