...
static void read_abov_sar_data_from_dts(struct device_node *dn)
{
	...
	uint16_t abov_channel_value_type;
	...
	if (of_property_read_u16(dn, "abov_channel_value", &abov_channel_value_type)) {
		sar_pdata.sar_datas.abov_data.abov_channel_value = 0;
		hwlog_err("%s:read abov_channel_value fail\n", __func__);
	} else {
		sar_pdata.sar_datas.abov_data.abov_channel_value = abov_channel_value_type;
		hwlog_info("%s:read abov_channel_value:0x%x\n", __func__,
			sar_pdata.sar_datas.abov_data.abov_channel_value);
	}
...
int motion_set_cfg_data(void)
{
	...
	/* set motion pickup cmd & sub cmd */
	app_config[0] = MOTION_TYPE_PICKUP;
	app_config[1] = SUB_CMD_MOTION_HORIZONTAL_PICKUP_REQ;
	/* pickup flag: 0 vertical, 1 horizontal. if 1, send motion config data to mcu */
