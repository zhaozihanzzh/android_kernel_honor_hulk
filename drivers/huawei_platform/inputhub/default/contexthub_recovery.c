...
static int write_ramdump_info_to_sharemem(void)
{
	...
	pConfigOnDDr->bbox_conifg.log_addr = current_sh_info.log_addr;
	pConfigOnDDr->bbox_conifg.log_size = current_sh_info.log_len;
	...
	hwlog_info("%s: bbox log addr: low 0x%x, high 0x%x, log len: 0x%x\n",
			__func__, (u32)(current_sh_info.log_addr), (u32)(current_sh_info.log_addr >> 32),
...
static int sh_rdr_register_exception(void)
{
	...
	if (ret == 0) {
		hwlog_err("%s: register SENSORHUB_MODID failed[%d].\n", __func__, ret);
		return ret;
	}
	...
	ret = rdr_register_exception(&einfo);
	if (ret == 0) {
		hwlog_err("%s: register SENSORHUB_USER_MODID failed[%d].\n", __func__, ret);
		return ret;
	}
	...
	ret = rdr_register_exception(&einfo);
	if (ret == 0) {
		hwlog_err("%s: register SENSORHUB_FDUL_MODID failed[%d].\n", __func__, ret);
		return ret;
	}
#endif
	return ret;
...
void ipc_dbg_record(uint8_t dir, pkt_header_t *head, char *info)
{
...
static void ipc_dbg_record_dump(void)
{
	...
	for (i = 0; i < SENSORHUB_TRACK_SIZE; i++) {
		...
		hwlog_err("%s [clk: %5lu.%06lus], dir=%d, tag=%d, cmd=%d, resp=%d, partial_order=%d, tranid=%d, length=%d, %s\n",
			__func__, (unsigned long)ts, rem_nsec / 1000, g_iom3_msg_buffer.msg[i].dir,
...
static int rdr_create_file()
{
	...
	if (IS_ERR_OR_NULL(ch_root)) {
		...
		if (IS_ERR_OR_NULL(ch_root)) {
			hwlog_err("%s debugfs_create_dir contexthub failed, err %ld\n", __func__, PTR_ERR(ch_root));
			ch_root = NULL;
			goto ERR_CREATE_DBGDIR;
		}
	} else {
		hwlog_info("%s dir contexthub contexthub is already exist\n", __func__);
	}

	file_dentry = debugfs_create_bool("dump_reset", S_IRUGO | S_IWUGO, ch_root, &g_sensorhub_dump_reset);
	if (IS_ERR_OR_NULL(file_dentry)) {
		hwlog_err("%s debugfs_create_file dump_reset failed\n", __func__);
		goto ERR_CREATE_DBGFS;
	}

#ifdef CONFIG_HISI_BB
	file_dentry = sh_debugfs_create_x64("rdr_core_mask", S_IRUGO | S_IWUGO, ch_root, &g_rdr_core_mask);
	if (IS_ERR_OR_NULL(file_dentry)) {
