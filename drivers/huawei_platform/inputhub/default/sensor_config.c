...
int send_als_calibrate_data_to_mcu(void)
{
	...
	//num of 4 save location lens 1
	memcpy(als_sensor_calibrate_data+ALS_CALIDATA_NV_SIZE,user_info.nv_data+4, 1);
	hwlog_info("als sensor calibrate location data: %c\n",*(als_sensor_calibrate_data+ALS_CALIDATA_NV_SIZE));

	//add 1 data save location
	if (send_calibrate_data_to_mcu(TAG_ALS, SUB_CMD_SET_OFFSET_REQ,
		als_sensor_calibrate_data, ALS_CALIDATA_NV_SIZE+1, false))
		return -1;
...
void select_stk3638_als_data(void)
{
	...
	hwlog_debug("stk3638_als_para_tabel= %d\n", j);
	for (i = 0; i < STK3638_PARA_SIZE; i++)
		hwlog_debug("the stk3638 %d als paramater is the %d", i,
...
void select_liteon_ltr2594_als_data(void)
{
	...
	hwlog_debug("ltr2594_als_para_tabel= %d\n", j);
	for (i = 0; i < LTR2594_PARA_SIZE; i++)
		hwlog_debug("the ltr2594 %d als paramater is the %d", i,
...
void set_vd6281_als_extend_prameters(void)
{
	...
	hwlog_debug("vd6281_als_para_table size = %d\n", i);
	min_threshold_als_para =
	...
	hwlog_debug("vd6281_als_para_tabel= %d\n", vd6281_als_para_table);
...
void set_rpr531_als_extend_prameters(void)
{
	...
	hwlog_debug("531_als_para_tabel= %d\n", rpr531_als_para_table);
	for (i = 0; i < RPR531_PARA_SIZE; i++)
		hwlog_debug("the 531 %d als paramater is the %d", i,
...
static void set_pinhole_als_extend_parameters(void)
{
	...
	hwlog_debug("als_para_tabel=%d phone_type=%d, sens_name=%d, phone_version=%d\n",
...
static void select_rohm_als_data(void)
{
	...
	hwlog_debug("als_para_tabel=%d ,bh1745 phone_color=0x%x tplcd_manufacture=%d, phone_type=%d,phone_version=%d\n",
...
static void select_rohmbh1749_als_data(void)
{
	...
	hwlog_info("als_para_tabel=%d ,phone_color=0x%x tplcd_manufacture=%d, phone_type=%d,phone_version=%d\n",
		als_para_table, phone_color, tplcd_manufacture,
		als_data.als_phone_type, als_data.als_phone_version);
}
...
static void select_avago_als_data(void)
{
	...
	hwlog_info("als_para_tabel=%d phone_color=0x%x phone_type=%d,phone_version=%d\n",
...
static void select_ams_tcs3701_als_data(void)
{
	...
	hwlog_info("als_para_tabel=%d phone_color=0x%x phone_type=%d,phone_version=%d, tp info is=%d\n",
...
void select_als_para(struct device_node *dn)
{
	...
	} else if (stk32670_als_flag == 1) {
		set_als_extend_prameters(
			&stk32670_als_para[0],
			ARRAY_SIZE(stk32670_als_para));
	} else if (stk32671_als_flag == 1) {
		set_als_extend_prameters(
			&stk32671_als_para[0],
			ARRAY_SIZE(stk32671_als_para));
	} else if (ltr311_als_flag == 1) {
		set_als_extend_prameters(
			&ltr311_als_para[0],
			ARRAY_SIZE(ltr311_als_para));
	} else if (syh399_als_flag == 1) {
		set_als_extend_prameters(
			&syh399_als_para[0],
			ARRAY_SIZE(syh399_als_para));
	} else {
