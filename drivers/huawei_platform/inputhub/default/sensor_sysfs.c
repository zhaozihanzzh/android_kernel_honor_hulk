	...
	int32_t *max_threshold, int sar_type)
{
	...
	if (cap_prox_data == NULL || min_threshold == NULL ||
		max_threshold == NULL) {
...
static ssize_t attr_als_calibrate_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rohm_rgb_flag != 1 && avago_rgb_flag != 1 && ams_tmd3725_rgb_flag != 1 && liteon_ltr582_rgb_flag != 1 &&
	    ...
	    vishay_vcnl36658_als_flag != 1 && ams_tcs3701_rgb_flag != 1 && bh1749_flag != 1 && vd6281_als_flag != 1) {
...
static void cap_prox_calibrate_data_process(int calibrate_index,
	char *data, uint16_t *diff_value, uint16_t *offset_value,
	uint16_t *calibrate_thred, int sar_type)
{
	...
	if (diff_value == NULL || offset_value == NULL ||
		calibrate_thred == NULL || data == NULL) {
		hwlog_err("%s: input error\n", __func__);
		return;
	}
	switch (calibrate_index) {
