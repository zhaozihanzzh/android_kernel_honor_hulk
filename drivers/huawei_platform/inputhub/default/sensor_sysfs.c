...
char *mag_sensor_id;
char *cap_sensor_id;
char *therm_sensor_id;
char *therm_calibrate_index;
...
RET_TYPE return_thermometer_calibration = RET_INIT;
...
static void ps_calibrate_process(unsigned long val, read_info_t *pkg_mcu)
{
	...
	if (val == PS_XTALK_CALIBRATE) {
		...
		ps_calib_data_for_data_collect[4] =
			(int32_t)((int16_t)(ps_sensor_offset[3] & 0x0000ffff));
		ps_calib_data_for_data_collect[7] =
			(int32_t)((int16_t)((ps_sensor_offset[3] & 0xffff0000) >> 16));
		...
		ps_calibate_offset_0 =
			(int16_t)(ps_sensor_offset[0] & 0x0000ffff);
		ps_calibate_offset_3 =
			(int16_t)((ps_sensor_offset[0] & 0xffff0000) >> 16);
		ps_calib_data_for_data_collect[0] = (int32_t)ps_calibate_offset_0;
		ps_calib_data_for_data_collect[3] = (int32_t)ps_calibate_offset_3;
		ps_sensor_offset[val - 1] = *((int32_t *)p->data);
...
static ssize_t attr_thermometer_calibrate_write(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long val;
	int calibrate_index;
	...
	memset(&pkg_mcu, 0, sizeof(pkg_mcu));
	pkg_mcu = send_calibrate_cmd(TAG_THERMOMETER, val,
		&return_thermometer_calibration);
	if (return_thermometer_calibration == COMMU_FAIL || pkg_mcu.errno != 0) {
		return_thermometer_calibration = 0;
...
ssize_t attr_get_therm_calibrate_index(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	hwlog_info("%s is %s\n", __func__, therm_calibrate_index);
