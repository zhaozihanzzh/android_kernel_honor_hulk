...
#define EGIS_RESET_PIN_HIGH	1
#define EGIS_RESET_PIN_LOW	0
#define EGIS_SHORT_SLEEP	5
#define EGIS_NAME_LEN		4
#define EGIS_LONG_SLEEP		10
...
static void read_ext_ps_data_from_dts(struct device_node *dn) {
	int temp;
...
static void read_motion_data_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u32_array(dn, "motion_horizontal_pickup_flag", wia, len)) {
	...
	hwlog_info("%s: pickup_flag %d\n", __func__, (int)motion_data.motion_horizontal_pickup_flag);
}
...
static int fingerprint_ud_sensor_detect(struct device_node *dn, int index, struct sensor_combo_cfg *cfg)
{
	...
	if (!ret) {
		...
		if(!strncmp(sensor_vendor, "goodix,goodixG2", 15)) {
			...
			gpio_direction_output(gpio_reset, EGIS_RESET_PIN_HIGH);
			msleep(EGIS_SHORT_SLEEP);
			gpio_direction_output(gpio_reset, EGIS_RESET_PIN_LOW);
			msleep(EGIS_LONG_SLEEP);
			gpio_direction_output(gpio_reset, EGIS_RESET_PIN_HIGH);
			msleep(EGIS_LONG_SLEEP);
			hwlog_info("%s: fingerprint device %s\n",
				__func__, sensor_vendor);
	...
	switch(s_id)
	{
		...
		case MAG:
			...
			ret = fill_extend_data_in_dts(dn, str_soft_para,
				mag_data.pdc_data, PDC_SIZE, EXTEND_DATA_TYPE_IN_DTS_BYTE);
			if (ret)
				hwlog_err("%s:fill_extend_data_in_dts fail\n", str_soft_para);
			ret = fill_extend_data_in_dts(dn, str_soft_para_far,
				mag_iron_data.pdc_data_far, PDC_SIZE, EXTEND_DATA_TYPE_IN_DTS_BYTE);
			if (ret) {
...
#ifdef CONFIG_CONTEXTHUB_SHMEM
				if (s_id == MAG) {
					ret = shmem_send(TAG_MAG,
						&mag_iron_data, sizeof(mag_iron_data));
					if (ret)
