...
extern char *therm_sensor_id;
extern char *therm_calibrate_index;
...
struct therm_paltform_data therm_data = {
	.cfg = {
		.bus_type = TAG_I2C,
		.bus_num = 0,
		.disable_sample_thread = 0,
...
struct sensor_detect_manager sensor_manager[SENSOR_MAX] = {
	...
	{ "thermometer", THERMOMETER, DET_INIT, TAG_THERMOMETER, &therm_data, sizeof(therm_data) },
};
...
void read_als_ud_from_dts(struct device_node *dn)
{
	int temp;
	int ret;
	...
	ret = fill_extend_data_in_dts(dn, "als_cali_after_sale_xy",
		(unsigned char *)als_ud_cali_xy, ALS_UD_CALI_LEN,
...
void read_als_data_from_dts(struct device_node *dn)
{
	...
	if (!strncmp(chip_info, "huawei,ltr578_als",
		...
		sens_name = LTR578;
		ltr578_flag = 1;
		hwlog_debug("%s:ltr578_als i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(chip_info, "huawei,apds9922_als",
		...
		sens_name = APDS9922;
		apds9922_flag = 1;
		hwlog_debug("%s:apds9922_als i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(chip_info, "huawei,vishay_vcnl36658",
		...
		hwlog_debug("%s:vishay_vcnl36658 i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(chip_info, "huawei,liteon_ltr2594",
		...
		ltr2594_als_flag = 1;
		hwlog_debug("%s:liteon_ltr2594 i2c_address suc,%d\n",
			__func__, temp);
	}

	if (!strncmp(chip_info, "huawei,liteon_ltr311",
		sizeof("huawei,liteon_ltr311"))) {
		ltr311_als_flag = 1;
		hwlog_debug("%s:liteon_ltr311 i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(chip_info, "huawei,sensortek_stk3638",
		...
		stk3638_als_flag = 1;
		hwlog_debug("%s:sensortek_stk3638 i2c_address suc,%d\n",
			__func__, temp);
	}

	if (!strncmp(chip_info, "huawei,sensortek_stk32670",
		sizeof("huawei,sensortek_stk32670"))) {
		stk32670_als_flag = 1;
		hwlog_debug("%s:sensortek_stk32670 i2c_address suc,%d\n",
			__func__, temp);
	}

	if (!strncmp(chip_info, "huawei,sensortek_stk32671",
		sizeof("huawei,sensortek_stk32671"))) {
		stk32671_als_flag = 1;
		hwlog_debug("%s:sensortek_stk32671 i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(chip_info, "huawei,st_vd6281",
		...
		vd6281_als_flag = 1;
		hwlog_debug("%s:st_vd6281 i2c_address suc,%d\n", __func__, temp);
	}

	if (!strncmp(chip_info, "huawei,A", sizeof("huawei,A"))) {
		tsl2591_flag = 1;
		hwlog_debug("%s:A i2c_address suc,%d\n", __func__, temp);
	}

	if (!strncmp(chip_info, "huawei,B", sizeof("huawei,B"))) {
		bh1726_flag = 1;
		hwlog_debug("%s:B i2c_address suc,%d\n", __func__, temp);
	}

	if (!strncmp(chip_info, "huawei,C", sizeof("huawei,C"))) {
		veml32185_als_flag = 1;
		hwlog_debug("%s:C i2c_address suc,%d\n", __func__, temp);
	}

	if (!strncmp(chip_info, "huawei,silergy_syh399", sizeof("huawei,silergy_syh399"))) {
		syh399_als_flag = 1;
		hwlog_info("%s:S i2c_address suc,%d\n", __func__, temp);
	}
	...
	if (of_property_read_u32(dn, "gpio_int1_sh", &temp))
		...
		als_data.gpio_int1_sh = (GPIO_NUM_TYPE)temp;

	if (of_property_read_u32(dn, "als_ud_type", &temp))
		hwlog_err("%s:read als_ud_type fail\n", __func__);
	else
		als_data.als_ud_type = (GPIO_NUM_TYPE)temp;
	...
	if (of_property_read_u32(dn, "gpio_func_num", &temp))
		...
		als_data.gpio_func_num = (uint8_t)temp;
...
static void read_ps_chip_info(void)
{
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,txc-pa224",
		...
		sensor_ps_flag = 1;
		hwlog_debug("%s:txc_ps i2c_address suc,%d\n", __func__, temp);
	}

	if (!strncmp(sensor_chip_info[PS], "huawei,ams-tmd2620",
		sizeof("huawei,ams-tmd2620"))) {
		sensor_ps_flag = 1;
		hwlog_debug("%s:ams_tmd2620_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,avago-apds9110",
		...
		sensor_ps_flag = 1;
		hwlog_debug("%s:avago_apds9110_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,ams-tmd3725",
		...
		sensor_ps_flag = 1;
		hwlog_debug("%s:ams-tmd3725_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,ams_tcs3701",
		sizeof("huawei,ams_tcs3701"))) {
		sensor_ps_flag = 1;
		hwlog_debug("%s:ams_tcs3701_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,liteon-ltr582",
		...
		sensor_ps_flag = 1;
		hwlog_debug("%s:liteon-ltr582_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,ams_tmd3702",
		sizeof("huawei,ams_tmd3702"))) {
		sensor_ps_flag = 1;
		hwlog_debug("%s:huawei,tmd3702 i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,vishay-vcnl36658",
		...
		sensor_ps_flag = 1;
		hwlog_debug("%s:vishay-vcnl36658_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,avago_apds9253_006",
		...
		hwlog_debug("%s:apds9253_006_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,sensortek_stk3638",
		sizeof("huawei,sensortek_stk3638"))) {
		sensor_ps_flag = 1;
		hwlog_debug("%s:sensortek_stk3638_ps i2c_address suc,%d\n",
			__func__, temp);
	}
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,liteon_ltr2594",
		sizeof("huawei,liteon_ltr2594"))) {
		sensor_ps_flag = 1;
		hwlog_debug("%s:liteon_ltr2594_ps i2c_address suc,%d\n",
			__func__, temp);
	}
...
static void read_thermometer_data_from_dts(struct device_node *dn)
{
	int temp = 0;
	...
	if (of_property_read_u32(dn, "blackbody_count", &temp))
		...
		therm_data.blackbody_count = (uint8_t)temp;
...
static void set_goodix_ud_g5x_cmd(GPIO_NUM_TYPE gpio_cs)
{
...
static void set_goodix_ud_g5x_config(GPIO_NUM_TYPE gpio_cs)
{
	struct goodix_reg_info g5x_reg_info[] = {
		{ 0x0130, 0x4101, 0 },
		{ 0x0132, 0x0000, 0 },
	};
	const uint32_t size = ARRAY_SIZE(g5x_reg_info);

	set_goodix_ud_g5x_cmd(gpio_cs);
	gf_ud_common_handle(gpio_cs, size, g5x_reg_info);
}
	...
	struct sensor_combo_cfg *cfg)
{
	...
	if (!ret) {
		...
		} else if (!strncmp(sensor_vendor, "goodix,goodixG5X",
			GF_G5X_NAME_LEN)) {
			set_goodix_ud_g5x_config(gpio_cs);
		} else if (!strncmp(sensor_vendor, "goodix", GOODIX_NAME_LEN)) {
...
static void read_tp_ud_from_dts(struct device_node *dn)
{
	...
	if (of_property_read_u32(dn, "tp_sensorhub_platform", &temp)) {
		...
		tp_ud_data.tp_sensorhub_platform = 4; /* default value */
	} else {
		tp_ud_data.tp_sensorhub_platform = (uint16_t)temp;
		hwlog_info("%s:read tp_sensorhub_platform = %u\n",
			__func__, tp_ud_data.tp_sensorhub_platform);
	}
...
static int thermometer_data_from_mcu(const pkt_header_t *head)
{
	int value;
	...
	case SUB_CMD_SELFCALI_REQ:
		...
		if (value == 0)
			return_thermometer_calibration = SUC;
		else
			return_thermometer_calibration = EXEC_FAIL;
		hwlog_info("therm calibrate cmd %d, value %d, result %d.\n", ((pkt_thermometer_data_req_t *)head)->subcmd,
					value, return_thermometer_calibration);
	...
	case SUB_CMD_SELFTEST_REQ:
		...
		if (value == 0) {
			memcpy_s(sensor_status.selftest_result[TAG_THERMOMETER], SELFTEST_RESULT_MAXLEN, "0", 2);
		} else {
			snprintf_s(buf, SELFTEST_RESULT_MAXLEN, SELFTEST_RESULT_MAXLEN - 1, "%d\n", value);
			memcpy_s(sensor_status.selftest_result[TAG_THERMOMETER], SELFTEST_RESULT_MAXLEN, buf, SELFTEST_RESULT_MAXLEN);
		}
		break;
	default:
		hwlog_err("uncorrect subcmd 0x%x\n", ((pkt_thermometer_data_req_t *)head)->subcmd);
