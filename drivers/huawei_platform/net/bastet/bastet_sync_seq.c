...
int start_bastet_sock(struct bst_set_sock_sync_delay *init_prop)
{
	...
	if (tcp_sk(sk)->repair) {
		bastet_loge("proxy_id:%d in repair mode", get_proxy_id(sk));
		sock_put(sk);
		return -EPERM;
	}

	bastet_logi("proxy_id:%d", get_proxy_id(sk));
	...
	if ((*sk)->sk_state == TCP_TIME_WAIT) {
		if (!is_ipv6_addr(*sk))
			post_indicate_packet(BST_IND_SOCK_STATE_WAIT,
				guide, sizeof(*guide));
		bastet_loge("proxy_id:%d not expected time wait sock",
			get_proxy_id(*sk));
	...
	struct bastet_sock **bsk)
{
	...
	*sk = get_sock_by_comm_prop_ipv6(guide);
	...
	if ((*sk)->sk_state == TCP_TIME_WAIT) {
		bastet_loge("ipv6 proxy_id:%d not expected time wait sock",
			get_proxy_id(*sk));
		inet_twsk_put(inet_twsk(*sk));
		return -EPERM;
	}
	...
	*bsk = (*sk)->bastet;
	if (*bsk == NULL) {
		bastet_loge("ipv6 proxy_id:%d not expected bastet null",
			get_proxy_id(*sk));
	...
	bastet_logi("ipv6 proxy_id:%d", get_proxy_id(*sk));
	...
	struct bst_get_sock_comm_prop_ipv6 *get_prop)
{
	...
	if (sk->sk_state != TCP_ESTABLISHED) {
		bastet_loge("proxy_id:%d sk_state not expected",
			get_proxy_id(sk));
		sock_put(sk);
		return -EPERM;
	}
