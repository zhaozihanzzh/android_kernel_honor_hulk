...
int get_sock_net_dev_name(struct get_netdev_name *dev_name)
{
	...
	dst = __sk_dst_get(sk);
	if (dst && dst->dev) {
		bastet_logi("proxy_id:%d, net device name: %s",
			get_proxy_id(sk), dst->dev->name);
		memcpy(dev_name->netdev_name, dst->dev->name, IFNAMSIZ);
	} else {
		bastet_loge("failed to get net device name, proxy_id:%d",
			get_proxy_id(sk));
		ret = -ENOENT;
	}
	sock_put(sk);
...
int bastet_set_freezer(struct freezer_state freezer)
{
	...
	if (is_wifi_proxy(sk)) {
		bastet_logi("proxy_id:%d, frozen: %d",
			get_proxy_id(sk), freezer.frozen);
