...
static void acr_det_sysfs_init_attrs(void)
{
	...
	for (i = 0; i < ACR_DET_SYSFS_ATTRS_SIZE; i++)
		acr_det_sysfs_attrs[i] = &acr_det_sysfs_field_tbl[i].attr.attr;

	acr_det_sysfs_attrs[i] = NULL;
}

static struct power_sysfs_attr_info *acr_det_sysfs_field_lookup(
	const char *name)
{
	int i;
