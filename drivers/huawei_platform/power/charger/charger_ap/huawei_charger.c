...
static void charger_handle_event(struct charge_device_info* di, enum charger_event_type event)
{
	...
	case START_SINK_WIRELESS:
		...
		break;
	case STOP_SINK:
		charger_event_notify(CHARGER_PRE_STOP_CHARGING_EVENT);
#ifdef CONFIG_WIRELESS_CHARGER
...
static bool adaptor_cfg_for_normal_chg(int vol, int cur)
{
	...
	sid.init_adaptor_voltage = vol;
	sid.watchdog_timer = 0; /* disable watchdog for normal charge */

	if (adapter_set_init_data(&sid))
		return false;
	if (adapter_set_output_voltage(vol))
		return false;
	if (adapter_set_output_current(cur))
		return false;
...
static void charge_full_handle(struct charge_device_info *di)
{
	...
	if (is_battery_full && huawei_battery_capacity() == 100 &&
		di->fcp_ops && di->fcp_ops->fcp_adapter_reset) {
...
static void charge_stop_charging(struct charge_device_info *di)
{
...
#ifdef CONFIG_DIRECT_CHARGER
	if (di->force_disable_dc_path && (is_in_scp_charging_stage() ==
		IN_SCP_CHARGING_STAGE))
	...
	/* call it after cancel charge work */
	if (di->scp_adp_normal_chg)
...
static void charge_parse_dts(struct charge_device_info *di)
{
	...
	ret = of_property_read_u32(of_find_compatible_node(NULL, NULL, "huawei,charger"), \
			"need_filter_pd_event", &di->need_filter_pd_event);
	if (ret) {
		hwlog_err("get need_filter_pd_event fail!\n");
		di->need_filter_pd_event = 0;/*default is clear*/
	}
	hwlog_info("need_filter_pd_event = %d\n", di->need_filter_pd_event);

	ret = of_property_read_u32(of_find_compatible_node(NULL, NULL,
		"huawei,charger"), "force_disable_dc_path",
		&di->force_disable_dc_path);
	if (ret) {
		hwlog_err("get force_disable_dc_path fail!\n");
		di->force_disable_dc_path = 0;
	}
	hwlog_info("force_disable_dc_path = %d\n", di->force_disable_dc_path);

	if (of_property_read_u32(of_find_compatible_node(NULL, NULL,
		"huawei,charger"), "scp_adp_normal_chg",
		&di->scp_adp_normal_chg))
		di->scp_adp_normal_chg = 0;
	hwlog_info("scp_adp_normal_chg = %d\n", di->scp_adp_normal_chg);
