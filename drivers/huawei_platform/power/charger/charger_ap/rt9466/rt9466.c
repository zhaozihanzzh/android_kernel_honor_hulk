	...
	enum rt9466_adc_sel adc_sel, int *adc_val)
{
	...
	if (ret < 0) {
		hwlog_err("select ch to %d fail(%d)\n", adc_sel, ret);
		goto out;
	}
	...
	if (ret < 0) {
		hwlog_err("start con fail(%d), sel = %d\n", ret, adc_sel);
		goto out_unlock_all;
	}
	...
	if (i == max_wait_times) {
		hwlog_err("wait con fail(%d), sel = %d\n", ret, adc_sel);
		ret = -EINVAL;
		goto out_unlock_all;
	}
...
static int rt9466_enable_otg(int en)
{
	...
	ret = rt9466_i2c_write_byte(info, RT9466_REG_CHG_HIDDEN_CTRL4,
		lg_slew_rate);
	if (ret < 0) {
		hwlog_err("%s: set Low side mos Gate drive speed fail(%d)\n",
			__func__, ret);
		goto out;
	}
...
err_en_otg:
	...
	if (ret < 0)
		hwlog_err("%s: recover Low side mos Gate drive speed fail(%d)\n",
			__func__, ret);
	ret = -EIO;
out:
