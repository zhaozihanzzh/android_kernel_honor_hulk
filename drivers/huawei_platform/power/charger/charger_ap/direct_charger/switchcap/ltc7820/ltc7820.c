...
static int ltc7820_probe(struct platform_device *pdev)
{
	...
	ret = power_gpio_config_input(power_gpio_tag(HWLOG_TAG), di->dev_node,
	...
	ret = power_gpio_config_output(power_gpio_tag(HWLOG_TAG), di->dev_node,
		"gpio_en", "ltc7820_gpio_en",
		&di->gpio_en, LTC7820_CHIP_DISABLE);
	if (ret)
		goto ltc7820_fail_1;

	ret = power_gpio_config_interrupt(power_gpio_tag(HWLOG_TAG),
		di->dev_node, "gpio_int", "ltc7820_gpio_int",
