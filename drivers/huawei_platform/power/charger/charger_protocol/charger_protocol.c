...
static int adapter_protocol_type = ADAPTER_PROTOCOL_SCP;
...
{
	if ((type >= ADAPTER_PROTOCOL_BEGIN) && (type < ADAPTER_PROTOCOL_END)) {
		adapter_protocol_type = type;
		return 0;
	}

	adapter_protocol_type = ADAPTER_PROTOCOL_END;
...
int adapter_show_device_info(void)
{
	...
	case ADAPTER_PROTOCOL_SCP:
		...
		hwlog_info("adp_b_type=%d\n", l_dev->info.adp_b_type);
		break;

	case ADAPTER_PROTOCOL_FCP:
		hwlog_info("support_mode=0x%x\n", l_dev->info.support_mode);
		hwlog_info("vendor_id=0x%x\n", l_dev->info.vendor_id);
		hwlog_info("compiledver=0x%x\n", l_dev->info.compiledver);
		hwlog_info("fwver=0x%x\n", l_dev->info.fwver);
		hwlog_info("specver=0x%x\n", l_dev->info.specver);
