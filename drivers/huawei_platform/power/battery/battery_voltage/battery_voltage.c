...
static void hw_batt_vol_sysfs_init_attrs(void)
{
	...
	for (i = 0; i < HW_BATT_VOL_SYSFS_ATTRS_SIZE; i++)
		hw_batt_vol_sysfs_attrs[i] = &hw_batt_vol_sysfs_field_tbl[i].attr.attr;

	hw_batt_vol_sysfs_attrs[i] = NULL;
}

static struct power_sysfs_attr_info *hw_batt_vol_sysfs_field_lookup(
	const char *name)
{
	int i;
	...
	if (i >= HW_BATT_VOL_SYSFS_ATTRS_SIZE)
		return NULL;
