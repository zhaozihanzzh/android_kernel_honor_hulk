...
static void vbus_ch_sysfs_init_attrs(void)
{
	...
	for (s = 0; s < VBUS_CH_SYSFS_ATTRS_SIZE; s++)
		vbus_ch_sysfs_attrs[s] = &vbus_ch_sysfs_field_tbl[s].attr.attr;

	vbus_ch_sysfs_attrs[s] = NULL;
}

static struct power_sysfs_attr_info *vbus_ch_sysfs_field_lookup(
	const char *name)
{
	int s;
