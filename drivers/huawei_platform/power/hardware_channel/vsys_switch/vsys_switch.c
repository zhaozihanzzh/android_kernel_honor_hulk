...
static void vsys_switch_sysfs_init_attrs(void)
{
	...
	for (i = 0; i < VSYS_SWITCH_SYSFS_ATTRS_SIZE; i++)
		vsys_switch_sysfs_attrs[i] = &vsys_switch_sysfs_field_tbl[i].attr.attr;

	vsys_switch_sysfs_attrs[i] = NULL;
}

static struct power_sysfs_attr_info *vsys_switch_sysfs_field_lookup(
	const char *name)
{
	int i;
	...
	if (i >= VSYS_SWITCH_SYSFS_ATTRS_SIZE)
		return NULL;
