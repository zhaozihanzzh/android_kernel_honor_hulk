...
static int sy6510_i2c_read(struct i2c_client *client,
...
static int sy6510_i2c_write(struct i2c_client *client,
...
static int sy6510_read_block(struct sy6510_dev_info *di,
	u8 reg, u8 *data, u8 len)
{
	return sy6510_i2c_read(di->client, &reg, SY6510_ADDR_LEN, data, len);
}

static int sy6510_write_block(struct sy6510_dev_info *di,
	u8 reg, u8 *data, u8 data_len)
{
	u8 cmd[SY6510_ADDR_LEN + data_len];

	cmd[0] = reg;
	memcpy(&cmd[SY6510_ADDR_LEN], data, data_len);

	return sy6510_i2c_write(di->client, cmd, SY6510_ADDR_LEN + data_len);
}
...
static int sy6510_read_byte(u8 reg, u8 *data)
{
	...
	return sy6510_read_block(di, reg, data, CP_BYTE_LEN);
}
...
static int sy6510_write_byte(u8 reg, u8 data)
{
	...
	return sy6510_write_block(di, reg, &data, CP_BYTE_LEN);
}
...
static struct charge_pump_ops sy6510_ops = {
	...
	.reverse_chip_init = sy6510_reverse_chip_init,
};
