...
{
	if (!tag || !np || !prop || !label || !gpio) {
		hwlog_err("tag or np or prop or label or gpio is null\n");
		return -EINVAL;
	}
...
int power_gpio_config_input(const char *tag, struct device_node *np,
	const char *prop, const char *label, unsigned int *gpio)
{
	if (power_gpio_request(tag, np, prop, label, gpio))
		return -EINVAL;
...
{
	char user_name[POWER_GPIO_RD_BUF_SIZE] = { 0 };
	unsigned int gpio;
	unsigned int value;
	...
	if (sscanf(buf, "%s %d %d", user_name, &gpio, &value) != 3) {
		hwlog_err("unable to parse input:%s\n", buf);
		return -EINVAL;
	}

	if (power_gpio_check_op_user(user_name))
		return -EINVAL;
