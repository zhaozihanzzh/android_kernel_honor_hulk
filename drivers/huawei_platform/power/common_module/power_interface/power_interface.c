...
static void power_if_sysfs_init_attrs(void)
{
	...
	for (s = 0; s < POWER_IF_SYSFS_ATTRS_SIZE; s++)
		power_if_sysfs_attrs[s] = &power_if_sysfs_field_tbl[s].attr.attr;

	power_if_sysfs_attrs[s] = NULL;
}

static struct power_sysfs_attr_info *power_if_sysfs_field_lookup(
	const char *name)
{
	int s;
	...
	if (s >= POWER_IF_SYSFS_ATTRS_SIZE)
		return NULL;
