...
static void wireless_sysfs_init_attrs(void)
{
	...
	for (s = 0; s < WIRELESS_SYSFS_ATTRS_SIZE; s++)
		wireless_sysfs_attrs[s] = &wireless_sysfs_field_tbl[s].attr.attr;

	wireless_sysfs_attrs[s] = NULL;
}

static struct power_sysfs_attr_info *wireless_sysfs_field_lookup(
	const char *name)
{
	int s;
	...
	if (s >= WIRELESS_SYSFS_ATTRS_SIZE)
		return NULL;
