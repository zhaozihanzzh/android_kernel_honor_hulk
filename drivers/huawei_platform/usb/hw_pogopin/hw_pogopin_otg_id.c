...
static void pogopin_otg_in_handle(struct pogopin_otg_id_dev *di)
{
	...
	if (pogopin_typec_chg_ana_audio_suport()) {
		pogopin_5pin_typec_set_status(POGO_OTG);
	} else {
...
static void pogopin_otg_out_handle(struct pogopin_otg_id_dev *di)
{
	...
	/* set charger type remove when pogo otg out and typec charging */
	if (pogopin_typec_chg_ana_audio_suport() &&
		((pogopin_get_interface_status() == TYPEC_INTERFACE) ||
		(pogopin_get_interface_status() == POGOPIN_AND_TYPEC)))
