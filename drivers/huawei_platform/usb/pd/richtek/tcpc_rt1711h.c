...
static int tcpm_typec_set_status(uint8_t in)
{
	if (!g_chip_for_reg_read) {
		hwlog_info("g_chip_for_reg_read null\n");
		return 0;
	}

	hwlog_info("%s:%d\n", __func__, in);
	return tcpm_set_state(g_chip_for_reg_read->tcpc, in);
}
...
struct pogopin_cc_ops rt1711h_cc_detect_ops = {
	...
	.typec_set_status = tcpm_typec_set_status,
};
