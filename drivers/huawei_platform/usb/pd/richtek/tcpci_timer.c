...
void tcpc_enable_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	wake_lock_timeout(&tcpc->tcpci_timer_wakelock,
				msecs_to_jiffies(TCPC_TIMER_WAKELOCK_TIMEOUT));
...
int tcpci_timer_init(struct tcpc_device *tcpc_dev)
{
	...
	wake_lock_init(&tcpc_dev->tcpci_timer_wakelock, WAKE_LOCK_SUSPEND,
...
int tcpci_timer_deinit(struct tcpc_device *tcpc_dev)
{
	...
	wake_lock_destroy(&tcpc_dev->tcpci_timer_wakelock);
