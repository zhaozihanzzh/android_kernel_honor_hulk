...
{
	if (!wake_lock_active(&tcpc->tcpci_event_wakelock))
		wake_lock(&tcpc->tcpci_event_wakelock);
}

static void tcpci_event_wake_unlock(struct tcpc_device *tcpc)
{
	if (wake_lock_active(&tcpc->tcpci_event_wakelock))
		wake_unlock(&tcpc->tcpci_event_wakelock);
}
...
int tcpci_event_init(struct tcpc_device *tcpc_dev)
{
	...
	wake_lock_init(&tcpc_dev->tcpci_event_wakelock, WAKE_LOCK_SUSPEND,
...
int tcpci_event_deinit(struct tcpc_device *tcpc_dev)
{
	...
	wake_lock_destroy(&tcpc_dev->tcpci_event_wakelock);
	return 0;
}
