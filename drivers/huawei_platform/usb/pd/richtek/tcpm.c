...
int tcpm_set_state(struct tcpc_device *tcpc_dev, uint8_t pogo_state)
{
	/* get curent typec state */
	uint8_t state = tcpm_inquire_typec_attach_state(tcpc_dev);
	bool no_need_change = false;

	mutex_lock(&tcpc_dev->access_lock);
	if (tcpc_dev->pogopin_state == pogo_state)
		no_need_change = true;
	/* do nothing if typec attached audio */
	if (state == TYPEC_ATTACHED_AUDIO && !no_need_change) {
		tcpc_dev->pogopin_state = pogo_state;
		no_need_change = true;
	}
	...
	/*
	 * disable and enable cc for
	 * typec state unattached when pogo in
	 * and typec devie reconnect when pogo out
	 */
	tcpm_typec_disable_function(tcpc_dev, true);
	mdelay(20); /* wait for typec disable complete */
	mutex_lock(&tcpc_dev->access_lock);
	tcpc_dev->pogopin_state = pogo_state;
	mutex_unlock(&tcpc_dev->access_lock);
	tcpm_typec_disable_function(tcpc_dev, false);
