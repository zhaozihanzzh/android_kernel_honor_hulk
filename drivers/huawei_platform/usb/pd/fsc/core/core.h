...
#endif // FSC_DEBUG
#define POGO_UNSUPPORT 3

#ifdef CONFIG_POGO_PIN
int core_cc_disable(bool disabled);
/*
 * return current statue of pogo when pogo support
 * return pogo unsupport if unsupport pogo function
 */
FSC_U8 fusb_get_pogo_state(void);
void fusb_set_pogo_state(FSC_U8 state);
FSC_BOOL fusb_get_audioacc_state(void);
void fusb_set_audioacc_state(FSC_BOOL state);
#else
static inline FSC_U8 fusb_get_pogo_state(void)
{
	return POGO_UNSUPPORT;
}

static inline void fusb_set_pogo_state(FSC_U8 state)
{
}

static inline FSC_BOOL fusb_get_audioacc_state(void)
{
	return false;
}

static inline void fusb_set_audioacc_state(FSC_BOOL state)
{
}
#endif
