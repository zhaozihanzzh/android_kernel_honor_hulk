...
#include <huawei_platform/power/adapter/adapter_protocol_pd.h>
...
void SetStateAttachedSink(void)
{
	...
	/*
	 * set pd enable when pogo otg plug in
	 * and unset pd when pogo charger plug in
	 */
	if (pogopin_typec_chg_ana_audio_suport() &&
		(fusb_get_pogo_state() == POGO_CHARGER))
		FSC_PRINT("%s:pogopin charger, pd disable\n", __func__);
	else
...
void clearState(void)
{
	...
	if (pogopin_typec_chg_ana_audio_suport() && fusb_get_audioacc_state())
