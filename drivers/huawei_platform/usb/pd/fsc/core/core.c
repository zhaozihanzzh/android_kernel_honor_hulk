...
FSC_U8 g_pogopin_state = POGO_UNSUPPORT;
FSC_BOOL g_audioacc_state;

FSC_U8 fusb_get_pogo_state(void)
{
	return g_pogopin_state;
}

void fusb_set_pogo_state(FSC_U8 state)
{
	g_pogopin_state = state;
}

FSC_BOOL fusb_get_audioacc_state(void)
{
	return g_audioacc_state;
}

void fusb_set_audioacc_state(FSC_BOOL state)
{
	g_audioacc_state = state;
}
