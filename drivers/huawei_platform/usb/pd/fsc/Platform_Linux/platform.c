...
{
	/*
	 * not notify source vbus if pogo otg or pogo charger plug in
	 * for avoid typec state effect pogo device
	 */
	if ((fusb_get_pogo_state() > POGO_NONE) &&
		(fusb_get_pogo_state() < POGO_UNSUPPORT))
		return 0;
...
static void fusb_typec_open_vbus(FSC_BOOL blnEnable)
{
	...
	/*
	 * not open vbus if pogo otg or pogo charger plug in
	 * for avoid typec state effect pogo device
	 */
	if ((fusb_get_pogo_state() > POGO_NONE) &&
		(fusb_get_pogo_state() < POGO_UNSUPPORT))
...
void platform_notify_cc_orientation(CC_ORIENTATION orientation)
{
	...
	if (pogopin_typec_chg_ana_audio_suport()) {
		/* judgment cc orientation and pogo plug in otg or charger */
		if (((fusb_get_pogo_state() == POGO_CHARGER) ||
			(fusb_get_pogo_state() == POGO_OTG)) &&
			(orientation != NONE)) {
			/*
			 * it's an audio accessory if cc1 and cc2 both are ra.
			 *  return if typec unaudio.
			 */
			if ((cc1State != CCTypeRa) || (cc2State != CCTypeRa))
				return;
		}

		/* update audio status when typec audio plug out */
		if ((orientation == NONE) && (fusb_get_audioacc_state() == TRUE))
			fusb_set_audioacc_state(FALSE);
	}
...
void platform_notify_debug_accessory_snk(CC_ORIENTATION orientation)
{
	...
	/* not notify when pogo charger plug in and typec plug purple cable */
	if (fusb_get_pogo_state() == POGO_CHARGER)
		return;
...
int notify_thread_sink_vbus(void* vbus_state)
{
	...
	/* not notify when pogopin charger plug in avoid typec sink effect pogo charge */
	if (fusb_get_pogo_state() == POGO_CHARGER)
		return 0;
...
{
	/*
	 * not notify when pogopin charger plug in avoid
	 * typec charger effect pogo charge
	 */
	if (fusb_get_pogo_state() == POGO_CHARGER) /* pogo snk mode */
		return;
...
void platform_notify_pd_state(SourceOrSink state)
{
	...
	/*
	 * not notify pd state when pgo charger plug in avoid
	 * typec pd state effect pogo charge
	 */
	if (fusb_get_pogo_state() == POGO_CHARGER) /* pogopin_state was snk mode */
		return;
...
void platform_notify_data_role(FSC_BOOL PolicyIsDFP)
{
	...
	/*
	 * not notify data role when pogo otg or charger plug in
	 * avoid err setting information
	 */
	if ((fusb_get_pogo_state() > POGO_NONE) &&
		(fusb_get_pogo_state() < POGO_UNSUPPORT))
		return;
