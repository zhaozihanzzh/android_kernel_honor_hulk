...
int fusb30x_set_state(FSC_U8 pogo_state)
{
	FSC_PRINT("pogopin_state:%d\n", pogo_state);
	/* notify unattach when typec isn't audio */
	if (!fusb_get_audioacc_state())
		SetStateUnattached();
	fusb_set_pogo_state(pogo_state);
	return 0;
}
...
struct pogopin_cc_ops fusb30x_cc_detect_ops = {
	...
	.typec_set_status = fusb30x_set_state,
};
