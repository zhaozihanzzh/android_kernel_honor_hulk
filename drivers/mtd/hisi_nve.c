...
static void update_header_valid_nvme_items(struct NVE_partittion *nve_ramdisk)
{
	...
	if (nve_ramdisk->header.valid_items > NV_ITEMS_MAX_NUMBER) {
		pr_err("update_header_valid_nvme_items valid_items error \n");
		return;
	}
...
int nve_update_and_check_item(unsigned int update_items, unsigned int valid_check_items, struct NVE_partittion *nve_store_ramdisk, struct NVE_partittion *nve_update_ramdisk)
{
	...
	if (update_items > NV_ITEMS_MAX_NUMBER) {
		pr_err("nve_update_and_check_item update_items error \n");
		return -1;
	}
...
static int nve_print_partition_test(struct NVE_partittion *nve_partition)
{
	...
	if (nve_partiiton_header->valid_items > NV_ITEMS_MAX_NUMBER) {
		pr_err("nve_print_partition_test valid_items error \n");
		return -1;
	}
...
int hisi_nve_direct_access_for_ramdisk(struct hisi_nve_info_user *user_info){
	...
	if (user_info->nv_number >= NV_ITEMS_MAX_NUMBER) {
		pr_err("hisi_nve_direct_access_for_ramdisk nv_number error \n");
		return -1;
	}
