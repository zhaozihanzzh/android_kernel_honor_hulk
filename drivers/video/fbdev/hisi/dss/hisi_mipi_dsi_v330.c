...
static void get_dsi_dphy_ctrl(struct hisi_fb_data_type *hisifd, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	n_pll = 2;
	if ((2400 < lane_clock) && (lane_clock <= 2500)) {
		n_pll = 1;
	}
	...
	char __iomem *mipi_dsi_base, struct mipi_dsi_phy_ctrl* phy_ctrl)
{
	...
	hsa_time = round1(pinfo->ldi.h_pulse_width * lane_byte_clk, pixel_clk);
	hbp_time = round1(pinfo->ldi.h_back_porch * lane_byte_clk, pixel_clk);
	hline_time = round1((pinfo->ldi.h_pulse_width + pinfo->ldi.h_back_porch +
		rect.w + pinfo->ldi.h_front_porch) * lane_byte_clk, pixel_clk);
...
int mipi_dsi_bit_clk_upt_isr_handler(struct hisi_fb_data_type *hisifd)
{
	...
	if (!spin_trylock(&hisifd->mipi_resource_lock)) {
		HISI_FB_DEBUG("dsi_bit_clk %u will update in next frame\n",
			dsi_bit_clk_upt);
		return 0;
	}
	...
	ldi_vstate = inp32(hisifd->dss_base + DSS_LDI0_OFFSET + LDI_VSTATE) & 0x7FF;
	...
	if ((!is_ready) || (ldi_vstate != 0x1)) {
		...
		hisifd->mipi_dsi_bit_clk_update = 0;
		HISI_FB_DEBUG("PERI_STAT0 is not ready or ldi_vstate=0x%x not in idle\n",
			ldi_vstate);
		return 0;
	}

	HISI_FB_DEBUG("start, phy status: 0x%x, vstate: 0x%x\n",
			inp32(hisifd->mipi_dsi0_base + MIPIDSI_PHY_STATUS_OFFSET),
			inp32(hisifd->mipi_dsi0_base + LDI_VSTATE));
	...
	if (is_mipi_cmd_panel(hisifd)) {
		...
		HISI_FB_DEBUG("after, phy status: 0x%x, vstate: 0x%x\n",
			inp32(hisifd->mipi_dsi0_base + MIPIDSI_PHY_STATUS_OFFSET),
			inp32(hisifd->mipi_dsi0_base + LDI_VSTATE));
