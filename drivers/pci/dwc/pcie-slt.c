...
/* normal, low, high */
static const u32 ldo30_vals[3] = { 0x19, 0x14, 0x20 };
static const u32 ldo37_vals[3] = { 0x0A, 0x04, 0x16 };
static const u32 buck2_vals[3] = { 0x14, 0x14, 0x1F };
static const u32 buck9_vals[3] = { 0x28, 0x28, 0x31 };
...
static int wait_for_power_status(struct kirin_pcie *pcie, enum aspm_stat mode)
{
	...
	PCIE_PR_ERR("PCIECTRL_STATE4: 0x%x, PCIECTRL_STATE5: 0x%x",
		    status4, status5);
	return mode;
...
static int apr_setup(void *data)
{
	...
	if (ret) {
		PCIE_PR_ERR("Failed to power EP");
...
static int apr_datatransfer(void *ddr, u32 size, u32 dir)
{
	...
	/* Write to EP Ram */
	if (dir == H2D)
		result = memcpy_s((void *)(uintptr_t)wl_cpu_base, SIZE_M,
				  (void *)(uintptr_t)ddr, size);
	else if (dir == D2H)
		result = memcpy_s((void *)(uintptr_t)ddr, SIZE_M,
				  (void *)(uintptr_t)wl_cpu_base, size);
	else
		result = -EINVAL;

	if (result != EOK) {
		PCIE_PR_ERR("Fail to do memcpy, errno = %d", result);
		goto TEST_FAIL_UNMAP;
	}
			     ...
			     void *slt_data_cmp, u32 loop_times)
{
	...
	if (!g_pcie_slt_cfg[pcie->rc_id].ep_ops.data_transfer) {
		PCIE_PR_ERR("maybe NullFuncPointer!");
		return -EINVAL;
	}
	...
	for (j = 0; j < loop_times; j++) {
		for (index = 0; index < DATA_ARR_SIZE; index++) {
			...
			if (ret0 != EOK || ret1 != ret2 || ret3) {
				PCIE_PR_ERR("Data transfer failed[%u, %d], ret1: %d, ret2: %d, ret3: %d",
...
static enum pcie_test_result pcie_slt_vary_voltage_test(struct kirin_pcie *pcie, u32 loop_times)
{
	...
	if (pcie->dtsinfo.ep_device_type == EP_DEVICE_BCM ||
		...
		PCIE_PR_ERR("Bypass, ep_device_type: %u", pcie->dtsinfo.ep_device_type);
		return ret;
	}
...
static void pcie_slt_resource_init(struct kirin_pcie *pcie)
{
	if (pcie->dtsinfo.ep_device_type == EP_DEVICE_APR) {
		...
		if (pcie_slt_hook_register(APR_RC_ID, EP_DEVICE_APR, NULL, NULL, NULL, apr_setup, apr_datatransfer)) {
...
static long pcie_slt_ioctl(struct file *file, u_int cmd, unsigned long result)
{
	...
	case PCIETESTCMD_NEW:
		...
		if (scb.rc_id >= g_rc_num || scb.loop_times <= 0 || scb.loop_times > PCIE_SLT_DATA_MAX_LOOP_TIMES) {
			PCIE_PR_ERR("InputPara is invalid, rc_id[%u], loop_times[%u]", scb.rc_id, scb.loop_times);
			ret = -1;
			goto FAIL;
		}
