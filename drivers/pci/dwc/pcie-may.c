...
static int pcie_noc_power(struct kirin_pcie *pcie, u32 exit)
{
	...
	if (exit) {
		...
		while ((val & val_bits) != val_bits) {
			udelay(1);
			if (!time) {
				PCIE_PR_ERR("Enter failed :%u", val);
				return -1;
			}
			time--;
			val = readl(pcie->pmctrl_base + NOC_POWER_IDLE);
...
static int pcie_fnpll_ctrl(struct kirin_pcie *pcie, u32 enable)
{
	...
	const u32 cfg6 = 6;
	const u32 cfg7 = 7;
	u32 pll_cfg_base = PLL_CFG0;
	u32 pciepll_state = PCIEPLL_STATE;
	const u32 pll_cfg_val[] = { 0x1C000004, 0x00B40000, 0x20101FA8,
	...
	val = hsdt_crg_reg_read(pcie, FNPLL_CFG(pll_cfg_base, cfg6));
	val |= FNPLL_BP_BIT;
	val &= ~FNPLL_EN_BIT;
	/* FNPLL Disable and Bypass take effect */
	hsdt_crg_reg_write(pcie, val, FNPLL_CFG(pll_cfg_base, cfg6));
	...
	if (enable) {
		...
		/* Set VCO : 3.0GHz,FOUTPOSTDIV:100MHz,en:0,bypass:1 */
		val = hsdt_crg_reg_read(pcie, FNPLL_CFG(pll_cfg_base, cfg6));
		val &= ~(0xFFFFFFFF);
		val |= 0x404E5012;
		hsdt_crg_reg_write(pcie, val, FNPLL_CFG(pll_cfg_base, cfg6));

		/* fnpll_pll_mode: 0x06200000 */
		val = hsdt_crg_reg_read(pcie, FNPLL_CFG(pll_cfg_base, cfg7));
		val &= ~(0xFFFFFFFF);
		val |= 0x06200000;
		hsdt_crg_reg_write(pcie, val, FNPLL_CFG(pll_cfg_base, cfg7));
		...
		while (i < FNPLL_LOCK_TIMEOUT) {
			if (val & FNPLL_LOCK_BIT) {
				...
				val &= ~FNPLL_BP_BIT;
				hsdt_crg_reg_write(pcie, val, FNPLL_CFG(pll_cfg_base, cfg6));
				PCIE_PR_INFO("FNPLL lock in %u us", i);
				return 0;
			}
			udelay(1);
			i++;
...
static int kirin_pcie_clk_config(struct kirin_pcie *pcie, enum pcie_clk_type clk_type, u32 enable)
{
	...
	case PCIE_EXTERNAL_CLK:
		...
		if (pcie->rc_id == PCIE_RC1) {
			...
			if (ret) {
				PCIE_PR_ERR("Failed to set axi_aclk rate");
				break;
			}
		}
		...
		break;
	default:
		PCIE_PR_ERR("Invalid input parameters");
		ret = -EINVAL;
	}
...
{
	if (en_flag) {
		writel((pcie->dtsinfo.iso_info[1] | (pcie->dtsinfo.iso_info[1] << 16)),
...
static int kirin_pcie_turn_on(struct kirin_pcie *pcie, enum rc_power_status on_flag)
{
	...
	/* unrst pcie_phy_apb_presetn pcie_ctrl_apb_presetn pcie_ctrl_por_n */
	...
	/* Enable pcie1pll serdes clk */
	if (pcie->rc_id == PCIE_RC1) {
		ret = clk_prepare_enable(pcie->pcie1_serdes_clk);
		if (ret) {
			PCIE_PR_ERR("Failed to enable pcie1_serdes_clk");
			goto SERDES_CLK;
		}
	}
...
REF_CLK:
	if (pcie->rc_id == PCIE_RC1)
		clk_disable_unprepare(pcie->pcie1_serdes_clk);
SERDES_CLK:
...
int pcie_plat_init(struct platform_device *pdev, struct kirin_pcie *pcie)
{
	...
	if (pcie->rc_id == PCIE_RC1) {
		...
		if (IS_ERR(pcie->pcie1_serdes_clk)) {
			PCIE_PR_ERR("Failed to get pcie1_serdes_clk");
