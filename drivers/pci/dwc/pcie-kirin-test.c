...
#define PIPELOOPBACK_ENABLE   (0x1 << 31)
...
int check_pcie_on_work(u32 rc_id)
{
	...
	if (!test_kirin_pcie->pcie) {
		PCIE_PR_ERR("PCIe%u is null", rc_id);
		return -EINVAL;
	}
...
long int show_trans_rate(u64 begin_time, u64 end_time, u64 size)
{
	...
	PCIE_PR_INFO("Data size [%llu], time sapce [%llu]",
		     (size * sizeof(char) * 8 * 100 * TRANSFER_TIMES), time_count);
	PCIE_PR_INFO("Time sapce [%llu]. Transferring Rate is :[%ld] Bit/s",
		     time_count, rate);
...
void show_pci_dev_state(u32 rc_id)
{
	...
	if (pm) {
		...
		PCIE_PR_INFO("RC: pci_pm_ctrl reg value = [0x%x]",
	...
	if (pm) {
		...
		PCIE_PR_INFO("EP: pci_pm_ctrl reg value = [0x%x]",
			     reg_value & PCI_PM_CTRL_MASK);
	}
}
...
int rc_read_ep_by_dma(u64 addr, u32 size, u32 rc_id)
{
	...
	if (chan) {
		...
		if (status != DMA_COMPLETE) {
			PCIE_PR_ERR("DMA transfer fail, error: %d", status);
			goto release_dma_channel;
		}
	} else {
...
static void calc_rate(const char *desc, u32 size, u64 start_time, u64 end_time)
{
	...
	PCIE_PR_INFO("%s total size: %llu MB, cost times: %llu ns, rate:%llu Mb/s",
...
int dma_trans_rate(u32 rc_id, int direction, u32 size, u64 addr)
{
	...
	kirin_pcie_write_dbi(pcie->pci, pcie->pci->dbi_base, BAR0_SHADOW_REG, REG_DWORD_ALIGN, 0xfffffff);
...
int rc_read_ep_cfg(u32 rc_id)
{
	...
	ep_vaddr_in_cpu = pci_resource_start(pcie->ep_dev, 0);

	PCIE_PR_INFO("EP addr in cpu physical is [0x%pK]", (void *)(uintptr_t)ep_vaddr_in_cpu);

	kirin_pcie_outbound_atu(rc_id, KIRIN_PCIE_ATU_REGION_INDEX0,
				KIRIN_PCIE_ATU_TYPE_CFG0, ep_vaddr_in_cpu,
				busdev, PCI_CONFIG_SPACE);
	...
	for (i = 0; i < PCI_CONFIG_SPACE; i += REG_DWORD_ALIGN)
		PCIE_PR_INFO("Wifi cfg register: offset[%u] = [0x%x]", i,
...
static void show_diff(u64 rc_addr, u64 ep_addr, unsigned int size)
{
	...
	for (i = 0; i < size; i += REG_DWORD_ALIGN) {
		...
		if (rc_val != ep_val)
			PCIE_PR_INFO("[0x%x]: rc_val=[0x%x], ep_val=[0x%x]", i, rc_val, ep_val);
	}
}
...
int rc_read_ep_mem(u32 rc_id, unsigned int size, int index)
{
	...
	if (ret != EOK) {
		PCIE_PR_ERR("Failed to set default val[0xFF]");
		goto FAIL;
	}
	ret = memset_s((void *)(uintptr_t)test_kirin_pcie->rc_rdmem_addr, TEST_MEM_SIZE, 0x0, size);
	if (ret != EOK) {
		PCIE_PR_ERR("Failed to set default val[0x0]");
		goto FAIL;
	}
	...
	for (i = 0; i < TRANSFER_TIMES; i++) {
		ret = memcpy_s((void *)(uintptr_t)test_kirin_pcie->rc_rdmem_addr,
			       TEST_MEM_SIZE, (void *)(uintptr_t)cpu_addr, size);
		if (ret != EOK) {
			PCIE_PR_ERR("Fail to do memcpy, errno = %d", ret);
			goto FAIL;
		}
	}
...
int set_ep_mem_inbound(u32 rc_id, int index)
{
	...
	PCIE_PR_INFO("Inbound pci_add [0x%pK] to cpu_addr[0x%pK]",
...
int rc_write_ep_mem(u32 rc_id, unsigned int size, int index)
{
	...
	if (ret != EOK) {
		PCIE_PR_ERR("Failed to set default val[0x56]");
		goto FAIL;
	}
	...
	for (i = 0; i < TRANSFER_TIMES; i++) {
		ret = memcpy_s((void *)(uintptr_t)cpu_addr, TEST_MEM_SIZE,
...
int ep_read_rc_mem(u32 rc_id, unsigned int size, int index)
{
	...
	for (i = 0; i < TRANSFER_TIMES; i++) {
		ret = memcpy_s((void *)(uintptr_t)test_kirin_pcie->ep_rdmem_addr, TEST_MEM_SIZE,
...
int ep_write_rc_mem(u32 rc_id, unsigned int size, int index)
{
	...
	if (ret != EOK) {
		PCIE_PR_ERR("Failed to set default val[0xDE]");
		goto FAIL;
	}
...
int kirin_pcie_ep_mac_init(u32 pcie_id)
{
	...
	if (pcie_id >= g_rc_num) {
		PCIE_PR_ERR("There is no pcie_id = %u", pcie_id);
		return -EINVAL;
	}
...
void ep_triggle_intr(u32 rc_id)
{
	...
	value_temp &= ~PCIE_VEN_MSI_REQ;
	kirin_elb_writel(pcie, value_temp, SOC_PCIECTRL_CTRL2_ADDR);
	PCIE_PR_INFO("Read after write, ctl2 is %x", kirin_elb_readl(pcie,
...
void print_credit_info(u32 rc_id)
{
	...
	PCIE_PR_INFO("POST Cre_dit info:	Header:%u	Data:%u", header, data);
	get_pcie_credit_info(pcie, TYPE_NONPOST, &header, &data);
	PCIE_PR_INFO("Non-POST Cre_dit info:	Header:%u	Data:%u", header, data);
	get_pcie_credit_info(pcie, TYPE_CPL, &header, &data);
	PCIE_PR_INFO("Completion Cre_dit info:	Header:%u	Data:%u", header, data);
...
#ifdef CONFIG_KIRIN_PCIE_L1SS_IDLE_SLEEP
	devm_free_irq(&pdev->dev, (unsigned int)pcie->irq[IRQ_IDLE_TIMEOUT].num, (void *)pcie);
	devm_free_irq(&pdev->dev, (unsigned int)pcie->irq[IRQ_IDLE_WAKE].num, (void *)pcie);
#endif
...
static int kirin_pcie_ep_probe(struct platform_device *pdev)
{
	...
	PCIE_PR_INFO("PCIe EP No.%u probe", pcie->rc_id);
