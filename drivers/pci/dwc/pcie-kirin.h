...
#define IRQ_IDLE_TIMEOUT	6
#define IRQ_IDLE_WAKE		7
#define EXTRA_IRQ_NUM		3
#else
...
struct kirin_pcie {
	...
	u32					ep_link_status;
#ifdef CONFIG_KIRIN_PCIE_L1SS_IDLE_SLEEP
	struct kirin_pcie_idle_sleep		*idle_sleep;
#endif
	/* for EP callbacks */
...
#define PCIE_PR_ERR(fmt, args ...)	pr_err("[Kirin_pcie][ERR]:" fmt "\n", ##args)

#define PCIE_PR_INFO(fmt, args ...)	pr_info("[Kirin_pcie][INF]:" fmt "\n", ##args)

#define PCIE_PR_DEBUG(fmt, args ...)	pr_debug("[Kirin_pcie][DBG]:" fmt "\n", ##args)
