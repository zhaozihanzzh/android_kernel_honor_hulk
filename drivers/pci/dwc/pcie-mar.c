...
static void kirin_pcie_fppll_reg_config(struct kirin_pcie *pcie)
{
	u32 reg_val;
...
static int kirin_pcie_fnpll_ctrl(struct kirin_pcie *pcie, bool enable)
{
	...
	if (enable) {
		kirin_pcie_fppll_reg_config(pcie);

		reg_val = readl(pcie->crg_base + MMC1_PCIEPLL_STAT_SYSCTRL);
		while (index) {
			if (reg_val & FNPLL_LOCK_STAT_BIT) {
				PCIE_PR_INFO("FNPLL lock in %u us", (FNPLL_LOCK_TIMEOUT - index));
...
static void kirin_pcie_hppll_reg_config(struct kirin_pcie *pcie)
{
	u32 reg_val;
...
static int kirin_pcie_hppll_ctrl(struct kirin_pcie *pcie, bool enable)
{
	...
	if (enable) {
		...
		reg_val |= FNPLL_UNLOCK_INTR_BYPASS;
		writel(reg_val, pcie->crgperiph_base + PLL_UNLOCK_DETECT_INTR_BYPSS);

		kirin_pcie_hppll_reg_config(pcie);
		...
		while (index) {
			if (reg_val & HPPLL_LOCK_STAT_BIT) {
				PCIE_PR_INFO("HPPLL lock in %u us", (HPPLL_LOCK_TIMEOUT - index));
