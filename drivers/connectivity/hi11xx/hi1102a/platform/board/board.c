...
static int32 get_board_id_from_name(const char *board_name)
{
    ...
    for (index = 0; index < OAL_ARRAY_SIZE(device_board_version_list); index++) {
        if (!oal_strcmp(device_board_version_list[index].name, board_name)) {
            return device_board_version_list[index].index;
        }
    }
...
{
    return board_info.bd_ops.board_wifi_tas_gpio_init();
}
...
{
    board_info.bd_ops.free_board_wifi_tas_gpio();
}
...
{
    if (board_info.wifi_tas_enable == WIFI_TAS_DISABLE) {
        return 0;
    }
    ...
    if (value) {
        return gpio_direction_output(board_info.rf_wifi_tas, GPIO_HIGHLEVEL);
    } else {
        return gpio_direction_output(board_info.rf_wifi_tas, GPIO_LOWLEVEL);
    }
}
...
int32 board_get_wifi_tas_gpio_state(void)
{
    ...
    return gpio_get_value(board_info.rf_wifi_tas);
}
...
{
    return (board_info.wifi_tas_enable == WIFI_TAS_ENABLE);
}
...
int32 board_get_wifi_tas_gpio_init_sts(int32 *gpio_sts)
{
    ...
    *gpio_sts = board_info.wifi_tas_gpio_init;
    return BOARD_SUCC;
}
...
void hi1102a_board_ops_init(void)
{
    ...
    board_info.bd_ops.board_wifi_tas_gpio_init = hi1102a_board_wifi_tas_gpio_init;
    board_info.bd_ops.free_board_wifi_tas_gpio = hi1102a_free_board_wifi_tas_gpio;
...
int32 board_func_init(void)
{
    ...
    PS_PRINT_INFO("board_info.chip_nr=%d\n", board_info.chip_nr);
    return BOARD_SUCC;
}
...
void ssi_read_low_power_state(void)
{
    ...
    st_ssi_cpu_infos.pwr_dbg.wifi_dbg = reg;
...
static void chr_cpu_info_rpt(void)
{
    ...
    if (halt_det_cnt || (hi11xx_kernel_crash)) {
        PS_PRINT_INFO("halt_det_cnt=%u hi11xx_kernel_crash=%d chr_cpu_info_rpt return\n",
                      halt_det_cnt, hi11xx_kernel_crash);
        return;
    }
    ...
    for (i = 0; i < SSI_CPU_ARM_REG_DUMP_CNT; i++) {
        ...
        CHR_EXCEPTION_REPORT(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_PC_LR, st_ssi_cpu_infos.wcpu_info.pc[i],
                             st_ssi_cpu_infos.wcpu_info.lr[i], st_ssi_cpu_infos.pwr_dbg.wifi_dbg);
        break;
    }
}
