...
int32 hi1102a_board_wifi_tas_gpio_init(void)
{
    ...
    if (of_property_read_bool(np, DTS_PROP_WIFI_TAS_EN)) {
        ...
        board_info.wifi_tas_enable = WIFI_TAS_ENABLE;
    } else {
        PS_PRINT_INFO("Hi110x wifi tas not enable\n");
        board_info.wifi_tas_enable = WIFI_TAS_DISABLE;
        return BOARD_SUCC;
    }
    ...
    board_info.rf_wifi_tas = physical_gpio;
    PS_PRINT_INFO("hi110x wifi tas gpio is %d\n", board_info.rf_wifi_tas);
    ...
    board_info.wifi_tas_gpio_init = (wifi_tas_state == 0) ? GPIOF_INIT_LOW : GPIOF_INIT_HIGH;
...
{
    gpio_free(board_info.rf_wifi_tas);
}
