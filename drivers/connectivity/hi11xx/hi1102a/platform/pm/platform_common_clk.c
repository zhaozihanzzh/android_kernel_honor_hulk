...
nv_part_str gst_dcxo_nv_part[] = {
    ...
    {   NV_GNSS_ID, NV_GNSS_SIZE, OAL_SIZEOF(dcxo_nv_info_stru) - OAL_SIZEOF(dcxo_nv_coeff_th_stru),
...
oal_void update_dcxo_coeff(oal_uint32 *coeff)
{
    ...
    ret = memcpy_s(pst_para->coeff.para, sizeof(pst_para->coeff.para), coeff, GNSS_COEFF_NUM * OAL_SIZEOF(oal_int32));
    if (ret != EOK) {
    ...
    for (loop = 0; loop < COEFF_NUM; loop++) {
        PS_PRINT_INFO("[dcxo] ioctl update coeff  a[%d] = 0x%x \n", loop, pst_para->coeff.para[loop]);
    }
}
...
oal_int32 process_plat_cali(oal_uint8 *pc_buffer, dcxo_dl_para_stru *pst_dl_para)
{
    ...
    ret = memcpy_s(pl_para, sizeof(pst_dl_para->coeff.para), pc_buffer, sizeof(default_coeff));
    if (ret != EOK) {
...
oal_int32 process_factory_cali(oal_uint8 *pc_buffer, dcxo_dl_para_stru *pst_dl_para)
{
    ...
    coeff_fac = &(factory.coeff_u.fac);

    if (DCXO_NV_CHK_OK != factory.valid) {
        PS_PRINT_ERR("[dcxo] read factory cali buffer error func [%s]\n", __func__);
        goto report_chr;
    }

    pl_coeff = coeff_fac->para;

    // 此处检测到工厂分区t0使用的是10位，需要修正到16位
    if (T0_BIT_WIDTH_10 == factory.coeff_th.t0_bit_width_fix) {
    ...
    ret = memcpy_s(&(pst_dl_para->coeff), OAL_SIZEOF(dcxo_coeff_stru), coeff_fac, OAL_SIZEOF(dcxo_coeff_stru));
    if (ret != EOK) {
        PS_PRINT_ERR("[dcxo] process_factory_cali: memcpy_s faild, ret = %d", ret);
        goto report_chr;
    }
...
report_chr:
    ...
    ret = memcpy_s(chr_nv_rd.l_para, sizeof(chr_nv_rd.l_para), coeff_fac, COEFF_NUM * OAL_SIZEOF(oal_int32));
    if (ret == EOK) {
...
oal_int32 process_gnss_self_study(oal_uint8 *pc_buffer, dcxo_dl_para_stru *pst_dl_para)
{
    ...
    if (nv_coeff_vld(pl_coeff, GNSS_COEFF_NUM, NV_GNSS_NAME) == INI_FAILED) {
        PS_PRINT_ERR("[dcxo] factory coeff data check failed\n");
        goto report_chr;
    }

    ret = memcpy_s(pst_dl_para->coeff.para, sizeof(pst_dl_para->coeff.para),
                   pst_info->coeff_u.gnss.para, OAL_SIZEOF(*pl_coeff) * GNSS_COEFF_NUM);
    if (ret != EOK) {
...
report_chr:
    ...
    chr_nv_rd.l_struct_len = OAL_SIZEOF(plat_chr_nv_rd_excep_stru);
    ret = memcpy_s(chr_nv_rd.l_para, sizeof(chr_nv_rd.l_para),
                   pst_info->coeff_u.gnss.para, GNSS_COEFF_NUM * OAL_SIZEOF(oal_int32));
    if (ret == EOK) {
        report_chr_err(PLAT_CHR_ERRNO_DCXO_NV_RD, (oal_uint8 *)&chr_nv_rd, OAL_SIZEOF(chr_nv_rd));
    }
    return INI_FAILED;
...
oal_int32 get_dcxo_coeff(dcxo_dl_para_stru *pst_dl_para)
{
    ...
    for (loop = 0; loop < nv_size; loop++) {
        ...
        if (NULL != gst_dcxo_nv_part[loop].process_cali) {
