...
OAL_STATIC oal_int32 wal_atcmsrv_ioctl_set_country(oal_net_device_stru *pst_net_dev, oal_int8 *puc_countrycode)
{
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
                         "{wal_atcmsrv_ioctl_set_country::regdomain_update_for_dfs return err code %d!}", l_ret);
        return l_ret;
    }
#endif
                                                            ...
                                                            oal_int32 l_gpio_level)
{
    ...
    if (l_gpio_level == 0) {
        ...
        if (g_uc_dev_lte_gpio_level == 0x64) {
...
OAL_STATIC oal_int32 wal_atcmsrv_ioctl_lte_gpio_check(oal_net_device_stru *pst_net_dev)
{
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, 0, "wal_atcmsrv_ioctl_lte_gpio_check:gpio low mode fail[%x]", g_uc_dev_lte_gpio_level);
        wal_atcmsrv_ioctl_lte_gpio_free(pst_net_dev);
        return l_ret;
    }
    ...
    if (l_ret != 0) {
        OAM_WARNING_LOG1(0, 0, "wal_atcmsrv_ioctl_lte_gpio_check:gpio high mode FAIL[%x]!", g_uc_dev_lte_gpio_level);
        wal_atcmsrv_ioctl_lte_gpio_free(pst_net_dev);
        return l_ret;
    }
...
{
    g_pd_bss_expire_time = (ul_expire_age < WAL_ATCMSRV_MIN_BSS_EXPIRATION_AGE) ? WAL_ATCMSRV_MIN_BSS_EXPIRATION_AGE :
                           ul_expire_age;
    g_pd_bss_expire_time = (g_pd_bss_expire_time > WAL_ATCMSRV_MAX_BSS_EXPIRATION_AGE)
                           ? WAL_ATCMSRV_MAX_BSS_EXPIRATION_AGE
                           : g_pd_bss_expire_time;
    ...
    switch (st_priv_cmd.ul_cmd) {
        ...
        case WAL_ATCMDSRV_IOCTL_CMD_GET_CONN_INFO:
            ...
            OAL_IO_PRINT("wal_atcmsrv_ioctl_get_wifi_connect_info::status %d, %.32s,%02x:%02x:xx:xx:%02x:%02x,%d",
                         ...
                         st_priv_cmd.pri_data.st_connect_info.auc_bssid[4],
                         st_priv_cmd.pri_data.st_connect_info.auc_bssid[5],
