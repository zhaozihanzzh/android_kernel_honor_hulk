...
oal_uint32 wal_flowctl_backp_event_handler(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (pst_flowctl_backp_event->us_assoc_id == 0xFFFF) {
        if (pst_flowctl_backp_event->uc_is_stop == 1) {
            ...
            OAM_INFO_LOG2(uc_vap_id, OAM_SF_ANY,
                          "{wal_flowctl_backp_event_handler::oal_net_tx_stop_all_queues,asoc_id=%d,tid=%d,mpdu_num=%d}",
                          pst_flowctl_backp_event->us_assoc_id,
                          pst_flowctl_backp_event->uc_tidno);
        } else {
            oal_net_tx_wake_all_queues(pst_net_device);
            OAM_INFO_LOG2(uc_vap_id, OAM_SF_ANY,
                          "{wal_flowctl_backp_event_handler::oal_net_tx_wake_all_queues,asoc_id=%d,tid=%d,mpdu_num=%d}",
                          pst_flowctl_backp_event->us_assoc_id,
                          pst_flowctl_backp_event->uc_tidno);
        }
    ...
    if (pst_flowctl_backp_event->uc_tidno == WLAN_TID_MAX_NUM) {
        for (uc_ac = 0; uc_ac <= MAC_LINUX_SUBQ_VO; uc_ac++) {
            ...
            if (pst_flowctl_backp_event->uc_is_stop == 1) {
                ...
                OAM_INFO_LOG2(uc_vap_id, OAM_SF_ANY,
                              "{wal_flowctl_backp_event_handler::oal_net_stop_subqueue,assoc_id=%d,tid=%d,mpdu_num=%d}",
                              pst_flowctl_backp_event->us_assoc_id,
                              pst_flowctl_backp_event->uc_tidno);
            } else {
                oal_net_wake_subqueue(pst_net_device, us_subq_idx);
                OAM_INFO_LOG2(uc_vap_id, OAM_SF_ANY,
                              "{wal_flowctl_backp_event_handler:oal_net_wake_subqueue,assoc_id=%d,tid= %d,mpdu_num=%d}",
                              pst_flowctl_backp_event->us_assoc_id,
                              pst_flowctl_backp_event->uc_tidno);
            }
        }
