...
oal_void hmac_get_rx_filter_en(oal_void)
{
    ...
    if (snprintf_s(ac_tmp_buff, OAL_SIZEOF(ac_tmp_buff), OAL_SIZEOF(ac_tmp_buff) - 1,
        ...
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_get_rx_filter_en::snprintf_s fail}");
    }
    oam_print(ac_tmp_buff);
}


oal_uint32 hmac_set_status_rx_filter_val(oal_uint uc_dev_mode, oal_uint uc_vap_mode, oal_uint uc_vap_status,
                                         oal_uint32 uc_val)
{
    switch (uc_dev_mode) {
        case 0:
...
oal_uint32 hmac_get_status_rx_filter_val(oal_uint uc_dev_mode, oal_uint uc_vap_mode, oal_uint uc_vap_status)
{
    ...
    oal_int8 *pac_vapstatus2string[] = { "INIT", "UP", "PAUSE", "WAIT_START", "AP_PAUSE", "FAKE_UP", "WAIT_SCAN",
                                         "SCAN_COMP", "WAIT_JOIN", "JOIN_COMP", "WAIT_AUTH_SEQ2", "WAIT_AUTH_SEQ4",
                                         "AUTH_COMP", "STA_WAIT_ASOC", "STA_OBSS_SCAN", "STA_BG_SCAN", "error" };
    switch (uc_dev_mode) {
        case 0:
