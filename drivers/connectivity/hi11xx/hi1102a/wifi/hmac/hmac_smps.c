...
oal_void hmac_smps_all_vap_update(mac_device_stru *pst_mac_device,
                                   ...
                                   oal_bool_enum_uint8 en_plus_user)
{
    ...
    if (OAL_ANY_NULL_PTR2(pst_mac_vap, pst_mac_user)) {
        OAM_ERROR_LOG2(0, OAM_SF_SMPS, "{hmac_smps_update_status: NULL PTR pst_mac_vap[%x]pst_mac_user[%x].}",
                       pst_mac_vap, pst_mac_user);
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SMPS,
                         "{hmac_smps_update_status::mac_vap_find_user_by_macaddr failed[%d].}", ul_ret);
    ...
    FRW_EVENT_HDR_INIT(&(pst_event->st_event_hdr), FRW_EVENT_TYPE_WLAN_CTX,
        DMAC_WLAN_CTX_EVENT_SUB_TYPE_SET_SMPS, OAL_SIZEOF(st_smps_mode),
        FRW_EVENT_PIPELINE_STAGE_1, pst_mac_vap->uc_chip_id, pst_mac_vap->uc_device_id, pst_mac_vap->uc_vap_id);

    /* 拷贝参数 */
    if (memcpy_s(pst_event->auc_event_data, OAL_SIZEOF(st_smps_mode), (oal_void *)&st_smps_mode,
        OAL_SIZEOF(st_smps_mode)) != EOK) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_SMPS, "{hmac_smps_update_status:: memcopy_s fail!}");
        FRW_EVENT_FREE(pst_event_mem);
        return OAL_FAIL;
    }
                                      ...
                                      mac_vap_stru *pst_mac_vap)
{
    ...
    } else if (uc_prev_smps_mode == WLAN_MIB_MIMO_POWER_SAVE_MIMO) { /* 第N次关联请求 由MIMO->DYNAMIC STATIC */
    ...
    } else { /* 第N次关联请求 由DYNAMIC STATIC -> MIMO */
