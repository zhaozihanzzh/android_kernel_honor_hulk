...
oal_void hmac_scan_ct_init(oal_void)
{
                                                      ...
                                                      oal_uint8 *puc_ie, oal_int32 l_len)
{
    ...
    if ((us_pcip_num == 0) || (us_pcip_num > l_ie_len_left / 4)) {
                                                                  ...
                                                                  oal_uint16 us_frame_len)
{
    ...
    if (OAL_PTR_NULL !=
        mac_find_vendor_ie(MAC_WLAN_CHIP_OUI_ATHEROSC, MAC_WLAN_CHIP_OUI_TYPE_ATHEROSC, puc_frame_body + us_offset,
                           us_frame_len - us_offset)) {
...
oal_uint32 hmac_scan_proc_scan_req_event(hmac_vap_stru *pst_hmac_vap, oal_void *p_params)
{
    ...
    /* 扫描停止模块测试 */
    if (((g_en_bgscan_enable_flag == HMAC_BGSCAN_DISABLE) &&
         (pst_hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_UP))
...
oal_uint32 hmac_scan_rrm_proc_save_bss(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "(hmac_scan_rrm_proc_save_bss:fail to find user by \
            xx:xx:xx:0x:0x:0x.}", pst_mac_user->auc_user_mac_addr[3], pst_mac_user->auc_user_mac_addr[4],
...
oal_uint32 hmac_scan_test(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    memcpy_s(ac_param, OAL_SIZEOF(ac_param), pst_scan_test->ac_scan_type, OAL_SIZEOF(pst_scan_test->ac_scan_type));
    memset_s(&st_scan_req, OAL_SIZEOF(st_scan_req), 0, OAL_SIZEOF(st_scan_req));

    if (strcmp("2g", ac_param) == 0) {
        en_2g_all = OAL_TRUE;
    } else if (strcmp("5g", ac_param) == 0) {
        en_5g_all = OAL_TRUE;
    } else if (strcmp("all", ac_param) == 0) {
        en_2g_all = OAL_TRUE;
        en_5g_all = OAL_TRUE;
    } else {
        /* 指定单个信道 */
