...
oal_void hmac_dfs_set_channel(mac_vap_stru *pst_mac_vap, oal_uint8 uc_channel)
{
    ...
    pst_mac_device = mac_res_get_dev(pst_mac_vap->uc_device_id);
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS, "{pst_device(%d) null.}", pst_mac_vap->uc_device_id);
        return;
    }
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_dfs_set_channel::get_idx failed[%d].}", ul_ret);
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if ((pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
            ...
            if (uc_channel == pst_vap->st_channel.uc_chan_number) {
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_DFS, "{same channel%d,no need to change.}", uc_channel);
                return;
            }
                                               ...
                                               wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    ...
    pst_mac_device = mac_res_get_dev(pst_mac_vap->uc_device_id);
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{pst_device(%d) null.}", pst_mac_vap->uc_device_id);
        return;
    }
    ...
    ul_ret = mac_get_channel_idx_from_num(pst_mac_vap->st_channel.en_band, uc_channel, &uc_chan_idx);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{switch_to_new_channel::get idx failed[%d].}", ul_ret);
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        pst_vap = (mac_vap_stru *)mac_res_get_mac_vap(pst_mac_device->auc_vap_id[uc_vap_idx]);
        if (pst_vap == OAL_PTR_NULL) {
            OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{chan_multi_switch_to_new_channel::pst_ap(%d) null.}",
                           pst_mac_device->auc_vap_id[uc_vap_idx]);
            continue;
        }
    ...
    if (pst_channel != OAL_PTR_NULL) {
        ...
        if ((mac_get_acs_switch(pst_mac_device) == MAC_ACS_SW_INIT) ||
            ...
            if (!pst_hmac_device ||
                ...
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                                 "{hmac_chan_start_bss::init acs enabled, but best channel of BAND%d is 0",
                                 pst_mac_vap->st_channel.en_band);
            } else {
