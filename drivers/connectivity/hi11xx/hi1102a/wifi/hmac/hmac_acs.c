...
OAL_STATIC oal_void hmac_acs_process_simple_channel_stats(mac_scan_chan_stats_simple_stru *pst_simple_channel_stats,
                                                          hmac_scan_record_stru *pst_scan_record, oal_uint32 ul_idx)
{
                                                           ...
                                                           hmac_scan_record_stru *pst_scan_record)
{
    ...
    if (!pst_mac_dev) {
        OAM_WARNING_LOG1(0, OAM_SF_ACS, "{pst_mac_dev is null, device id=%d}", pst_scan_record->uc_device_id);
    }
#endif
    ...
    for (ul_idx = 0; ul_idx < ul_chan_numbers; ul_idx++) {
        ...
        if (pst_mac_dev && (pst_simple_channel_stats[ul_idx].uc_channel_number > MAX_CHANNEL_NUM_FREQ_2G)) {
            if (mac_get_channel_idx_from_num(WLAN_BAND_5G, pst_simple_channel_stats[ul_idx].uc_channel_number,
                &uc_chan_idx) == OAL_SUCC) {
...
#endif
        hmac_acs_process_simple_channel_stats(pst_simple_channel_stats, pst_scan_record, ul_idx);
    }
                                               ...
                                               oal_bool_enum_uint8 en_need_rank)
{
    ...
    mac_device_stru *pst_mac_devce = mac_res_get_dev(uc_device_id);
    ...
    ul_size = OAL_SIZEOF(mac_acs_response_hdr_stru) + OAL_SIZEOF(mac_scan_event_stru) +
    ...
    FRW_EVENT_HDR_INIT(&(pst_event->st_event_hdr), FRW_EVENT_TYPE_HOST_CTX,
                       ...
                       (oal_uint16)ul_size, FRW_EVENT_PIPELINE_STAGE_0,
                       uc_chip_id, uc_device_id, 0);
                                  ...
                                  mac_acs_cmd_stru *pst_cmd)
{
    ...
    if (OAL_SUCC != mac_get_channel_idx_from_num(pst_ch->en_band, pst_ch->uc_chan_number, &pst_ch->uc_idx)) {
        OAM_WARNING_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_ACS,  "{got_init_rank::invalid 2G channel band=%d bw=%d ch=%d",
    ...
    OAL_IO_PRINT("got init rank result [%d %d %d] -  [%d %d %d] init=%d for %d:%d\n",
                 pst_ch[-1].en_band, pst_ch[-1].en_bandwidth, pst_ch[-1].uc_chan_number, pst_ch[0].en_band,
                 pst_ch[0].en_bandwidth, pst_ch[0].uc_chan_number, pst_hmac_device->en_init_scan,
...
oal_uint32 hmac_acs_process_rescan_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    ul_ret = hmac_acs_rescan_handler ((oal_void *)(oal_uint32) pst_req_param->uc_device_id);
    if (ul_ret != OAL_SUCC) {
    }
    return OAL_SUCC;
}
