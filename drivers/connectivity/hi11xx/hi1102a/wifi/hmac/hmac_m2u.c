...
OAL_STATIC hmac_m2u_grp_list_entry_stru *hmac_m2u_find_group_list(hmac_vap_stru *pst_hmac_vap,
                                                                  oal_uint8 *puc_grp_addr)
{
...
OAL_STATIC hmac_m2u_grp_list_entry_stru *hmac_m2u_create_grp_list(hmac_vap_stru *pst_hmac_vap,
                                                                  oal_uint8 *puc_grp_addr)
{
    hmac_m2u_snoop_list_stru *pst_snp_list = OAL_PTR_NULL;
    hmac_m2u_grp_list_entry_stru *pst_grp_list_member = OAL_PTR_NULL;
    ...
    if (pst_list_entry->ul_src_ip_addr) {
        ...
        if (pst_grp_member_list != OAL_PTR_NULL) {
            ...
            if (pst_list_entry->en_cmd != HMAC_M2U_IGMP_CMD_ADD_EXC_LIST) {
                OAM_INFO_LOG1(pst_list_entry->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                              "{hmac_m2u_update_snoop_list::grp_member_list == NULL && pst_list_entry->en_cmd is[%d].}",
                              pst_list_entry->en_cmd);
                ul_ret = hmac_m2u_add_member_list(pst_grp_list, pst_list_entry);
            }
        }
                                     ...
                                     mac_igmp_header_stru *pst_igmp)
{
    ...
    oal_void hmac_m2u_snoop_inspecting(hmac_vap_stru *pst_hmac_vap, hmac_user_stru *pst_hmac_user,
                                       oal_netbuf_stru *pst_buf)
    {
        mac_ether_header_stru *pst_ether_header = OAL_PTR_NULL;
        mac_ip_header_stru *pst_ip_hdr = OAL_PTR_NULL;
        mac_igmp_header_stru *pst_igmp = OAL_PTR_NULL;
        mac_igmp_v3_report_stru *pst_igmpr3 = OAL_PTR_NULL;
        ...
        if ((pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
            ...
            /* v1 & v2 igmp */
            pst_igmp =
            (mac_igmp_header_stru *)(oal_netbuf_data(pst_buf) + OAL_SIZEOF(mac_ether_header_stru) + 4 * uc_ip_hdr_len);
    ...
    OAL_STATIC oal_uint8 hmac_m2u_count_member_anysrclist(hmac_m2u_grp_list_entry_stru *pst_grp_list,
                                                          oal_uint8 *puc_table,
    ...
    OAL_STATIC oal_void hmac_m2u_remove_node_grp(hmac_m2u_grp_list_entry_stru *pst_grp_list,
    ...
    oal_uint32 hmac_m2u_tx_event(hmac_vap_stru *pst_vap, hmac_user_stru *pst_user, oal_netbuf_stru *pst_buf)
    {
        ...
        if (OAL_UNLIKELY(pst_event_mem == OAL_PTR_NULL)) {
            OAM_ERROR_LOG0(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_TX, "{hmac_tx_lan_to_wlan::event_mem null.}");
                                           ...
                                           oal_netbuf_stru *pst_buf)
    {
        ...
        } else { /* en_mcast_mode = 2 */ /* 不加自定义tunnel协议 */
    ...
    oal_uint32 hmac_m2u_snoop_convert(hmac_vap_stru *pst_vap, oal_netbuf_stru *pst_buf)
    {
        ...
        if ((uc_newmaccnt > 0) && (pst_m2u->en_mcast_mode == 0)) {
            OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_mcast_cnt, 1);
            return HMAC_TX_PASS;
            ...
            if (uc_newmaccnt > 0) {
                ...
                if (OAL_UNLIKELY(ul_ret != HMAC_TX_PASS)) {
                    if ((ul_ret != HMAC_TX_BUFF)) {
                        ...
                        OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_M2U,
                                         "{hmac_m2u_snoop_convert::tx_ucast_process not pass,ret = [%d]}", ul_ret);
                        OAM_STAT_VAP_INCR(pst_vap->st_vap_base_info.uc_vap_id, tx_m2u_ucast_droped, 1);
                        hmac_free_netbuf_list(pst_buf);
                    }
