...
/*****************************************************************************
  5 内部静态函数声明
*****************************************************************************/
OAL_STATIC oal_void hmac_dfs_nol_addchan(mac_device_stru *pst_mac_device, oal_uint8 uc_chan_idx);
...
#endif
oal_uint32 hmac_dfs_off_chan_cac_off_ch_dwell_timeout(oal_void *p_arg);
oal_uint32 hmac_dfs_off_chan_cac_opern_ch_dwell_timeout(oal_void *p_arg);
...
oal_uint32 hmac_dfs_recalculate_channel(mac_device_stru *pst_mac_device,
                                        oal_uint8 *puc_freq,
...
OAL_STATIC oal_void hmac_dfs_update_available_channel_list(
    mac_device_stru *pst_mac_device, oal_uint8 uc_chan_idx, oal_bool_enum_uint8 en_radar_detected)
{
    oal_uint32 ul_ret;
    ...
    if (pst_mac_device->st_ap_channel_list[uc_chan_idx].en_ch_status == MAC_CHAN_AVAILABLE_ALWAYS) {
        OAM_ERROR_LOG1(0, OAM_SF_DFS, "{dfs_update_available_channel_list::Radar detected in Non-Radar Channel(%d)!}",
                       uc_chan_idx);
                                                      ...
                                                      oal_uint32 *ul_scan_time)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG2(0, OAM_SF_DFS,
                       "{hmac_chan_get_off_chan_cac_time::get_channel_idx_from_num fail. en_band:%u, offchan_num:%u}",
                       pst_mac_vap->st_channel.en_band, pst_mac_device->st_dfs.st_dfs_info.uc_offchan_num);
        return ul_ret;
    }
...
OAL_STATIC oal_uint32 hmac_dfs_nol_timeout_fn(oal_void *p_arg)
{
    ...
    OAM_INFO_LOG1(0, OAM_SF_DFS,
                  "{[DFS]dfs_nol_timeout_fn, Non-Occupancy Period expired, remove channel %d from NOL.}", uc_chan_num);
...
OAL_STATIC oal_void hmac_dfs_nol_addchan(mac_device_stru *pst_mac_device, oal_uint8 uc_chan_idx)
{
    ...
    /* 更新可用信道列列表 */
    hmac_dfs_update_available_channel_list(pst_mac_device, uc_chan_idx, OAL_TRUE);
}
    ...
    if (pst_mac_device->st_dfs.st_dfs_info.uc_offchan_flag == (BIT0 | BIT1)) {
        ...
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG2(0, OAM_SF_DFS,
                             "{hmac_dfs_channel_mark_radar::get_channel_idx_from_num fail. en_band:%u, offchan_num:%u}",
                             pst_mac_vap->st_channel.en_band, pst_mac_device->st_dfs.st_dfs_info.uc_offchan_num);
            return;
        }
        hmac_chan_get_ext_chan_info(uc_idx,
...
#define HMAC_80MHZ_BITMAP_MASK 0xFFFFFF
    ...
    if (OAL_UNLIKELY(pst_mac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                       "{hmac_dfs_select_random_channel::pst_device null, device_id = %d.}", pst_mac_vap->uc_device_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
oal_uint32 hmac_dfs_ap_wait_start_radar_handler(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if ((ul_ret != OAL_SUCC) || (en_new_bandwidth >= WLAN_BAND_WIDTH_BUTT)) {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_SCAN,
                         "{dfs_start_radar::Could not start network using the selected channel[%d] or bandwidth[%d].}",
...
oal_bool_enum_uint8 hmac_dfs_try_cac(hmac_device_stru *pst_hmac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY, "{hmac_dfs_try_cac::res_get_hmac_vap fail.vap_id[%u]}", pst_mac_vap->uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
#ifdef _PRE_SUPPORT_ACS
