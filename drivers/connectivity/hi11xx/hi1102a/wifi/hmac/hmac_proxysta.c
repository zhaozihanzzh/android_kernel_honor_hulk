...
OAL_STATIC oal_void hmac_psta_fdb_change_handler(struct br_fdb_change_info *info)
{
    ...
    if (memcpy_s(st_rsp.auc_payload, OAL_SIZEOF (st_rsp.auc_payload), info,
        OAL_SIZEOF(struct br_fdb_change_info)) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "hmac_psta_fdb_change_handler::memcpy fail.");
    }
    st_rsp.en_type = VAMP_RESP_TYPE_FDB_CHANGE;
    st_rsp.uc_size = OAL_SIZEOF(struct br_fdb_change_info) + 2;  // header大小是2
                                             ...
                                             oal_net_device_stru **ppst_net_device)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &pst_dev_psta->ast_hash[uc_hash_value])
    {
        ...
        if (oal_compare_mac_addr(hmac_vap_psta_oma(pst_hmac_vap), puc_mac_addr) == 0) {
...
oal_uint32 hmac_psta_tx_process(oal_netbuf_stru *pst_buf, mac_vap_stru **ppst_vap)
{
    ...
    if (OAL_UNLIKELY(ppst_vap == OAL_PTR_NULL || *ppst_vap == OAL_PTR_NULL)) {
    ...
    if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_PROXYSTA, "{hmac_psta_tx_process::pst_hmac_vap[%d] = OAL_PTR_NULL!}",
                       (*ppst_vap)->uc_vap_id);
        return OAL_FAIL;
    }
    ...
    if (mac_vap_is_vsta(*ppst_vap)) {
        ...
        if (!OAL_MEMCMP(hmac_vap_psta_oma(pst_hmac_vap), pst_ether_header->auc_ether_dhost, WLAN_MAC_ADDR_LEN)) {
            OAM_WARNING_LOG0((*ppst_vap)->uc_vap_id, OAM_SF_PROXYSTA, "{hmac_psta_tx_process::drop pkt to oma vsta}");
            return OAL_FAIL;
        }
        ...
        if (OAL_SUCC != hmac_psta_find_net_dev(hmac_psta_get_rep(pst_hmac_vap),
            ...
            OAM_WARNING_LOG0((*ppst_vap)->uc_vap_id, OAM_SF_PROXYSTA, "{hmac_psta_tx_process::drop pkt unknown oma}");
            return OAL_FAIL;
        }
...
oal_uint32 hmac_psta_rx_process(oal_netbuf_stru *pst_buf, hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_buf == OAL_PTR_NULL || pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG2(0, OAM_SF_PROXYSTA, "{hmac_psta_rx_process::null ptr pst_buf=%x pst_hmac_vap=%x}", pst_buf,
                       pst_hmac_vap);
        return OAL_FAIL;
    }
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id,
                       OAM_SF_PROXYSTA, "{null device id=%d}", pst_hmac_vap->st_vap_base_info.uc_vap_id);
        return OAL_FAIL;
    }
    ...
    if (en_is_mcast == OAL_TRUE) {
        ...
        OAL_DLIST_SEARCH_FOR_EACH(pst_list, &pst_rep->st_msta_list)
        {
            ...
            if ((pst_tmp_hvap != pst_hmac_vap) &&
                !oal_compare_mac_addr(pst_ether_header->auc_ether_shost,
                                      mac_mib_get_StationID(&pst_tmp_hvap->st_vap_base_info))) {
                OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PROXYSTA,
                                 "{hmac_psta_rx_mat::drop pkt to vsta}");
                return OAL_FAIL;
            }
        }
...
oal_uint32 hmac_psta_add_vap(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (mac_dev_xsta_num(pst_mac_device) == 0) {
...
oal_uint32 hmac_psta_del_vap(hmac_vap_stru *pst_hmac_vap)
{
    ...
    memcpy_s(auc_proxysta_oma, WLAN_MAC_ADDR_LEN, hmac_vap_psta_oma(pst_hmac_vap), WLAN_MAC_ADDR_LEN);
    ...
    OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_entry_temp, pst_hash_head)
    {
        ...
        if (oal_compare_mac_addr(hmac_vap_psta_oma(pst_vap), auc_proxysta_oma) == 0) {
...
oal_int32 hmac_psta_write_proc(oal_file_stru *pst_file, const oal_int8 *pc_buffer, oal_uint32 ul_len,
                               oal_void *p_data)
{
    return -1;
}
    ...
    for (ul_rep_id = 0; ul_rep_id < WLAN_PROXY_STA_MAX_REP; ul_rep_id++) {
        ...
        OAL_DLIST_SEARCH_FOR_EACH(pst_list, &pst_rep->st_pbss_list)
        {
            ...
            OAL_IO_PRINT("   pbss:oma=%02x:%02x:%02x:%02x:%02x:%02x vma=%02x:%02x:%02x:%02x:%02x:%02x %s\n",
                         ...
                         puc_vma[0], puc_vma[1], puc_vma[2], puc_vma[3], puc_vma[4], puc_vma[5],
                         pst_hmac_vap->pst_net_device->name);
        }
                                           ...
                                           oal_uint32 ul_pkt_len)
{
    ...
    if (ul_pkt_len < ul_contig_len) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                       "{buf len is less than the sum of mac_ether_header_stru and oal_eth_arphdr_stru.}");
        return OAL_FAIL;
    }
    ...
    if (pst_arp != OAL_PTR_NULL) {
        if (en_is_mcast != OAL_TRUE) {
            switch (OAL_NET2HOST_SHORT(pst_arp->us_ar_op)) {
                ...
                default:
                    OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                                  "{hmac_psta_rx_arp_mat::do not replace arp addr.}");
            }
        } else {
            OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
            ...
            if (oal_compare_mac_addr(puc_arp_dmac, hmac_vap_psta_oma(pst_hmac_vap)) != OAL_FALSE) {
                OAM_INFO_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                ...
                for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
                    ...
                    if (OAL_UNLIKELY(pst_hmac_temp_vap == OAL_PTR_NULL)) {
                        OAM_WARNING_LOG0(uc_vap_idx, OAM_SF_PROXYSTA, "{hmac_psta_rx_arp_mat::mac_vap is null ptr!}");
                        return OAL_SUCC;
                    }

                    if (oal_compare_mac_addr(puc_arp_dmac, mac_mib_get_StationID(&pst_hmac_temp_vap->st_vap_base_info))
                        == OAL_FALSE) {
                        /* 替换mac地址为out vap的mac地址 */
                                          ...
                                          oal_uint32 ul_pkt_len)
{
    ...
    if ((pst_mac_vap == OAL_PTR_NULL) || (pst_ether_header == OAL_PTR_NULL) || (pst_mac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_PROXYSTA,
                       "{hmac_psta_rx_ip_mat::The input parameter of hmac_psta_rx_ip_mat is OAL_PTR_NULL.}");
        return OAL_FAIL;
    }
    ...
    if (ul_pkt_len < ul_contig_len) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                       "{buf len is less than the sum of mac_ether_header_stru and oal_ip_header_stru.}");
        return OAL_FAIL;
    }
                ...
                if (oal_compare_mac_addr(pst_dhcp_packet->chaddr, mac_mib_get_StationID(pst_mac_vap)) != OAL_FALSE) {
                    ...
                    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
                        ...
                        if (oal_compare_mac_addr(pst_dhcp_packet->chaddr,
                            ...
                            /* 重新计算udp checksum */
                            pst_udp_header->check =
                            hmac_psta_rx_checksum(pst_udp_header->check,
                                                  mac_mib_get_StationID(&pst_hmac_temp_vap->st_vap_base_info),
...
oal_uint32 hmac_psta_rx_mat(oal_netbuf_stru *pst_buf, hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_PROXYSTA,
                       "{hmac_psta_rx_mat::null device id=%d}", pst_hmac_vap->st_vap_base_info.uc_vap_id);
        return OAL_FAIL;
    }
    ...
    if (ul_pkt_len < ul_contig_len) {
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                       "{drop for invalid len, ul_pkt_len=%d ul_contig_len=%d}", ul_pkt_len, ul_contig_len);
        return OAL_FAIL;
    }
                                              ...
                                              oal_uint32 ul_contig_len)
{
    ...
    if (ul_pkt_len < ul_contig_len) {
        OAM_ERROR_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                       "{hmac_psta_tx_icmpv6_mat::ul_pkt_len[%d], ul_contig_len[%d]}",
                       ul_pkt_len, ul_contig_len);
        return OAL_FAIL;
    }
 ...
 */
            ...
            case OAL_NDISC_NEIGHBOUR_ADVERTISEMENT: {
                ...
                if (ul_pkt_len < ul_contig_len) {
                    OAM_ERROR_LOG3(pst_mac_vap->uc_vap_id, OAM_SF_PROXYSTA,
                                   "{The length of buf is invalid ul_pkt_len[%d], ul_contig_len[%d] icmp6_type[%d].}",
                ...
                us_check_sum = hmac_psta_tx_checksum((oal_uint16)OAL_IPPROTO_ICMPV6, us_icmp6len,
                                                     pst_ipv6hdr->saddr.s6_addr, pst_ipv6hdr->daddr.s6_addr, 16,
        ...
        if ((en_packet_changed == OAL_TRUE) && (pst_eth_icmp6_lladdr != OAL_PTR_NULL)) {
            ...
            us_check_sum = hmac_psta_tx_checksum((oal_uint16)OAL_IPPROTO_ICMPV6, us_icmp6len,
                                                 pst_ipv6hdr->saddr.s6_addr, pst_ipv6hdr->daddr.s6_addr,
                                                 16, (oal_uint8 *)pst_icmp6hdr);
...
oal_uint32 hmac_psta_tx_mat(oal_netbuf_stru *pst_buf, hmac_vap_stru *pst_hmac_vap)
{
    ...
    /* 参数合法性检查 */
    if (OAL_UNLIKELY((pst_buf == OAL_PTR_NULL) || (pst_hmac_vap == OAL_PTR_NULL))) {
        OAM_ERROR_LOG0(0, OAM_SF_PROXYSTA,
...
oal_uint32 hmac_psta_resume_bss(hmac_vap_stru *pst_hmac_vap, mac_channel_stru *pst_channel,
                                wlan_protocol_enum_uint8 en_proto)
{
    if (hmac_chan_start_bss(pst_hmac_vap, pst_channel, en_proto) == OAL_SUCC) {
...
oal_uint32 hmac_psta_proc_wait_join(hmac_vap_stru *pst_hmac_sta, hmac_join_req_stru *pst_join_req)
{
    ...
    if (pst_hmac_sta == OAL_PTR_NULL || pst_join_req == OAL_PTR_NULL) {
        OAM_WARNING_LOG2(0, OAM_SF_ANY, "{hmac_psta_proc_wait_join::null ptr, sta=%p join_req=%p}", pst_hmac_sta,
                         pst_join_req);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (!mac_is_proxysta_enabled(pst_mac_device) || (!mac_vap_is_msta(&pst_hmac_sta->st_vap_base_info))) {
        return OAL_SUCC;
    }
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if ((pst_hmac_vap_temp != OAL_PTR_NULL) &&
            (pst_hmac_vap_temp->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP)) {
            if (hmac_psta_pause_bss(pst_hmac_vap_temp) != OAL_SUCC) {
                ul_ret = OAL_FAIL;
            }
        }
    }
...
oal_uint32 hmac_psta_proc_join_result(hmac_vap_stru *pst_hmac_sta, oal_bool_enum_uint8 en_succ)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (pst_hmac_vap_temp && (pst_hmac_vap_temp->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) &&
            ...
            if (hmac_psta_resume_bss(pst_hmac_vap_temp, &pst_hmac_sta->st_vap_base_info.st_channel,
                                     pst_hmac_sta->st_vap_base_info.en_protocol) != OAL_SUCC) {
                ul_ret = OAL_FAIL;
            }
        }
    }
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_list, &pst_rep->st_pbss_list)
    {
        pst_hmac_vap_temp = OAL_DLIST_GET_ENTRY(pst_list, hmac_vap_stru, st_psta.st_hash_entry);
        if (pst_hmac_vap_temp->st_vap_base_info.uc_device_id != pst_hmac_sta->st_vap_base_info.uc_device_id) {
            if (hmac_psta_resume_bss(pst_hmac_vap_temp, OAL_PTR_NULL, WLAN_PROTOCOL_BUTT) != OAL_SUCC) {
                ul_ret = OAL_FAIL;
            }
        }
    }
