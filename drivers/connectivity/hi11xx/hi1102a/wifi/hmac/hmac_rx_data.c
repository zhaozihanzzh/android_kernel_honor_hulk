                                                 ...
                                                 dmac_msdu_stru *pst_msdu)
{
    ...
    if (mac_vap_is_msta(pst_mac_vap) || mac_vap_is_vsta(pst_mac_vap)) {
        if ((hmac_psta_rx_process(pst_netbuf, pst_vap) != OAL_SUCC) ||
            (hmac_psta_rx_mat(pst_netbuf, pst_vap) != OAL_SUCC)) {
            oal_netbuf_free(pst_netbuf);
            return;
        }
    }
                                                        ...
                                                        oal_netbuf_stru *netbuf)
{
    ...
    if (pst_hmac_device->sys_tcp_rx_ack_opt_enable == OAL_TRUE) {
        ...
        OAM_WARNING_LOG1(0, OAM_SF_TX,
                         "{hmac_transfer_rx_handler::mac_header_len = %d}", pst_rx_ctrl->st_rx_info.uc_mac_header_len);
#endif
                                                ...
                                                mac_ieee80211_frame_stru *pst_frame_hdr)
{
    ...
    if ((g_st_pkt_check_rx_info.uc_pkt_check_num0 + g_st_pkt_check_rx_info.uc_pkt_check_num1 +
        ...
        (g_st_pkt_check_rx_info.us_pkt_check_send_num * WLAN_PACKET_CHECK_TYPE_NUM)) {
