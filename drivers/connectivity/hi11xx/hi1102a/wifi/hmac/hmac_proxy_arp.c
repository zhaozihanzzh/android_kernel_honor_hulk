...
oal_err_code_enum_uint32 hmac_proxy_get_mac_by_ipv4(mac_vap_stru *pst_mac_vap, oal_uint32 ul_ipv4,
                                                    oal_uint8 *puc_mac)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_mac_vap->pst_vap_proxyarp->ast_ipv4_head[ul_user_hash_value]))
    {
        ...
        memcpy_s(puc_mac, WLAN_MAC_ADDR_LEN, pst_hash->auc_mac, WLAN_MAC_ADDR_LEN);
...
oal_err_code_enum_uint32 hmac_proxy_add_ipv4(mac_vap_stru *pst_mac_vap, oal_uint32 ul_ipv4, oal_uint8 *puc_mac)
{
    ...
    memcpy_s(pst_hash->auc_mac, OAL_MAC_ADDR_LEN, puc_mac, OAL_MAC_ADDR_LEN);
...
oal_err_code_enum_uint32 hmac_proxy_remove_mac(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_mac)
{
    ...
    oal_dlist_head_stru *pst_tmp = OAL_PTR_NULL;
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv6_num > 0) {
        for (ul_loop = 0; ul_loop < MAC_VAP_USER_HASH_MAX_VALUE; ul_loop++) {
            OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_tmp, &(pst_mac_vap->pst_vap_proxyarp->ast_ipv6_head[ul_loop]))
            {
    ...
    if (pst_mac_vap->pst_vap_proxyarp->uc_ipv4_num > 0) {
        for (ul_loop = 0; ul_loop < MAC_VAP_USER_HASH_MAX_VALUE; ul_loop++) {
            OAL_DLIST_SEARCH_FOR_EACH_SAFE(pst_entry, pst_tmp, &(pst_mac_vap->pst_vap_proxyarp->ast_ipv4_head[ul_loop]))
            {
                                                ...
                                                oal_eth_arphdr_stru *pst_arp_hdr)
{
    ...
    memcpy_s(auc_src_mac, ETHER_ADDR_LEN, pst_arp_hdr->auc_ar_sha, ETHER_ADDR_LEN);
    memcpy_s(&ul_src_ipv4, ETH_SENDER_IP_ADDR_LEN, pst_arp_hdr->auc_ar_sip, ETH_SENDER_IP_ADDR_LEN);
    memcpy_s(&ul_tgt_ipv4, ETH_TARGET_IP_ADDR_LEN, pst_arp_hdr->auc_ar_tip, ETH_TARGET_IP_ADDR_LEN);
                                                     ...
                                                     oal_uint8 *puc_mac_src)
{
    ...
    if ((en_err == OAL_SUCC) && (memcmp(puc_mac_src, auc_mac, OAL_MAC_ADDR_LEN))) {
...
oal_netbuf_stru *hmac_proxy_create_na(oal_net_device_stru *pst_net_dev, oal_uint8 *puc_src_mac,
                                      oal_uint8 *puc_dst_mac, oal_uint8 *puc_src_ipv6, oal_uint8 *puc_dst_ipv6)
{
    oal_netbuf_stru *pst_netbuff = OAL_PTR_NULL;
    oal_uint32 ul_size;
    ...
    *(oal_uint32 *)pst_ipv6 = OAL_HOST2NET_LONG(0x60000000);
    ...
    /*lint -e415*//*lint -e416*/
    pst_nd->opt[1] = 1;
    memcpy_s(&pst_nd->opt[2], OAL_MAC_ADDR_LEN, puc_src_mac, OAL_MAC_ADDR_LEN);
    /*lint +e415*//*lint +e416*/
                                              ...
                                              mac_ether_header_stru *pst_ether_hdr, oal_int32 l_opt_len)
{
    ...
    if (hmac_proxy_get_mac_by_ipv6(pst_mac_vap, &pst_msg->target, ast_dst_mac) != OAL_SUCC) {
    ...
    if (mac_vap_find_user_by_macaddr(pst_mac_vap, pst_src_mac, &us_idx) == OAL_SUCC) {
        ...
        if (l_netifrx_rst != OAL_SUCC) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_PROXYARP,
                             "{hmac_proxy_nd_rcv_ns 2br Err[%d]!!}", l_netifrx_rst);
        }
                                              ...
                                              oal_int32 l_opt_len)
{
    ...
    en_solicited = (pst_nd->icmph.icmp6_dataun.u_nd_advt.solicited) &&
                   !(hmac_proxy_ipv6_addr_is_mcast(&pst_nd->target));
