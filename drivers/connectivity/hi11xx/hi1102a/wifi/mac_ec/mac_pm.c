...
oal_uint32 mac_pm_arbiter_alloc_id(
    mac_device_stru *pst_device, oal_uint8 *pst_name, mac_pm_arbiter_type_enum en_arbiter_type)
{
    ...
    if (pst_pm_arbiter == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_device->uc_cfg_vap_id, OAM_SF_PWR, "hmac_pm_arbiter_alloc_id FAIL:pst_pm_arbiter is null");
    ...
    for (i = 0; i < MAC_PM_ARBITER_MAX_REQUESTORS; i++) {
        if (((1 << i) & pst_pm_arbiter->ul_id_bitmap) == 0) {
            ...
            if (snprintf_s((char *)&pst_pm_arbiter->requestor[i].auc_id_name[0],
                MAC_PM_ARBITER_MAX_REQ_NAME, MAC_PM_ARBITER_MAX_REQ_NAME - 1, "%s", pst_name) < 0) {
                OAM_ERROR_LOG0(pst_device->uc_cfg_vap_id, OAM_SF_PWR, "mac_pm_arbiter_alloc_id::snprintf_s error.");
            }
                                 ...
                                 oal_uint8 uc_state_from, oal_uint8 uc_state_to)
{
    ...
    if (en_can_trans == OAL_TRUE) {
        ...
        /* 操作hal层接口 */
        if (mac_pm_set_hal_state(pst_device, pst_mac_vap, uc_state_to) == OAL_SUCC) {
...
oal_uint32 mac_pm_wow_prepare_probe_resp(dmac_vap_stru *pst_dmac_vap)
{
    ...
    if (pst_dmac_vap->pst_wow_probe_resp == OAL_PTR_NULL) {
        pst_dmac_vap->pst_wow_probe_resp =
...
oal_uint32 mac_pm_wow_prepare_null_data(dmac_vap_stru *pst_dmac_vap)
{
    ...
    if (pst_dmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        if (pst_dmac_vap->pst_wow_null_data == OAL_PTR_NULL) {
            /* 申请net_buff */
            pst_dmac_vap->pst_wow_null_data =
                OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_SHORT_NETBUF_SIZE, OAL_NETBUF_PRIORITY_MID);
            if (pst_dmac_vap->pst_wow_null_data == OAL_PTR_NULL) {
        ...
        mac_hdr_set_frame_control(oal_netbuf_header(pst_dmac_vap->pst_wow_null_data),
                                  (oal_uint16)(WLAN_PROTOCOL_VERSION | WLAN_FC0_TYPE_DATA |
                                               WLAN_FC0_SUBTYPE_NODATA) |
                                  0x0200);
...
oal_uint32 mac_pm_set_wow_para(
    dmac_vap_stru *pst_dmac_vap, hal_wow_param_stru *pst_wow_para, mac_cfg_lpm_wow_en_stru st_wow_en)
{
    ...
    if (st_wow_en.uc_en) {
        ...
        for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
            ...
            if (pst_dmac_vap->pst_hal_vap->uc_vap_id == 0) {
                ...
                pst_wow_para->uc_ap0_probe_resp_phy = 0;
                pst_wow_para->uc_ap0_probe_resp_rate = 0;
                ...
                pst_wow_para->uc_ap1_probe_resp_phy = 0;
                pst_wow_para->uc_ap1_probe_resp_rate = 0;
