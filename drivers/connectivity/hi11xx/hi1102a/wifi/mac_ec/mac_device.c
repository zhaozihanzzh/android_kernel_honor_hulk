    ...
    mac_device_stru *pst_mac_device, oal_uint32 ul_chip_ver, oal_uint8 uc_chip_id, oal_uint8 uc_device_id)
{
    ...
    memset_s(&pst_mac_device->st_psta, OAL_SIZEOF(pst_mac_device->st_psta), 0, OAL_SIZEOF(pst_mac_device->st_psta));
#endif
...
mac_vap_stru *mac_find_main_proxysta(mac_device_stru *pst_mac_device)
{
    ...
    if (pst_mac_device == OAL_PTR_NULL || !mac_is_proxysta_enabled(pst_mac_device)) {
        return OAL_PTR_NULL;
    }
