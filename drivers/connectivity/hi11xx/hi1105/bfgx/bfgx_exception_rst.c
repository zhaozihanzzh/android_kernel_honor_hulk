...
#else
void bfgx_beat_timer_expire_etc(struct timer_list *t);
#endif
...
void plat_exception_reset_work_etc(struct work_struct *work)
{
    ...
    if (get_exception_info_reference_etc(&pst_exception_data) != EXCEPTION_SUCCESS) {
...
int32 bfgx_store_stack_mem_to_file_etc(void)
{
    ...
    for (i = 0; i < BFGX_MEM_DUMP_BLOCK_COUNT; i++) {
        ...
        oal_kernel_file_write_etc(fp, pst_mem_info->exception_mem_addr, pst_mem_info->recved_size);
...
int32 __store_wifi_mem_to_file_etc(void)
{
    ...
    for (i = 0; i < block_count; i++) {
        ...
        /* 将接收到的内存写入到文件中 */
        PS_PRINT_INFO("pos = %d\n", (int)fp->f_pos);
        oal_kernel_file_write_etc(fp, pst_mem_info->exception_mem_addr + offset, block_size);
        filp_close(fp, NULL);
        offset += block_size;
    }
    set_fs(fs);
...
int32 sdio_get_and_save_data_etc(exception_bcpu_dump_msg *sdio_read_info, uint32 count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        while (index < send_total_len) {
            ...
            oal_kernel_file_write_etc(fp, st_bcpu_dump_buff_etc.mem_addr, st_bcpu_dump_buff_etc.data_len);
...
#else
    timer_setup(&p_exception_data->bfgx_beat_timer, bfgx_beat_timer_expire_etc, 0);
#endif
