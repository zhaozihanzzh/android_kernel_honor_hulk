...
static int32 core_recv_prepare(void *disc_data, const uint8 *data, int32 count)
{
    ...
    if (uart_rx_dump_etc) {
        if (!OAL_IS_ERR_OR_NULL(ps_core_d->rx_data_fp)) {
            ...
            kernel_write(ps_core_d->rx_data_fp, data, count, &ps_core_d->rx_data_fp->f_pos);
#else
...
/*
 * Prototype    : ps_skb_queue_len_etc
 * Description  : internal queue skb nums.
 */
int32 ps_skb_queue_len_etc(struct ps_core_s *ps_core_d, uint8 type)
{
