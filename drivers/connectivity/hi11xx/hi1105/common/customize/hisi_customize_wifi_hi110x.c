...
OAL_STATIC wlan_cfg_nv_map_handler wifi_nvram_cfg_handler[WLAN_CFG_DTS_NVRAM_END] = {
    ...
    {"WITXBW0",  "dp2ginit0",   NULL,      WLAN_CFG_NVRAM_DP2G_INIT0,          {0},  0x0},
    {"WITXBW1",  "dp2ginit1",   NULL,      WLAN_CFG_NVRAM_DP2G_INIT1,          {0},  0x0},
#endif
...
OAL_STATIC oal_void original_value_for_dts_params(oal_void)
{
    ...
    dts_params_etc[WLAN_CFG_DTS_DPD_CALI_20M_DEL_POW_CORE0] = 0x00000000,
    dts_params_etc[WLAN_CFG_DTS_DPD_CALI_40M_DEL_POW_CORE0] = 0x00000000,
    ...
    dts_params_etc[WLAN_CFG_DTS_DPD_CALI_20M_DEL_POW_CORE1] = 0x00000000,
    dts_params_etc[WLAN_CFG_DTS_DPD_CALI_40M_DEL_POW_CORE1] = 0x00000000,
    dts_params_etc[WLAN_CFG_DTS_DYN_CALI_DSCR_ITERVL] = 0x0;
}

/*
 * 函 数 名  : host_auto_freq_params_init
 * 功能描述  : 给定制化参数全局数组 host_init_params_etc中自动调频相关参数赋初值
 *             ini文件读取失败时用初值
 */
 ...
 * 功能描述  : 给定制化参数全局数组 host_init_params_etc中amsdu聚合门限相关参数赋初值
 *             ini文件读取失败时用初值
 */
OAL_STATIC oal_void host_amsdu_th_params_init(oal_void)
{
    ...
    host_init_params_etc[WLAN_CFG_INIT_AMSDU_AMPDU_TH_MIDDLE] = WLAN_AMSDU_AMPDU_TH_MIDDLE;
...
{
    /* 动态绑PCIE中断 */
    host_init_params_etc[WLAN_CFG_PRIV_DMA_LATENCY] = OAL_FALSE;
    host_init_params_etc[WLAN_CFG_PRIV_LOCK_CPU_TH_HIGH] = 0;
    host_init_params_etc[WLAN_CFG_PRIV_LOCK_CPU_TH_LOW] = 0;
    ...
    host_init_params_etc[WLAN_CFG_INIT_RX_AMPDU_BITMAP] = OAL_FALSE;
    host_init_params_etc[WLAN_CFG_INIT_HIEX_CAP] = WLAN_HIEX_DEV_CAP;
}
...
OAL_STATIC oal_void host_params_init_first(oal_void)
{
    ...
    /* miracast */
    host_init_params_etc[WLAN_CFG_INIT_PROBE_RESP_MODE] = 0x10;
    host_init_params_etc[WLAN_CFG_INIT_MIRACAST_SINK_ENABLE] = 0;
    host_init_params_etc[WLAN_CFG_INIT_REDUCE_MIRACAST_LOG] = 0;
    host_init_params_etc[WLAN_CFG_INIT_CORE_BIND_CAP] = 1;
    ...
    /* connect */
    host_init_params_etc[WLAN_CFG_INIT_DDR_FREQ] = WLAN_DDR_CAHNL_FREQ;
...
OAL_STATIC oal_void hwifi_custom_adapt_priv_ini_param_extend_etc
                                (hmac_to_dmac_cfg_custom_data_stru *pst_syn_msg,
 ...
 * 功能描述  : 解圈复杂度拆分函数
 */
OAL_STATIC oal_void hwifi_custom_adapt_priv_ini_extend
                                               (hmac_to_dmac_cfg_custom_data_stru *pst_syn_msg,
                                                wlan_cfg_priv_id_uint8 uc_cfg_id,
                                                oal_uint8 *puc_priv_cfg_value)
{
                                                                       ...
                                                                       oal_uint32 *pul_len)
{
    ...
    hwifi_custom_adapt_device_priv_ini_hiex_cap(puc_data + ul_data_len, &ul_data_len);
    ...
    oal_uint32 *pul_data_len)
{
    ...
    *pul_data_len += OAL_SIZEOF(st_fem_pow_ctl) + CUSTOM_MSG_DATA_HDR_LEN;
    OAM_WARNING_LOG3(0, OAM_SF_CFG, "{hwifi_custom_adapt_device_priv_ini_fem_pow_ctl_param::len[%d]delt_pow[%d] \
                     start_idx[%d].}", *pul_data_len, st_fem_pow_ctl.uc_fem_delt_pow, st_fem_pow_ctl.uc_tpc_adj_pow_start_idx);
...
int32 hwifi_custom_adapt_device_priv_ini_param(oal_uint8 *puc_data)
{
    ...
    l_ret = hwifi_custom_adapt_priv_ini_param(WLAN_CFG_PRIV_COUNRTYCODE_SELFSTUDY_CFG,
                                              puc_data + ul_data_len, &ul_data_len);
...
OAL_STATIC oal_int32 hwifi_read_conf_from_etc(oal_uint8 *puc_buffer_cust_nvram, oal_uint8 uc_idx)
{
    oal_int32 l_ret;
#ifdef _PRE_CONFIG_READ_DYNCALI_E2PROM
    l_ret = read_conf_from_eeprom_etc(puc_buffer_cust_nvram, CUS_PARAMS_LEN_MAX,
        wifi_nvram_cfg_handler[uc_idx].ul_eeprom_offset);
#else
    l_ret = read_conf_from_nvram_etc(puc_buffer_cust_nvram, CUS_PARAMS_LEN_MAX,
        wifi_nvram_cfg_handler[uc_idx].ul_nv_map_idx, wifi_nvram_cfg_handler[uc_idx].puc_nv_name);
#endif
    return l_ret;
}
...
oal_int32 hwifi_custom_host_read_dyn_cali_nvram(oal_void)
{
    ...
    for (uc_idx = WLAN_CFG_NVRAM_DP2G_INIT0; uc_idx <= WLAN_CFG_NVRAM_DP2G_INIT1; uc_idx++) {
        l_ret = hwifi_read_conf_from_etc(puc_buffer_cust_nvram_tmp, uc_idx);
        if (l_ret != INI_SUCC) {
...
static oal_int32 hwifi_get_5g_ru_max_ru_pow_val(wlan_cust_nvram_params *cust_nv_params)
{
    ...
    for (band_idx = 0; band_idx < MAC_NUM_5G_BAND; band_idx++) {
        for (idx = CUS_POW_TX_CHAIN_MIMO; idx < CUS_POW_TX_CHAIN_BUTT; idx++) {
            if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[nvram_idx].name, &nvram_params) != INI_SUCC) {
...
static oal_void hwifi_get_total_max_ru_pow_val(wlan_cust_nvram_params *pst_cust_nv_params)
{
    oal_uint32 ul_idx;
    ...
    for (ul_idx = WLAN_BAND_2G; ul_idx < WLAN_BAND_BUTT; ul_idx++) {
        if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[ul_nvram_idx].name, &l_nvram_params) != INI_SUCC) {
            l_nvram_params = 0xFF;
        }
        pst_cust_nv_params->auc_tpc_tb_ru_max_power[ul_idx] = CUS_GET_FIRST_BYTE(l_nvram_params);
        ul_nvram_idx++;
    }
}
...
static oal_void hwifi_get_max_ru_pow_val(void)
{
    ...
    for (ul_idx = CUS_POW_TX_CHAIN_MIMO; ul_idx < CUS_POW_TX_CHAIN_BUTT; ul_idx++) {
        ...
        l_ret += memcpy_s(pst_cust_nv_params->auc_tpc_tb_ru_2g_max_power[ul_idx], sizeof(l_nvram_params), \
                          &l_nvram_params, sizeof(l_nvram_params));
        ul_nvram_idx++;
        if (get_cust_conf_int32_etc(INI_MODU_WIFI, nvram_config_ini[ul_nvram_idx].name, &l_nvram_params) != INI_SUCC) {
            l_nvram_params = 0xFF;
        }
        pst_cust_nv_params->auc_tpc_tb_ru_2g_max_power[ul_idx][WLAN_HE_RU_SIZE_484] = CUS_GET_FIRST_BYTE(l_nvram_params);
        ul_nvram_idx++;
    }
    hwifi_get_5g_ru_max_ru_pow_val(pst_cust_nv_params);
    if (l_ret != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_CUSTOM, "hwifi_get_max_ru_pow_val::memcpy failed!");
        memset_s(pst_cust_nv_params->auc_tpc_tb_ru_2g_max_power, sizeof(pst_cust_nv_params->auc_tpc_tb_ru_2g_max_power),
                 0xFF, sizeof(pst_cust_nv_params->auc_tpc_tb_ru_2g_max_power));
        memset_s(pst_cust_nv_params->auc_tpc_tb_ru_5g_max_power, sizeof(pst_cust_nv_params->auc_tpc_tb_ru_5g_max_power),
                 0xFF, sizeof(pst_cust_nv_params->auc_tpc_tb_ru_5g_max_power));
    }

    hwifi_get_total_max_ru_pow_val(pst_cust_nv_params);
}
#endif
...
extern int32 get_board_mac_etc(uint8 *puc_buf, uint8 type);
#endif

#if defined(_PRE_CONFIG_READ_DYNCALI_E2PROM) && defined(_PRE_CONFIG_READ_E2PROM_MAC)
/*
 * 函 数 名  : hwifi_get_mac_addr_drveprom_etc
 * 功能描述  : 从e2prom中获取mac地址，用宏_PRE_CONFIG_READ_E2PROM_MACADDR区分PC和大屏
 */
int32 hwifi_get_mac_addr_drveprom_etc(uint8 *puc_buf)
{
...
#define READ_CHECK_WAIT_TIME  50
    ...
    if (wifimac_etc[0] != 0 || wifimac_etc[1]  != 0 || wifimac_etc[2]  != 0 || wifimac_etc[3] != 0
        ...
        INI_WARNING("hwifi_get_mac_addr_drveprom_etc get MAC from wifimac_etc SUCC\n");
        return INI_SUCC;
    }

    for (i = 0; i < READ_CHECK_MAX_CNT; i++) {
        if (DrvEepromRead("MACWLAN", offset, original_mac_addr, bit_Len) == INI_SUCC) {
            INI_WARNING("hwifi_get_mac_addr_drveprom_etc get MAC from EEPROM SUCC\n");
            ret = INI_SUCC;
            break;
        }
    ...
    if (ret == INI_SUCC) {
        OAL_IO_PRINT("hwifi_get_mac_addr_drveprom_etc DrvEepromRead return success\n");
        ul_sum = char2byte(original_mac_addr, (int8 *)puc_buf);
        if (ul_sum != 0) {
            INI_WARNING("hwifi_get_mac_addr_drveprom_etc get MAC from EEPROM: mac=" MACFMT "\n", MAC2STR(puc_buf));
            if (memcpy_s(wifimac_etc, MAC_LEN, puc_buf, MAC_LEN) != EOK) {
                INI_WARNING("hwifi_get_mac_addr_drveprom_etc memcpy_s wifimac_etc fail\n");
            }
        } else {
            INI_WARNING("hwifi_get_mac_addr_drveprom_etc get MAC from EEPROM is not char,use random mac\n");
            random_ether_addr(puc_buf);
            puc_buf[1] = 0x11;
            puc_buf[2] = 0x03;
        }
    } else {
        OAL_IO_PRINT("hwifi_get_mac_addr_etc DrvEepromRead return fail,use random mac\n");
...
int32 hwifi_get_mac_addr_etc(uint8 *puc_buf)
{
    ...
    if (get_board_mac_etc(puc_buf, WIFI_2G_MAC_TYPE) == 0) {
        INI_WARNING("hwifi_get_mac_addr_etc get MAC from NV: mac="MACFMT"\n", MAC2STR(puc_buf));
        if (memcpy_s(wifimac_etc, MAC_LEN, puc_buf, MAC_LEN) != EOK) {
            OAM_ERROR_LOG0(0, OAM_SF_ANY, "hwifi_get_mac_addr_etc::memcpy_s mac to wifimac_etc failed");
        }
    } else {
        random_ether_addr(puc_buf);
        puc_buf[1] = 0x11;
        puc_buf[2] = 0x02;
        INI_WARNING("hwifi_get_mac_addr_etc get random mac: mac="MACFMT"\n", MAC2STR(puc_buf));
    }
    return INI_SUCC;
}
...
int32 hwifi_get_mac_addr_etc(uint8 *puc_buf)
{
    return hwifi_get_mac_addr_drveprom_etc(puc_buf);
}
