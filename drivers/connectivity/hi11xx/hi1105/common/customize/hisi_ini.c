...
int32 find_download_channel_etc(uint8 *buff, uint32 buf_len, int8 *puc_var)
{
    ...
    if (strcpy_s((int8 *)buff, buf_len, version_buff) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_INI, "find_download_channel_etc:strcpy_s space is not enough");
        goto read_ini_var_fail;
    }
    buff[buf_len - 1] = '\0';
...
int32 read_conf_from_nvram_etc(uint8 *pc_out, uint32 size, uint32 nv_number, const char *nv_name)
{
    ...
    if (strcpy_s(info.nv_name, sizeof(info.nv_name), nv_name) != EOK) {
        ...
        OAM_ERROR_LOG1(0, OAM_SF_INI, "read_conf_from_nvram_etc:read nvm failed nv_name size[%lu] less than input",
                       sizeof(info.nv_name));
        return INI_FAILED;
    }
    ...
    if (ret == INI_SUCC) {
        if (memcpy_s(pc_out, size, info.nv_data, sizeof(info.nv_data)) != EOK) {
            ...
            OAM_ERROR_LOG2(0, OAM_SF_INI, "read_conf_from_nvram_etc:read nvm lenth[%d] longer than input[%d]",
...
int32 write_conf_to_nvram_etc(int8 *name, int8 *pc_arr)
{
    ...
    if (strcpy_s(info.nv_name, sizeof(info.nv_name), HISI_CUST_NVRAM_NAME) != EOK) {
        OAM_ERROR_LOG1(0, OAM_SF_INI,
                       "write_conf_to_nvram_etc:write_conf_to_nvram failed nv_name size[%lu] less than input[WINVRAM]",
                       sizeof(info.nv_name));
        return INI_FAILED;
    }
    ...
    if (memcpy_s(info.nv_data, sizeof(info.nv_data), pc_arr, HISI_CUST_NVRAM_LEN) != EOK) {
        OAM_ERROR_LOG0(0, OAM_SF_INI, "write_conf_to_nvram_etc:write nvm[memcpy_s] failed");
        return INI_FAILED;
    }
...
int32 read_conf_from_eeprom_etc(uint8 *pc_out, uint32 size, uint32 offset)
{
...
static int32 ini_file_check_timespec(INI_FILE *fp)
{
    ...
    if (ini_file_time_sec != INF_FILE_GET_CTIME(fp->f_path.dentry)) {
        INI_INFO("ini_file time_secs changed from [%ld]to[%ld]\n",
                 ini_file_time_sec, (int64)INF_FILE_GET_CTIME(fp->f_path.dentry));
...
int ini_cfg_init_etc(void)
{
    ...
    snprintf_s(ini_file_name_etc, sizeof(ini_file_name_etc), sizeof(ini_file_name_etc) - 1, "%s", INI_PATH_INI_CONN);
