...
{
    oal_uint8 uc_idx;
    wlan_channel_bandwidth_enum_uint8 en_channel_bw = WLAN_BAND_WIDTH_20M;
    uc_idx = mac_regdomain_get_channel_to_bw_mode_idx(uc_channel);

    if (uc_channel <= 14) { /* 14为2G最大信道号 */
        if (uc_idx < (OAL_SIZEOF(g_bw_mode_table_2g) / OAL_SIZEOF(g_bw_mode_table_2g[0]))) {
            en_channel_bw = g_bw_mode_table_2g[uc_idx].aen_supp_bw[en_bw_cap];
        }
    } else {
        if (uc_idx < (OAL_SIZEOF(g_bw_mode_table_5g) / OAL_SIZEOF(g_bw_mode_table_5g[0]))) {
            en_channel_bw = g_bw_mode_table_5g[uc_idx].aen_supp_bw[en_bw_cap];
        }
    }
    return en_channel_bw;
}
