...
#define HAL_RX_DSCR_NORMAL_MAX_THRES 500 /* Rx normal队列最大挂载描述符个数 */
#else
...
/* 普通优先级描述符Q-empty时算法门限累加的Delta值 */
#define HAL_NORMAL_RX_QEMPTY_DELTA_BUFFS 32
/* 普通优先级描述符Q-empty时算法门限累加到的峰值 */
#define HAL_NORMAL_RX_QEMPTY_TOP_BUFFS 128
#if (_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1103_DEV)
#define HAL_NORMAL_RX_PSM_BUFFS 32
#else
#define HAL_NORMAL_RX_PSM_BUFFS 64
#endif
...
#ifndef _PRE_WLAN_FEATURE_VO_AGGR
#define WLAN_AUTORATE_VO_AGGR_SUPPORT OAL_FALSE
#else
#define WLAN_AUTORATE_VO_AGGR_SUPPORT OAL_TRUE
#endif
/* Autorate 最小聚合时间索引 */
...
#define WLAN_PHY_EXTLNA_CHGPTDBM_TH_VAL(_en_band, _en_pm_flag) ( \
    ((_en_pm_flag) == OAL_FALSE) ? (WLAN_PHY_EXTLNA_CHGPTDBM_TH) : (((_en_band) == WLAN_BAND_2G) ? (-52) : (-40)))
