                                  ...
                                  oam_ota_type_enum_uint8 en_ota_type)
{
    ...
    /* 获取打印开关 */
    ul_ret = mac_report_80211_get_switch(pst_mac_vap, pst_rx_cb, &en_frame_switch, &en_cb_switch, &en_dscr_switch);
    if (ul_ret == OAL_ERR_CODE_PTR_NULL) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
    ...
    if ((en_ota_type == OAM_OTA_TYPE_RX_DSCR) || (en_ota_type == OAM_OTA_TYPE_RX_DSCR_PILOT)) {
        ...
        if (en_dscr_switch == OAL_SWITCH_ON) {
            ul_ret = oam_report_dscr_etc(&auc_user_macaddr[0], puc_des_addr,
                                         (oal_uint16)WLAN_RX_DSCR_SIZE, en_ota_type);
            if (ul_ret != OAL_SUCC) {
