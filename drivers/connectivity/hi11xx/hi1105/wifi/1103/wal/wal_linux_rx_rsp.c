...
OAL_STATIC oal_void wal_normal_scan_comp_handle(hmac_scan_stru *pst_scan_mgmt,
...
OAL_STATIC oal_void wal_scan_upper_issued_proc_etc(hmac_scan_rsp_stru *pst_scan_rsp, hmac_scan_stru *pst_scan_mgmt,
...
oal_uint32 wal_scan_comp_proc_sta_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (OAL_ANY_NULL_PTR2(pst_hmac_vap, pst_hmac_device)) {
        OAM_ERROR_LOG2(pst_event->st_event_hdr.uc_vap_id, 0, "{wal_scan_comp_proc_sta_etc::hmac_vap:%p hmac_dev:%p!}",
    ...
    if ((pst_scan_rsp->uc_result_code != MAC_SCAN_SUCCESS) && (pst_scan_rsp->uc_result_code != MAC_SCAN_PNO)) {
        OAM_WARNING_LOG1(pst_event->st_event_hdr.uc_vap_id, OAM_SF_SCAN,
            "{wal_scan_comp_proc_sta_etc::scan not succ, err_code[%d]!}", pst_scan_rsp->uc_result_code);
    }
    if (hmac_get_feature_switch(HMAC_MIRACAST_SINK_SWITCH)) {
        if (!pst_hmac_vap->bit_in_p2p_listen) {
            /* P2P listen在强制终止时会调用到本函数，此时不需要上报扫描结果 */
            OAM_WARNING_LOG1(pst_event->st_event_hdr.uc_vap_id, OAM_SF_SCAN,
                "{wal_scan_comp_proc_sta_etc::P2P listen NOT inform all bss, bit_in_p2p_listen[%d].}",
    ...
    /* 上层下发的普通扫描进行对应处理 */
    wal_scan_upper_issued_proc_etc(pst_scan_rsp, pst_scan_mgmt, pst_wiphy, pst_hmac_vap);

    /* 通知完内核，释放资源后解锁 */
    oal_spin_unlock(&(pst_scan_mgmt->st_scan_request_spinlock));
#ifdef _PRE_WLAN_COUNTRYCODE_SELFSTUDY
    /* 扫描完成国家码更新 */
    wal_counrtycode_selfstudy_scan_comp_etc(pst_hmac_vap);
#endif
    return OAL_SUCC;
}
...
oal_uint32 wal_asoc_comp_proc_sta_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    oal_cfg80211_connect_result_etc(pst_net_device,
                                    ...
                                    GFP_ATOMIC);
#else
    ul_ret = oal_cfg80211_connect_result_etc(pst_net_device, st_connet_result.auc_bssid, st_connet_result.puc_req_ie,
...
#ifdef _PRE_WLAN_COUNTRYCODE_SELFSTUDY
        wal_selfstudy_regdomain_update_by_ap_etc(pst_event);
#endif
    }
#endif
    ...
    OAM_WARNING_LOG1(pst_event->st_event_hdr.uc_vap_id, OAM_SF_ASSOC,
                     "{wal_asoc_comp_proc_sta_etc status_code[%d] OK!}", st_connet_result.us_status_code);
...
oal_uint32 wal_send_mgmt_to_host_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    /* 获取net_device */
    pst_net_device = wal_config_get_netdev_etc(pst_mgmt_frame->ac_name, OAL_STRLEN(pst_mgmt_frame->ac_name));
    if (pst_net_device == OAL_PTR_NULL) {
...
oal_uint32 wal_report_channel_switch_etc(frw_event_mem_stru *pst_event_mem)
{
    frw_event_stru *pst_event = OAL_PTR_NULL;
    oal_net_device_stru *pst_net_device = OAL_PTR_NULL;
    ...
    if (OAL_UNLIKELY(pst_event_mem == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_CHAN, "{wal_report_channel_switch_etc::pst_event_mem is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_net_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_CHAN, "{wal_report_channel_switch_etc::get net device ptr is null! vap_id %d}",
                       pst_event->st_event_hdr.uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_hmac_device == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_CHAN, "{wal_report_channel_switch_etc::get pst_hmac_device is null! device_id %d}",
    ...
    if (OAL_UNLIKELY(pst_wiphy == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(0, OAM_SF_CHAN, "{wal_report_channel_switch_etc:: pst_wiphy is null!}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(st_chandef.chan == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(0, OAM_SF_CHAN, "{wal_report_channel_switch_etc::can't get chan pointer from wiphy(freq=%d)!}",
    ...
    oal_cfg80211_ch_switch_notify_etc(pst_net_device, &st_chandef);

    OAM_WARNING_LOG4(pst_event->st_event_hdr.uc_vap_id, OAM_SF_CHAN,
                     "{wal_report_channel_switch_etc::center_freq=%d, width=%d, center_freq1=%d, center_freq2=%d}",
                     st_chandef.chan->center_freq,
                     st_chandef.width,
                     st_chandef.center_freq1,
                     st_chandef.center_freq2);

    wal_update_bss_for_csa_etc(pst_wiphy, pst_net_device);
