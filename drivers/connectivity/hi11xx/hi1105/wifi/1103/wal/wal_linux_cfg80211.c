...
#ifdef _PRE_CONFIG_HISI_S3S4_POWER_STATE
extern wal_info_recovery_stru g_st_recovery_info_etc;
#endif
...
oal_uint32 wal_cfg80211_open_wmm_etc(mac_vap_stru *pst_mac_vap, oal_uint16 us_len, oal_uint8 *puc_param)
{
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_CONFIG) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
            "{wal_cfg80211_open_wmm_etc::this is config vap! can't get info.}");
        return OAL_FAIL;
    }
                                   ...
                                   mac_beacon_param_stru *pst_beacon_param)
{
    ...
    if (us_capability_info & WLAN_WITP_CAPABILITY_PRIVACY) {
        ...
        if (puc_ie_tmp != OAL_PTR_NULL) {
            l_ret += memcpy_s(pst_beacon_param->auc_rsn_ie, MAC_MAX_RSN_LEN, puc_ie_tmp,
                        MAC_IE_HDR_LEN + puc_ie_tmp[1]);
        }

        /* 查找 WPA 信息元素，并解析出认证类型 */
        puc_ie_tmp = mac_find_vendor_ie_etc(MAC_WLAN_OUI_MICROSOFT, MAC_OUITYPE_WPA,
            pst_beacon_info->tail, pst_beacon_info->tail_len);
        if (puc_ie_tmp != OAL_PTR_NULL) {
            l_ret += memcpy_s(pst_beacon_param->auc_wpa_ie, MAC_MAX_RSN_LEN, puc_ie_tmp,
                        MAC_IE_HDR_LEN + puc_ie_tmp[1]);
        }
    }
    if (l_ret != EOK) {
    ...
    /*  找到wmm ie，顺便判断下uapsd是否使能 */
        ...
        if (OAL_UNLIKELY(ul_ret != OAL_SUCC)) {
            ...
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                "{wal_parse_wmm_ie_etc::uapsd switch set failed[%d].}", ul_ret);
        }
    }
#endif
    /*  wmm 开启/关闭 标记  */
...
oal_uint32 wal_cfg80211_add_vap_etc(mac_cfg_add_vap_param_stru *pst_add_vap_param)
{
    ...
    if (OAL_UNLIKELY(pst_cfg_mac_vap == OAL_PTR_NULL)) {
        OAM_WARNING_LOG1(0, 0, "{wal_cfg80211_add_vap_etc::cfg_mac_vap null vap_id:%d}", pst_mac_device->uc_cfg_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (OAL_UNLIKELY(pst_cfg_hmac_vap == OAL_PTR_NULL)) {
        OAM_WARNING_LOG1(0, 0, "{wal_cfg80211_add_vap_etc::cfg_hmac_vap null vapid:%d}", pst_mac_device->uc_cfg_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(pst_cfg_mac_vap->uc_vap_id, 0, "{wal_cfg80211_add_vap_etc::return err code %d!}", l_ret);
    ...
    if (ul_err_code != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_add_vap_etc::hmac add vap fail, ul_err_code[%u]!}\r\n", ul_err_code);
        return ul_err_code;
    }
...
oal_uint32 wal_cfg80211_del_vap_etc(mac_cfg_del_vap_param_stru *pst_del_vap_param)
{
    ...
    ***************************************************************************/
    ...
    if (OAL_SUCC != wal_send_cfg_event_etc(pst_net_dev, WAL_MSG_TYPE_WRITE,
...
oal_uint32 wal_cfg80211_vowifi_report_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{wal_cfg80211_vowifi_report_etc::pst_hmac_vap null.vap_id[%d]}",
...
oal_uint32 wal_cfg80211_cac_report(frw_event_mem_stru *pst_event_mem)
{
    ...
    pst_hmac_vap = mac_res_get_hmac_vap(pst_event->st_event_hdr.uc_vap_id);
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, 0, "{wal_cfg80211_cac_report::hmac_vap null.vap_id[%d]}", pst_event->st_event_hdr.uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
#endif
        default:
            OAM_WARNING_LOG1(0, 0, "{wal_cfg80211_cac_report::err chan width type[%d]}", pst_cac_event->en_bw_mode);
            return OAL_FAIL;
    }
...
OAL_STATIC OAL_INLINE oal_uint32 wal_parse_protocol_mode_rate_nums_check(oal_uint8 uc_rate_nums,
    oal_uint8 uc_supported_rates_num, oal_uint8 uc_extended_supported_rates_num, wlan_protocol_enum_uint8 *pen_protocol)
{
...
OAL_STATIC oal_uint32 wal_parse_protocol_mode(wlan_channel_band_enum_uint8 en_band,
    oal_beacon_parameters *pst_beacon_info, oal_uint8 *puc_ht_ie, oal_uint8 *puc_vht_ie,
    oal_uint8 *puc_he_ie, wlan_protocol_enum_uint8 *pen_protocol, oal_uint8 ul_he_flag)
{
    ...
    if (en_band == WLAN_BAND_2G) {
        ...
        puc_extended_supported_rates_ie =
            mac_find_ie_etc(MAC_EID_XRATES, pst_beacon_info->tail, pst_beacon_info->tail_len);
        if (puc_extended_supported_rates_ie != OAL_PTR_NULL) {
...
OAL_STATIC oal_uint8* wal_parse_he_ie(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_flag,
...
OAL_STATIC oal_void wal_he_flag_handle(mac_vap_stru *pst_mac_vap, oal_uint8 **ppuc_he_ie, oal_uint8 *ul_he_flag)
{
    ...
    if (p_another_vap == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id,  OAM_SF_ANY,
                         "wal_he_flag_handle:: another vap is invaild");
        return;
    }

    /* 如果STA是5G, GO或信号桥是2.4G频段,则不起在ax协议上 */
    if (p_another_vap->st_channel.en_band == WLAN_BAND_5G &&
        pst_mac_vap->st_channel.en_band == WLAN_BAND_2G) {
        // 如果STA是5G，信号桥是2.4G频段,则不起在ax协议上
        *ppuc_he_ie = OAL_PTR_NULL;
        *ul_he_flag = OAL_FALSE;

        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY,
            "wal_he_flag_handle::another vap band: STA in 5G, 2G GO can not work in he");
    }
}

OAL_STATIC oal_void wal_go_protocol_handle(mac_vap_stru *pst_mac_vap, mac_beacon_param_stru *pst_beacon_param)
{
#ifdef _PRE_WLAN_FEATURE_P2P
    if (!IS_P2P_GO(pst_mac_vap)) {
        return;
...
OAL_STATIC oal_uint32 wal_parse_ht_vht_ie(mac_vap_stru *pst_mac_vap,
                                          ...
                                          mac_beacon_param_stru *pst_beacon_param)
{
    oal_uint8 *puc_ht_ie, *puc_vht_ie;
    oal_uint8 *puc_he_ie = OAL_PTR_NULL;
    oal_uint32 ul_ret;
...
OAL_STATIC oal_int32 wal_p2p_stop_roc(mac_vap_stru *pst_mac_vap, oal_net_device_stru *pst_netdev)
{
    ...
    if (pst_mac_vap->en_vap_state == MAC_VAP_STATE_STA_LISTEN) {
        ...
        if (0 == oal_wait_for_completion_timeout(&(pst_hmac_vap->st_roc_end_ready),
            (oal_uint32)OAL_MSECS_TO_JIFFIES(200))) {
...
OAL_STATIC mac_vap_stru *wal_find_another_listen_vap_etc(mac_device_stru *pst_mac_device,
...
OAL_STATIC oal_void wal_another_vap_stop_p2p_listen(mac_device_stru *pst_mac_device, oal_uint8 uc_self_vap_id)
{
    mac_vap_stru *pst_listen_mac_vap = OAL_PTR_NULL;
    hmac_vap_stru *pst_listen_hmac_vap = OAL_PTR_NULL;
    oal_int32 l_ret;

    pst_listen_mac_vap = wal_find_another_listen_vap_etc(pst_mac_device, uc_self_vap_id);
...
OAL_STATIC OAL_INLINE oal_bool_enum_uint8 wal_cfg80211_dfr_and_s3s4_param_check(oal_void)
{
#ifdef _PRE_WLAN_FEATURE_DFR
...
#ifdef  _PRE_CONFIG_HISI_S3S4_POWER_STATE
    if (g_st_recovery_info_etc.device_s3s4_process_flag) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_dfr_and_s3s4_param_check::s3s4_process_status[%d]!}",
                         g_st_recovery_info_etc.device_s3s4_process_flag);
        return OAL_FALSE;
    }
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 44))
OAL_STATIC OAL_INLINE oal_void wal_cfg80211_scan_sched_scan_stop(hmac_scan_stru *pst_scan_mgmt,
...
#endif
{
    ...
    if (IS_P2P_SCAN_REQ(pst_request) && (OAL_TRUE == hmac_user_is_wapi_connected_etc(pst_mac_vap->uc_device_id))) {
...
OAL_STATIC oal_int32 wal_cfg80211_connect(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_net_device,
                                             ...
                                             oal_uint16 us_reason_code)
{
    ...
    if (pst_mac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_disconnect::mac_res_get_mac_user_etc pst_mac_user is null, user idx[%d]!}\r\n",
...
#endif
{
    ...
    ***************************************************************************/
    ...
    if (OAL_SUCC != wal_send_cfg_event_etc(pst_netdev, WAL_MSG_TYPE_WRITE,
...
#endif
{
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_set_default_key::wal_send_cfg_event_etc return err %d}", l_ret);
        return -OAL_EFAIL;
    }
                                                ...
                                                oal_uint8 uc_key_index)
{
    ...
    /* 1.1 入参检查 */
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR2(pst_wiphy, pst_netdev))) {
        OAM_ERROR_LOG2(0, OAM_SF_ANY, "{wal_cfg80211_set_default_mgmt_key_etc::wiphy or netdev null,error %x, %x!}",
                       (uintptr_t)pst_wiphy, (uintptr_t)pst_netdev);
        return -OAL_EINVAL;
    }
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_set_default_mgmt_key_etc::wal_send_cfg_event_etc return err code %d!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
                                             ...
                                             oal_beacon_parameters *pst_beacon_info)
{
    ...
    puc_ie = mac_get_ssid_etc(pst_beacon_info->head + MAC_80211_FRAME_LEN,
    ...
    *****************************************************************************/
    ...
    /* 发送消息 */
    l_ret = wal_send_cfg_event_etc(pst_dev, WAL_MSG_TYPE_WRITE,
                                             ...
                                             oal_net_device_stru *pst_netdev)
{
    ...
    if (wal_cfg80211_dfr_and_s3s4_param_check() == OAL_FALSE) {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_del_beacon:: dfr or s3s4 is processing!}");
        return OAL_SUCC;
    }
                                                    ...
                                                    mac_beacon_param_stru *pst_beacon_param)
{
    ...
    oal_uint32 ul_beacon_head_len, ul_beacon_tail_len, ul_offset, ul_ret;
    oal_uint8 uc_vap_id;
    mac_vap_stru *pst_mac_vap;
    oal_int32 l_ret = EOK;
    ...
    *****************************************************************************/
    ...
    /* 配置Wmm信息元素 */
    if (wal_parse_wmm_ie_etc(pst_netdev, pst_mac_vap, &st_beacon_info_tmp) != OAL_SUCC) {
...
OAL_STATIC wlan_channel_bandwidth_enum_uint8 wal_cfg80211_set_channel_cal_bw(oal_net_device_stru *netdev)
{
    ...
    if (bandwidth_value == 80 || bandwidth_value == 160) {
                                                   ...
                                                   oal_net_device_stru *pst_netdev)
{
    ...
    OAM_WARNING_LOG4(uc_vap_id, OAM_SF_ANY,
    ...
    ***************************************************************************/
    ...
    /* 发送消息 */
    l_ret = wal_send_cfg_event_etc(pst_netdev, WAL_MSG_TYPE_WRITE,
                                   WAL_MSG_WRITE_MSG_HDR_LENGTH + OAL_SIZEOF(mac_cfg_channel_param_stru),
                                   (oal_uint8 *)&st_write_msg,
                                   OAL_TRUE, &pst_rsp_msg);

    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_P2P, "{wal_cfg80211_set_channel_info::wal_send_cfg_event_etc return err:%d}", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if (ul_err_code != OAL_SUCC) {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_set_channel_info::wal_check_and_release_msg_resp_etc fail return err code:%u}", ul_err_code);
        return -OAL_EFAIL;
    }
                                                   ...
                                                   oal_cfg80211_sched_scan_request_stru *pst_request)
{
    ...
    mac_vap_stru *pst_mac_vap;
    oal_cfg80211_ssid_stru *pst_ssid_tmp, *pst_scan_ssid_list;
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, 0, "{wal_cfg80211_sched_scan_start::wal_cfg80211_start_sched_scan_etc fail[%d]!}", ul_ret);
        return -OAL_EBUSY;
    }
...
#endif
{
    ...
    if (wal_cfg80211_dfr_and_s3s4_param_check() == OAL_FALSE) {
        OAM_WARNING_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_change_virtual_intf_etc:: dfr or s3s4 is processing!}");
        return OAL_SUCC;
    }

    if (OAL_ANY_NULL_PTR2(pst_wiphy, pst_params)) {
        OAM_ERROR_LOG2(0, OAM_SF_CFG,
            "{wal_cfg80211_change_virtual_intf_etc::wiphy or flag or params ptr is null,error %x, %x, %x!}\r\n",
    ...
    if (pst_net_dev->ieee80211_ptr->iftype == en_type) {
        OAM_WARNING_LOG1(0, 0, "{wal_cfg80211_change_virtual_intf_etc::same iftype[%d],do not need change!}", en_type);
        return OAL_SUCC;
    }
    ...
    switch (en_type) {
        ...
        case NL80211_IFTYPE_ADHOC:
            OAM_ERROR_LOG1(0, 0, "{wal_cfg80211_change_virtual_intf_etc::currently do not sup this type[%d]}", en_type);
            return -OAL_EINVAL;
        ...
        case NL80211_IFTYPE_STATION:
            ...
            if (pst_net_dev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP) {
                ...
                return wal_netdev_stop_ap_etc(pst_net_dev);
            }
#endif
            // TBD:P2P BUG P2P_DEVICE 提前创建，不需要通过wpa_supplicant 创建
            pst_net_dev->ieee80211_ptr->iftype = en_type;
    ...
    if (en_type == NL80211_IFTYPE_AP) {
        return wal_setup_ap_etc(pst_net_dev);
    }
#endif
    ...
    pst_mac_vap = OAL_NET_DEV_PRIV(pst_net_dev);
    if (pst_mac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "{wal_cfg80211_change_virtual_intf_etc::can't get mac vap from netdevice priv}");
        return -OAL_EINVAL;
    }
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_CFG,
            "{wal_cfg80211_change_virtual_intf_etc::mac_res_get_dev_etc fail dev_id[%d].}", pst_mac_vap->uc_device_id);
        return -OAL_EINVAL;
    }
    ...
    if (0 == (oal_strcmp(ac_p2p_netdev_name, pst_net_dev->name))) {
        ...
        OAM_WARNING_LOG0(0, 0, "{wal_cfg80211_change_virtual_intf_etc::p2p0 netdevice can not change to P2P CLI/GO.}");
        return -OAL_EINVAL;
    }
...
OAL_STATIC oal_void wal_fill_station_rate_info(oal_station_info_stru *sta_info,
                                                   oal_station_info_stru *stats)
{
    ...
    if ((stats->txrate.flags & RATE_INFO_FLAGS_VHT_MCS) || (stats->txrate.flags & MAC_RATE_INFO_FLAGS_HE_MCS)) {
        ...
        sta_info->txrate.legacy = (oal_uint16)(stats->txrate.legacy * 10); /* 内核中单位为100kbps */
...
OAL_STATIC oal_void wal_cfg80211_fill_station_info_ext(oal_station_info_stru *pst_sta_info,
    hmac_vap_stru *pst_hmac_vap)
{
    ...
    pst_sta_info->snr = OAL_MAX(pst_hmac_vap->st_station_info_extend.c_snr_ant0,
    ...
    pst_sta_info->filled |= BIT(NL80211_STA_INFO_UL_DELAY);
    pst_sta_info->ul_delay = pst_hmac_vap->station_info.ul_delay;
#endif
}
    ...
    oal_station_info_stru *pst_sta_info)
{
    ...
    ********************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(uc_vap_id, 0, "{wal_cfg80211_get_station::wal_send_cfg_event_etc return err code %d!}", l_ret);
        return -OAL_EFAIL;
    }
    /*lint -e730*/ /* info, boolean argument to function */
...
OAL_STATIC oal_void wal_set_p2p_group_del_status(hmac_vap_stru *pst_hmac_vap,
    oal_net_device_stru *pst_net_dev)
{
    hmac_device_stru *pst_hmac_device;
...
oal_uint32 wal_del_p2p_group_etc(mac_device_stru *pst_mac_device)
{
    ...
    for (uc_vap_idx = 0; uc_vap_idx < pst_mac_device->uc_vap_num; uc_vap_idx++) {
        ...
        if (IS_P2P_GO(pst_mac_vap) || IS_P2P_CL(pst_mac_vap)) {
            ...
            if (wal_cfg80211_del_vap_etc(&st_del_vap_param) == OAL_SUCC) {
...
OAL_STATIC oal_void wal_cfg80211_add_p2p_interface_init(oal_net_device_stru *pst_net_dev,mac_device_stru *pst_mac_device)
{
    ...
    if (OAL_UNLIKELY(pst_mac_device->st_p2p_info.pst_primary_net_device == OAL_PTR_NULL)) {
        ...
        if (OAL_UNLIKELY(OAL_NETDEVICE_MAC_ADDR(pst_mac_device->st_p2p_info.pst_primary_net_device)
            == OAL_PTR_NULL)) {
        ...
        l_ret += memcpy_s(auc_primary_mac_addr, WLAN_MAC_ADDR_LEN,
...
OAL_STATIC oal_void wal_cfg80211_add_virtual_intf_netdev_init(oal_net_device_stru *pst_net_dev, oal_wireless_dev_stru *pst_wdev)
{
...
#endif
{
    ...
    mac_vap_stru *pst_cfg_mac_vap, *pst_mac_vap;
    ...
    switch (en_type) {
        ...
        case NL80211_IFTYPE_P2P_DEVICE:
            ...
            if (OAL_UNLIKELY(pst_p2p0_hmac_vap == OAL_PTR_NULL)) {
                OAM_ERROR_LOG1(0, 0, "{wal_cfg80211_add_virtual_intf::p2p0_hmac_vap[id=%d] null!}", uc_p2p0_vap_idx);
    ...
    /* 根据dev_name找到dev */
    pst_net_dev = wal_config_get_netdev_etc(puc_name, OAL_STRLEN(puc_name));
    if (pst_net_dev != OAL_PTR_NULL) {
    ...
    ***************************************************************************/
...
#endif
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    pst_add_vap_param->bit_11ac2g_enable =
        (oal_uint8)!!hwifi_get_init_value_etc(CUS_TAG_INI, WLAN_CFG_INIT_11AC2G_ENABLE);
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_cfg80211_add_virtual_intf::wal_send_cfg_event_etc err code:%d!}", l_ret);
        /*lint -e801*/
        goto ERR_STEP;
        /*lint +e801*/
    }
    ...
    if (OAL_SUCC != wal_check_and_release_msg_resp_etc(pst_rsp_msg)) {
        OAM_WARNING_LOG0(pst_cfg_mac_vap->uc_vap_id, OAM_SF_ANY,
            "{wal_cfg80211_add_virtual_intf::wal_check_and_release_msg_resp_etc fail:ul_err_code!}");
        /*lint -e801*/
        goto ERR_STEP;
        /*lint +e801*/
    }
                                                   ...
                                                   oal_wireless_dev_stru *pst_wdev)
{
    ...
    ***************************************************************************/
    ...
    if (wal_wireless_iftype_to_mac_p2p_mode_etc(pst_wdev->iftype) == WLAN_P2P_BUTT) {
        OAM_ERROR_LOG0(0, 0, "{wal_cfg80211_del_virtual_intf::wal_wireless_iftype_to_mac_p2p_mode_etc return BUTT}");
        return -OAL_EINVAL;
    }
...
OAL_STATIC oal_void wal_drv_remain_on_channel_debug_info(mac_vap_stru *pst_mac_vap,
    oal_int32 l_channel, oal_uint32 ul_duration, oal_uint64 *pull_cookie, wlan_ieee80211_roc_type_uint8 en_roc_type)
{
                                               ...
                                               wlan_ieee80211_roc_type_uint8 en_roc_type)
{
    ...
    ***************************************************************************/
    ...
    /* 3.3 发送消息 */
    l_ret = wal_send_cfg_event_etc(pst_netdev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        OAL_SIZEOF(mac_remain_on_channel_param_stru), (oal_uint8 *)&st_write_msg, OAL_TRUE, &pst_rsp_msg);

    if (l_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_P2P,
            "{wal_drv_remain_on_channel::wal_send_cfg_event_etc return err code:[%d]!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if (ul_err_code != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
            "{wal_drv_remain_on_channel::wal_send_cfg_event_etc return err code:[%u]!}", ul_err_code);
        return -OAL_EFAIL;
    }
    ...
    ***************************************************************************/
    ...
    if (l_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_P2P,
            "{wal_cfg80211_cancel_remain_on_channel::wal_send_cfg_event_etc return err code: [%d]!}", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if (ul_err_code != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_P2P,
            "{wal_cfg80211_cancel_remain_on_channel::wal_check_and_release_msg_resp_etc fail return err code:%u!}",
            ul_err_code);
        return -OAL_EFAIL;
    }
                                     ...
                                     oal_uint32 ul_wait)
{
    ...
    if (!IS_P2P_GO(pst_mac_vap)) {
        ...
        if (en_offchan == OAL_TRUE) {
            if (pst_mac_vap->en_vap_state != MAC_VAP_STATE_STA_LISTEN) {
                OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
                    "{wal_mgmt_do_tx::pst_mac_vap state[%d]not in listen!}\r\n", pst_mac_vap->en_vap_state);
    ...
    ***************************************************************************/
    ...
    /*lint -e730*/
    OAM_INFO_LOG1(0, OAM_SF_ANY, "{wal_mgmt_do_tx:: It will wait %d ms at most for tx status report!}\r\n", ul_wait);
...
OAL_STATIC oal_int32 wal_cfg80211_mgmt_tx_bss_ap_need_offchan(hmac_vap_stru *pst_hmac_vap,
                                          ...
                                          oal_uint64 *pull_cookie)
{
    ...
    *pull_cookie = pst_p2p_info->ull_send_action_id++; /* cookie值上层调用需要判断是否是这次的发送导致的callback */
    ...
    if (OAL_UNLIKELY(pst_mac_vap_rom == OAL_PTR_NULL)) {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, 0, "{wal_cfg80211_mgmt_tx::macvap_rom:%p}", (uintptr_t)pst_mac_vap_rom);
        return -OAL_EINVAL;
    }
    ...
    temp_flag = !hmac_get_feature_switch(HMAC_MIRACAST_SINK_SWITCH) ? temp_flag :
        (temp_flag && (pst_mac_vap_rom->st_probe_resp_ctrl.en_probe_resp_status == MAC_PROBE_RESP_MODE_ACTIVE));
    if (temp_flag) {
...
oal_uint32 wal_cfg80211_mgmt_tx_status_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    pst_hmac_vap = mac_res_get_hmac_vap(pst_event->st_event_hdr.uc_vap_id);
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG1(0, OAM_SF_TX, "{wal_cfg80211_mgmt_tx_status_etc::pst_hmac_vap null.vap_id[%d]}",
            pst_event->st_event_hdr.uc_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
                                              ...
                                              bool reg)
{
    ...
    OAM_INFO_LOG3(0, OAM_SF_CFG,
        "{wal_cfg80211_mgmt_frame_register_etc::enter.frame_type[0x%02x], reg[%d], if_type[%d]}",
...
OAL_STATIC OAL_INLINE oal_void wal_cfg80211_mgmt_tx_wait_event_left_time(oal_int i_leftime,
    oal_uint64 *pull_cookie, oal_mgmt_tx_stru *pst_mgmt_tx)
{
    if (i_leftime == 0) {
        /* 超时还没有上报扫描结束 */
                               ...
                               oal_uint64 *pull_cookie)
{
    ...
    if (OAL_ANY_NULL_PTR5(pst_wiphy, pst_wdev, pst_chan, pull_cookie, puc_buf)) {
        OAM_ERROR_LOG4(0, OAM_SF_CFG, "{wal_cfg80211_mgmt_tx::pst_wiphy or pst_wdev or pst_chan or \
            pull_cookie or puc_buf ptr is null, error %x, %x, %x, %x!}\r\n",
            (uintptr_t)pst_wiphy, (uintptr_t)pst_wdev, (uintptr_t)pst_chan, (uintptr_t)pull_cookie);
        return -OAL_EINVAL;
    }
    ...
    ***************************************************************************/
    ...
    /* 3.3 发送消息 */
    l_ret = wal_send_cfg_event_etc(pst_netdev, WAL_MSG_TYPE_WRITE,
                                   WAL_MSG_WRITE_MSG_HDR_LENGTH + OAL_SIZEOF(st_mgmt_tx), (oal_uint8 *)&st_write_msg,
                                   OAL_FALSE, NULL);

    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_mgmt_tx::wal_send_cfg_event_etc return err code %d!}\r\n", l_ret);
        return -OAL_EINVAL;
    }
    ...
    ***************************************************************************/
    ...
    if (OAL_UNLIKELY(l_ret != OAL_SUCC)) {
        OAM_ERROR_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_update_ft_ies::wal_send_cfg_event_etc: return err code %d!}\r\n", l_ret);
        return -OAL_EFAIL;
    }
    ...
    if (ul_err_code != OAL_SUCC) {
        OAM_WARNING_LOG1(0, OAM_SF_ANY,
            "{wal_cfg80211_update_ft_ies::wal_check_and_release_msg_resp_etc fail return err code:[%u]!}\r\n",
            ul_err_code);
        return -OAL_EFAIL;
    }
...
{
    if (wal_cfg80211_dfr_and_s3s4_param_check() == OAL_FALSE) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{wal_cfg80211_dump_survey_etc:: dfr or s3s4 is processing!}");
        return -OAL_EFAIL;
    }
...
#endif
#if (_PRE_CONFIG_TARGET_PRODUCT != _PRE_TARGET_PRODUCT_TYPE_E5) && \
     (_PRE_CONFIG_TARGET_PRODUCT != _PRE_TARGET_PRODUCT_TYPE_CPE)  // E5等hostapd适配后统一调试
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 44)) // TBD:确认正确的 Linux 版本号
...
OAL_STATIC oal_uint32 wal_cfg80211_init_netdev_etc(mac_device_stru *pst_device,oal_wiphy_stru *pst_wiphy,oal_uint8 uc_dev_id)
{
    oal_int32 l_return;
#if defined(_PRE_PRODUCT_ID_HI110X_HOST)
    oal_int8 ac_vap_netdev_name[MAC_NET_DEVICE_NAME_LENGTH] = {0};
...
#ifdef _PRE_WLAN_FEATURE_STA_AP_COEX
    /* 信道跟随softAP增加hwlan0 */
    snprintf_s(ac_vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH,
               MAC_NET_DEVICE_NAME_LENGTH - 1, "hwlan%d", uc_dev_id);

    l_return = wal_init_wlan_netdev_etc(pst_wiphy, ac_vap_netdev_name);
    if (l_return != OAL_SUCC) {
        /* 释放wlan网络设备资源 */
        OAL_MEM_FREE(OAL_NETDEVICE_WDEV(pst_device->st_p2p_info.pst_primary_net_device), OAL
_TRUE);
        oal_net_unregister_netdev(pst_device->st_p2p_info.pst_primary_net_device);
        return (oal_uint32)l_return;
    }
...
oal_uint32 wal_cfg80211_init_etc(oal_void)
{
    ...
    for (ul_chip = 0; ul_chip < ul_chip_max_num; ul_chip++) {
     ...
     (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
            ...
            l_return = wal_cfg80211_init_netdev_etc(pst_device,pst_wiphy,uc_dev_id);
            if (l_return != OAL_SUCC)
                return (oal_uint32)l_return;
