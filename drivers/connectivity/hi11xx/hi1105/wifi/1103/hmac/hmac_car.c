...
oal_uint32 hmac_car_show_info(hmac_device_stru *pst_hmac_dev)
{
    ...
    OAL_IO_PRINT("\n******************************************************\n");
    OAL_IO_PRINT("car_enable_switch:[%d]    timer_timeout:[%d]", pst_hmac_dev->en_car_enable_flag,
                 pst_hmac_dev->us_car_ctl_cycle_ms);
    OAL_IO_PRINT("   device: id[%d]\n"
        "       uplink:   flag:[%d] limit_kbps_bw:[%d] drop_packet_num:[%d]\n"
        "       downlink: flag:[%d] limit_kbps_bw:[%d] drop_packet_num:[%d]\n"
        "       multicast: flag:[%d] limit_kbps_bw:[%d] drop_packet_num:[%d];  \
        orgin_pps_num:[%d]  drop_packet_num_pps:[%d]\n",
        ...
        (pst_hmac_dev->st_car_device_multicast_cfg.ul_car_orgin_multicast_pps_num * 1000 /
        pst_hmac_dev->us_car_ctl_cycle_ms),
    ...
    for (uc_vap_index = 0; uc_vap_index < pst_mac_device->uc_vap_num; uc_vap_index++) {
        ...
        if (pst_mac_vap->en_vap_mode != WLAN_VAP_MODE_BSS_AP) {
            OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_TX,
                "{hmac_car_show_info:: CAR only used in AP mode; en_vap_mode=%d.}", pst_mac_vap->en_vap_mode);
            continue;
        }
