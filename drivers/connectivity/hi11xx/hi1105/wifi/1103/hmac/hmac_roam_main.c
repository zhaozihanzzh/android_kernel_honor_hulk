...
OAL_STATIC oal_uint32 hmac_roam_preauth_etc(hmac_vap_stru *pst_hmac_vap);
#endif
...
oal_uint32 hmac_roam_check_signal_bridge_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_other_vap == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ROAM,
                         "{hmac_roam_check_signal_bridge_etc::another vap is null!}");
        return OAL_SUCC;
    }
...
OAL_STATIC oal_void hmac_roam_param_update_etc(hmac_roam_info_stru *pst_roam_info,
    roam_channel_org_enum uc_scan_type, oal_bool_enum_uint8 en_current_bss_ignore,
    roam_trigger_enum_uint8 en_roam_trigger, oal_uint8 *pauc_target_bssid)
{
...
OAL_STATIC void hmac_roam_stop_p2p_listen(mac_vap_stru *mac_vap)
{
    ...
    hmac_device = hmac_res_get_mac_dev_etc(mac_vap->uc_device_id);
    if (hmac_device == NULL) {
    ...
    for (vap_idx = 0; vap_idx < mac_device->uc_vap_num; vap_idx++) {
        ...
        if (stop_mac_vap->en_vap_state == MAC_VAP_STATE_STA_LISTEN) {
            ...
            hmac_config_scan_abort_etc(stop_mac_vap, OAL_SIZEOF(uint32_t), (uint8_t*)&pedding_data);
        }
    }
}
                               ...
                               roam_trigger_enum_uint8 en_roam_trigger)
{
    ...
    hmac_roam_param_update_etc(pst_roam_info, uc_scan_type, en_current_bss_ignore, en_roam_trigger, pauc_target_bssid);
    ...
    hmac_chr_roam_info_report_etc(pst_roam_info, HMAC_CHR_ROAM_START);
#endif
    return OAL_SUCC;
}
...
oal_bool_enum_uint8 hmac_roam_is_neighbor_report_allowed(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (puc_cowork_ie == OAL_PTR_NULL || sizeof(oal_sta_ap_cowork_ie_beacon) > (MAC_IE_HDR_LEN + puc_cowork_ie[1])) {
...
OAL_STATIC oal_uint32 hmac_roam_scan_init(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_ROAM, "{hmac_roam_scan_init::hmac_roam_alg_scan_channel_init_etc fail[%d]}", ul_ret);
        return ul_ret;
    }
    hmac_roam_main_change_state(pst_roam_info, ROAM_MAIN_STATE_SCANING);
...
OAL_STATIC oal_uint32 hmac_roam_11v_send_btm_resp(hmac_roam_info_stru *pst_roam_info, mac_bss_dscr_stru *pst_bss_dscr)
{
    hmac_vap_stru *pst_hmac_vap = OAL_PTR_NULL;
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_RX, "{hmac_roam_11v_send_btm_resp::mac_res_get_hmac_user_etc failed.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (pst_roam_info->st_bsst_rsp_info.uc_status_code != WNM_BSS_TM_ACCEPT) {
        pst_roam_info->st_static.ul_roam_11v_scan_fail++;
        hmac_config_reg_info_etc(&(pst_hmac_vap->st_vap_base_info), BSS_TRANSITION_RESP_MSG_LEN, "all");
    }
...
OAL_STATIC oal_uint32 hmac_roam_search_bss_in_scan_list(hmac_roam_info_stru *pst_roam_info,
                                                        ...
                                                        oal_bool_enum_uint8 *bp_search_flag)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_bss_mgmt->st_bss_list_head))
    {
        ...
        if (hmac_roam_alg_bss_check_etc(pst_roam_info, pst_bss_dscr) != OAL_SUCC) {
            continue;
        }
...
OAL_STATIC oal_uint32 hmac_roam_handle_scan_result(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    pst_mac_device = mac_res_get_dev_etc(pst_roam_info->pst_hmac_vap->st_vap_base_info.uc_device_id);
    ...
    if (hmac_roam_check_scan_result(pst_roam_info, p_param, &pst_bss_dscr) == OAL_SUCC) {
        ...
        hmac_send_rsp_to_sme_sta_etc(pst_roam_info->pst_hmac_vap, HMAC_SME_SCAN_RSP, (oal_uint8 *)&st_scan_rsp);
        /* 扫描到可用的bss，开始connect */
...
OAL_STATIC oal_uint32 hmac_roam_connecting_timeout(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    ul_ret = hmac_fsm_call_func_sta_etc(pst_roam_info->pst_hmac_vap, HMAC_FSM_INPUT_ROAMING_STOP,
        (oal_void *)&post_dmac);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(0, OAM_SF_ROAM,
                       "{hmac_roam_connecting_timeout:: hmac_fsm_call_func_sta_etc fail[%d]!}", ul_ret);
    }
...
#ifdef _PRE_WLAN_1103_CHR
    hmac_chr_roam_info_report_etc(pst_roam_info, HMAC_CHR_ROAM_CONNECT_FAIL);
#endif
...
OAL_STATIC oal_uint32 hmac_roam_handle_fail_handshake_phase(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    /* 切换vap的状态为UP，恢复用户节能，恢复发送 */
    ul_ret = hmac_fsm_call_func_sta_etc(pst_roam_info->pst_hmac_vap, HMAC_FSM_INPUT_ROAMING_STOP, &post_dmac);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_roam_info->pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
...
OAL_STATIC oal_uint32 hmac_roam_to_new_bss(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    /* 切换vap的状态为UP，恢复用户节能，恢复发送 */
    ul_ret = hmac_fsm_call_func_sta_etc(pst_hmac_vap, HMAC_FSM_INPUT_ROAMING_STOP, (oal_void *)&post_dmac);
    if (ul_ret != OAL_SUCC) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
    ...
    ul_ret = hmac_config_send_event_etc(&pst_hmac_vap->st_vap_base_info,
        WLAN_CFGID_ROAM_SUCC_H2D_SYNC, OAL_SIZEOF(oal_uint8), (oal_uint8 *)&uc_roam_band_state);
    if (ul_ret != OAL_SUCC) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
...
oal_uint32 hmac_roam_pause_user_etc(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    pst_roam_info = (hmac_roam_info_stru *)pst_hmac_vap->pul_roam_info;
    if ((pst_roam_info == OAL_PTR_NULL) || (pst_roam_info->uc_enable == 0)) {
        OAM_ERROR_LOG0(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_pause_user_etc::roam disable!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    ...
    if (pst_hmac_vap->st_vap_base_info.en_vap_state != MAC_VAP_STATE_UP) {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_pause_user_etc::vap state =[%d]NOT FOR ROAMING!}", pst_hmac_vap->st_vap_base_info.en_vap_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
...
OAL_STATIC oal_void hamc_roam_mode_h2d_syn(hmac_vap_stru *pst_hmac_vap,
...
oal_uint32 hmac_roam_resume_user_etc(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    hamc_roam_mode_h2d_syn(pst_hmac_vap, pst_roam_info, *(oal_uint8 *)p_param);
...
oal_uint32 hmac_roam_reassoc_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    /* 漫游开关没有开时，不处理 */
    if (pst_roam_info->uc_enable == 0) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM, "{hmac_roam_reassoc_etc::roam disable}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }

    /* 主状态机为非CONNECTING状态，失败 */
    if (pst_roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
        OAM_WARNING_LOG1(0, OAM_SF_ROAM, "{hmac_roam_reassoc_etc::main_state=[%d] not CONNECTING}",
    ...
    if (pst_roam_info->st_connect.en_state != ROAM_CONNECT_STATE_WAIT_ASSOC_COMP) {
        if (pst_roam_info->st_connect.en_state == ROAM_CONNECT_STATE_WAIT_PREAUTH_COMP) {
            ul_ret =  hmac_roam_preauth_etc(pst_hmac_vap);
            return ul_ret;
        }
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
    ...
    if (OAL_TRUE == (mac_mib_get_ft_trainsistion(&pst_hmac_vap->st_vap_base_info))) {
        ...
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_reassoc_etc::ft_over_ds=[%d],to send reassoc!}",
                         mac_mib_get_ft_over_ds(&pst_hmac_vap->st_vap_base_info));
        ul_ret = hmac_roam_connect_ft_reassoc_etc(pst_hmac_vap);
        if (ul_ret != OAL_SUCC) {
            OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_reassoc_etc::reassoc FAIL[%d]!}", ul_ret);
            return ul_ret;
        }
    }
...
OAL_STATIC oal_uint32 hmac_roam_preauth_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL)
    {
        OAM_ERROR_LOG0(0, OAM_SF_ROAM, "{hmac_roam_preauth_etc::vap null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    ...
    if (pst_roam_info == OAL_PTR_NULL)
    {
        OAM_ERROR_LOG0(uc_vap_id, OAM_SF_ROAM, "{hmac_roam_preauth_etc::roam info null!}");
        return OAL_ERR_CODE_ROAM_INVALID_VAP;
    }
    ...
    if (0 == pst_roam_info->uc_enable) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_preauth_etc::roam disabled.}");
        return OAL_ERR_CODE_ROAM_DISABLED;
    }
    ...
    if (pst_roam_info->en_main_state != ROAM_MAIN_STATE_CONNECTING) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_preauth_etc::roam_en_main_state=[%d] not ROAM_MAIN_STATE_CONNECTING, return.}",
            pst_roam_info->en_main_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }
    ...
    if (pst_roam_info->st_connect.en_state != ROAM_CONNECT_STATE_WAIT_PREAUTH_COMP) {
        OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_preauth_etc::connect state[%d] error.}", pst_roam_info->st_connect.en_state);
        return OAL_ERR_CODE_ROAM_STATE_UNEXPECT;
    }

    if (pst_hmac_vap->bit_11r_private_preauth != OAL_TRUE) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_preauth_etc::not support preauth.}");
        return OAL_FAIL;
    }

    if (mac_mib_get_ft_trainsistion(&pst_hmac_vap->st_vap_base_info) == OAL_TRUE) {
        OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM,
            "{hmac_roam_preauth_etc::ft_over_ds=[%d],to send preauth!}",
            mac_mib_get_ft_over_ds(&pst_hmac_vap->st_vap_base_info));
        ul_ret = hmac_roam_connect_ft_preauth_etc(pst_hmac_vap);
        if (OAL_SUCC != ul_ret) {
            OAM_WARNING_LOG1(uc_vap_id, OAM_SF_ROAM,
                "{hmac_roam_preauth_etc::hmac_roam_connect_ft_preauth_etc FAIL[%d]!}", ul_ret);
            return ul_ret;
        }
    }
...
OAL_STATIC oal_uint32 hmac_sta_up_neighbor_response_process(hmac_vap_stru *pst_hmac_vap, oal_void *p_param)
{
    ...
    oal_uint8                           uc_channl_num = 0;

    pst_netbuf = (oal_netbuf_stru *)p_param;
    pst_rx_ctrl = (dmac_rx_ctl_stru *)oal_netbuf_cb(pst_netbuf);
    if (pst_rx_ctrl == OAL_PTR_NULL || pst_hmac_vap == OAL_PTR_NULL || pst_hmac_vap->pul_roam_info == OAL_PTR_NULL) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    while (l_framebody_len >= MAC_NEIGHBOR_RPT_FIX_LEN) {
        if (puc_ies_search_addr == OAL_PTR_NULL) {
            puc_ies_search_addr =((mac_action_neighbor_rsp_stru *)mac_netbuf_get_payload(pst_netbuf))->neighbor_rpt_ies;
        }
        ...
        uc_channl_num = pst_neighbor_rpt_ie->uc_channum;
        en_band = (uc_channl_num <= MAC_CHANNEL_FREQ_2_BUTT) ? WLAN_BAND_2G : WLAN_BAND_5G;
        if (mac_get_channel_idx_from_num_etc(en_band, uc_channl_num, &uc_channel_idx) == OAL_SUCC) {
            hmac_roam_neighbor_report_add_chan(pst_hmac_vap, uc_channl_num, en_band, uc_channel_idx);
        }
        /* 将解析出来的 auc_bssid 存储到 pst_roam_info->auc_neighbor_rpt_bssid[] 中 */
        /* 同时更新要扫描的邻居AP数量(pst_roam_info->uc_neighbor_rpt_bssid_num) 的值 */
        hmac_roam_neighbor_report_add_bssid(pst_hmac_vap, pst_neighbor_rpt_ie->auc_bssid);
...
oal_uint32 hmac_roam_rx_neighbor_response_action_etc(hmac_vap_stru *pst_hmac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    if (ul_ret != OAL_SUCC) {
        hmac_sta_up_neighbor_response_process(pst_hmac_vap, (oal_void *)pst_netbuf);
        return ul_ret;
    }
...
oal_void hmac_roam_neighbor_report_add_chan(hmac_vap_stru *pst_hmac_vap,
    oal_uint8 uc_channum, wlan_channel_band_enum_uint8 en_band, oal_uint8 uc_channel_idx)
{
    oal_uint8   uc_loop;
    mac_scan_req_stru *pst_scan_params = OAL_PTR_NULL;
    hmac_roam_info_stru *pst_roam_info = OAL_PTR_NULL;
    ...
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ROAM, "{hmac_roam_neighbor_report_add_chan::pst_hmac_vap is NULL}");
        return;
    }

    pst_roam_info = (hmac_roam_info_stru *)pst_hmac_vap->pul_roam_info;
    if (pst_roam_info == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ROAM, "{hmac_roam_neighbor_report_add_chan::pst_roam_info is NULL}");
        return;
    }
...
oal_void hmac_roam_neighbor_report_add_bssid(hmac_vap_stru *pst_hmac_vap, oal_uint8 *puc_bssid)
{
    oal_uint8 *puc_idex = OAL_PTR_NULL;
    hmac_roam_info_stru *pst_roam_info = OAL_PTR_NULL;

    if (pst_hmac_vap == OAL_PTR_NULL || puc_bssid == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ROAM, "{hmac_roam_neighbor_report_add_chan::NULL pointer}");
        return;
    }
...
OAL_STATIC oal_uint32 hmac_roam_neighbor_response_process(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    if (pst_rx_ctrl == OAL_PTR_NULL || pst_roam_info == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(0, OAM_SF_RRM, "{hmac_roam_neighbor_response_process::pst_rx_ctrl or pst_roam_info is NULL}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (l_framebody_len > MAC_NEIGHBOR_REPORT_ACTION_RSP_FIX_LEN) {
        ...
        while (l_framebody_len >= MAC_NEIGHBOR_RPT_FIX_LEN) {
            puc_ies_search_addr =
                (puc_ies_search_addr == OAL_PTR_NULL) ? pst_neighbor_rsp->neighbor_rpt_ies : puc_ies_search_addr;
            puc_neighbor_rpt_ie = mac_find_ie_etc(MAC_EID_NEIGHBOR_REPORT, puc_ies_search_addr, l_framebody_len);
            if (puc_neighbor_rpt_ie == OAL_PTR_NULL || sizeof(mac_neighbor_rpt_ie_stru) > (MAC_IE_HDR_LEN + puc_neighbor_rpt_ie[1])) {
                break;
            }
            pst_neighbor_rpt_ie = (mac_neighbor_rpt_ie_stru *)puc_neighbor_rpt_ie;
            en_band = (pst_neighbor_rpt_ie->uc_channum <= MAC_CHANNEL_FREQ_2_BUTT) ? WLAN_BAND_2G : WLAN_BAND_5G;
            pst_hmac_vap = hmac_container_of(&pst_roam_info, hmac_vap_stru, pul_roam_info);
            ul_ret = mac_get_channel_idx_from_num_etc(en_band, pst_neighbor_rpt_ie->uc_channum, &uc_channel_idx);
            if (ul_ret == OAL_SUCC) {
                hmac_roam_neighbor_report_add_chan(pst_hmac_vap, pst_neighbor_rpt_ie->uc_channum,
...
oal_void hmac_roam_neighbor_add_chan_bssid(hmac_vap_stru *pst_hmac_vap,
                                                       hmac_neighbor_bss_info_stru *pst_neighbor_bss_info,
                                                       oal_uint8 uc_bss_num)
{
    ...
    if (pst_hmac_vap == OAL_PTR_NULL || pst_neighbor_bss_info == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_roam_neighbor_add_chan_bssid::pst_hmac_vap or bss_info is null}");
        return;
    }
    pst_roam_info = (hmac_roam_info_stru *)pst_hmac_vap->pul_roam_info;
    if (pst_roam_info == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_roam_neighbor_add_chan_bssid::pst_roam_info is null}");
        return;
    }
    pst_scan_params = &pst_roam_info->st_scan_h2d_params.st_scan_params;
    if (pst_scan_params == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_roam_neighbor_add_chan_bssid::pst_scan_params is null}");
        return;
    }

    /* 将channel list和BSSID list相关统计信息清零 */
    pst_scan_params->uc_channel_nums = 0;
    memset_s(&pst_scan_params->ast_channel_list[0], WLAN_MAX_CHANNEL_NUM * sizeof(mac_channel_stru),
        0, WLAN_MAX_CHANNEL_NUM * sizeof(mac_channel_stru));
    pst_roam_info->uc_neighbor_rpt_bssid_num = 0;
    memset_s(&pst_roam_info->auc_neighbor_rpt_bssid[0][0], ROAM_NEIGHBOR_RPT_BSSID_MAX_NUM * WLAN_MAC_ADDR_LEN,
        0, ROAM_NEIGHBOR_RPT_BSSID_MAX_NUM * WLAN_MAC_ADDR_LEN);

    for (uc_bss_list_index = 0; uc_bss_list_index < uc_bss_num; uc_bss_list_index++) {
        uc_channum = pst_neighbor_bss_info[uc_bss_list_index].uc_chl_num;
        en_band = (uc_channum <= MAC_CHANNEL_FREQ_2_BUTT) ? WLAN_BAND_2G : WLAN_BAND_5G;
        if (mac_get_channel_idx_from_num_etc(en_band, uc_channum, &uc_channel_idx) == OAL_SUCC) {
            hmac_roam_neighbor_report_add_chan(pst_hmac_vap, uc_channum, en_band, uc_channel_idx);
        }
        hmac_roam_neighbor_report_add_bssid(pst_hmac_vap, pst_neighbor_bss_info[uc_bss_list_index].auc_mac_addr);
    }

    /* 未解析到信道, BSST流程的标志位清零 */
    if (pst_scan_params->uc_channel_nums == 0) {
        pst_scan_params->uc_bss_transition_process_flag = OAL_FALSE;
        return;
    }

    OAM_INFO_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "hmac_roam_neighbor_add_chan_bssid::ret OK");
    return;
}
#endif
...
OAL_STATIC oal_void hmac_chr_report_roam_fail_info(hmac_roam_info_stru *pst_roam_info,
    roam_fail_reason_enum_uint8 ul_result)
{
    ...
    hmac_vap_stru *pst_hmac_vap = OAL_PTR_NULL;
    mac_bss_dscr_stru *pst_bss_dscr = OAL_PTR_NULL;
    oal_uint32 ul_roam_dur;
    oal_uint32 ul_now;

    if (ul_result == HMAC_CHR_ROAM_SUCCESS || ul_result == HMAC_CHR_ROAM_START) {
        return;
    }

    ul_now = (oal_uint32)OAL_TIME_GET_STAMP_MS();
    ul_roam_dur = OAL_TIME_GET_RUNTIME(pst_roam_info->st_static.ul_connect_start_timetamp, ul_now);
    ...
    OAM_WARNING_LOG2(0, OAM_SF_ROAM, "{hmac_chr_report_roam_fail_info::1[%d]->[%d]}",
    ...
    CHR_EXCEPTION_P(CHR_WIFI_ROAM_FAIL_REPORT_EVENTID,
                    (oal_uint8 *)(&st_chr_roam_connect_info),
