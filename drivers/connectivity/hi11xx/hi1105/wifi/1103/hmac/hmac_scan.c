...
oal_void hmac_scan_print_scanned_bss_info_etc(oal_uint8 uc_device_id)
{
    ...
    OAL_DLIST_SEARCH_FOR_EACH(pst_entry, &(pst_bss_mgmt->st_bss_list_head)) {
        ...
        if (pst_scanned_bss->st_bss_dscr_info.en_new_scan_bss == OAL_TRUE) {
            ...
            /*lint -e416*/
            oam_report_80211_frame_etc(BROADCAST_MACADDR, (oal_uint8 *)auc_sdt_parse_hdr,
                                       MAC_80211_FRAME_LEN, puc_tmp_mac_body_addr,
...
OAL_STATIC oal_void hmac_scan_update_11ntxbf(mac_bss_dscr_stru *p_bss_dscr,
    uint8_t *p_frame_body, uint8_t frame_len)
{
    uint8_t *puc_ie = OAL_PTR_NULL;
    puc_ie = mac_find_vendor_ie_etc(MAC_HUAWEI_VENDER_IE, MAC_EID_11NTXBF, p_frame_body, (uint32_t)frame_len);
    if (puc_ie != OAL_PTR_NULL && sizeof(mac_11ntxbf_vendor_ie_stru) <= (MAC_IE_HDR_LEN + puc_ie[1])) {
                                                  ...
                                                  oal_uint16 us_frame_len)
{
    ...
    hmac_scan_update_11ntxbf(pst_bss_dscr, puc_frame_body, us_frame_len);
#endif
}

OAL_STATIC oal_void hmac_scan_update_bss_list_11ac_vht_cap(mac_bss_dscr_stru *pst_bss_dscr,
    oal_uint8 *puc_frame_body, oal_uint16 us_frame_len, oal_uint en_is_vendor_ie, oal_uint8 uc_vendor_subtype)
{
    oal_uint8 *puc_ie = OAL_PTR_NULL;
    ...
    if ((puc_ie != OAL_PTR_NULL) && (puc_ie[1] >= MAC_VHT_CAP_IE_LEN)) {
        ...
        if (en_is_vendor_ie == OAL_TRUE) {
            ...
            if ((uc_vendor_subtype == MAC_WLAN_OUI_VENDOR_VHT_SUBTYPE2)
...
OAL_STATIC oal_void hmac_scan_update_bss_list_11ac(mac_bss_dscr_stru *pst_bss_dscr,
                                                   ...
                                                   oal_uint8 uc_vendor_subtype)
{
    oal_uint8 *puc_ie = OAL_PTR_NULL;
    oal_uint8 uc_vht_chan_width, uc_chan_center_freq, uc_chan_center_freq_1;
...
OAL_STATIC oal_void hmac_scan_update_bw_cap(mac_frame_he_cap_ie_stru *pst_he_cap_value,
                                                   ...
                                                   oal_uint16 us_frame_len)
{
    ...
    if ((puc_ie != OAL_PTR_NULL) && (puc_ie[1] >= MAC_HE_CAP_MIN_LEN)) {
        ...
        if (st_he_cap_value.st_he_phy_cap.bit_ldpc_coding_in_paylod == OAL_FALSE) {
            pst_bss_dscr->en_he_capable = OAL_FALSE;
        }

        hmac_scan_update_bw_cap(&st_he_cap_value, pst_bss_dscr);
        /* 更新用户支持发送的txbf天线数 */
...
OAL_STATIC OAL_INLINE oal_void hmac_scan_update_11ax_ie(hmac_vap_stru *pst_hmac_vap,
                                                        mac_bss_dscr_stru *pst_bss_dscr,
                                                        oal_uint8 *puc_frame_body,
                                                        oal_uint16 us_frame_len)
{
...
OAL_STATIC OAL_INLINE oal_void hmac_scan_update_hisi_cap_ie(hmac_vap_stru *pst_hmac_vap,
...
OAL_STATIC OAL_INLINE oal_void hmac_scan_update_cap_ie_part1(hmac_vap_stru *pst_hmac_vap,
    mac_bss_dscr_stru *pst_bss_dscr, oal_uint8 *puc_frame_body, oal_uint16 us_frame_len)
{
    /* 11ax */
#ifdef _PRE_WLAN_FEATURE_11AX
                                                    ...
                                                    oal_uint16 us_frame_len)
{
    ...
    if (mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_RALINK,
                               MAC_WLAN_CHIP_OUI_TYPE_RALINK,
                               puc_frame_body, us_frame_len) != OAL_PTR_NULL ||
        mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_SHENZHEN,
                               MAC_WLAN_CHIP_OUI_TYPE_SHENZHEN,
                               puc_frame_body, us_frame_len) != OAL_PTR_NULL ||
        mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_BROADCOM,
                               MAC_WLAN_CHIP_OUI_TYPE_BROADCOM,
                               puc_frame_body, us_frame_len) != OAL_PTR_NULL ||
        mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_ATHEROSC,
                               MAC_WLAN_CHIP_OUI_TYPE_ATHEROSC,
                               puc_frame_body, us_frame_len) != OAL_PTR_NULL ||
        mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_REALTEK,
                               MAC_WLAN_CHIP_OUI_TYPE_REALTEK,
                               puc_frame_body, us_frame_len) != OAL_PTR_NULL ||
        mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_METALINK,
                               MAC_WLAN_CHIP_OUI_TYPE_METALINK,
                               puc_frame_body, us_frame_len) != OAL_PTR_NULL) {
        pst_bss_dscr->en_btcoex_blacklist_chip_oui |= MAC_BTCOEX_BLACKLIST_LEV0;
    }

    if (((mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_APPLE1,
                                 MAC_WLAN_CHIP_OUI_TYPE_APPLE_1_1,
                                 puc_frame_body, us_frame_len) != OAL_PTR_NULL) ||
         (mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_APPLE1,
                                 MAC_WLAN_CHIP_OUI_TYPE_APPLE_1_2,
                                 puc_frame_body, us_frame_len) != OAL_PTR_NULL) ||
         (mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_APPLE2,
...
OAL_STATIC OAL_INLINE oal_void hmac_scan_bss_chip_oui(mac_bss_dscr_stru *pst_bss_dscr,
    oal_uint8 *puc_frame_body, oal_uint16 us_frame_len)
{
    ...
    } else if (OAL_PTR_NULL != mac_find_vendor_ie_etc(
        MAC_WLAN_CHIP_OUI_QUALCOMM, MAC_WLAN_CHIP_OUI_TYPE_QUALCOMM, puc_frame_body, us_frame_len)) {
                                                                  ...
                                                                  oal_uint16 us_frame_len)
{
    ...
    if (OAL_PTR_NULL != mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_ATHEROSC, MAC_WLAN_CHIP_OUI_TYPE_ATHEROSC,
                                               puc_frame_body, us_frame_len)) {
...
#ifdef _PRE_WLAN_FEATURE_ROAM
    if (OAL_PTR_NULL != mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_BROADCOM, MAC_WLAN_CHIP_OUI_TYPE_BROADCOM,
                                               puc_frame_body, us_frame_len)) {
    ...
    if (OAL_PTR_NULL != mac_find_vendor_ie_etc(MAC_WLAN_CHIP_OUI_MARVELL, MAC_WLAN_CHIP_OUI_TYPE_MARVELL,
                                               puc_frame_body, us_frame_len)) {
...
oal_bool_enum_uint8 hmac_scan_need_update_old_scan_result_etc(hmac_vap_stru *pst_hmac_vap,
                                                              hmac_scanned_bss_info *pst_new_bss,
                                                              hmac_scanned_bss_info *pst_old_bss)
{
...
OAL_STATIC oal_uint32 hmac_scan_update_bss_dscr(hmac_vap_stru *pst_hmac_vap,
    oal_netbuf_stru *pst_netbuf, oal_uint16 us_buf_len, hmac_scanned_bss_info *pst_scanned_bss, oal_uint8 uc_ext_len)
{
                                             ...
                                             mac_multi_bssid_frame_info_stru *pst_mbss_frame_info)
{
    ...
    if (OAL_FALSE == hmac_scan_need_update_old_scan_result_etc(pst_hmac_vap, pst_new_scanned_bss, pst_old_scanned_bss)
...
oal_uint8 hmac_scan_proc_non_transmit_bssid(hmac_vap_stru *pst_hmac_vap,
    ...
    oal_uint16 us_left_frame_body_len, mac_ieee80211_frame_stru *pst_frame_header)
{
...
oal_uint32 hmac_scan_proc_scan_comp_event_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    pst_hmac_vap = mac_res_get_hmac_vap(pst_event_hdr->uc_vap_id);
    if (OAL_UNLIKELY(pst_hmac_vap == OAL_PTR_NULL)) {
        OAM_ERROR_LOG0(pst_event_hdr->uc_vap_id, OAM_SF_SCAN, "{hmac_scan_proc_scan_comp_event_etc::hmac_vap null.}");
    ...
    if (pst_hmac_vap->en_wait_roc_end == OAL_TRUE) {
        ...
        OAM_WARNING_LOG1(pst_event_hdr->uc_vap_id, OAM_SF_SCAN,
            "{hmac_scan_proc_scan_comp_event_etc::scan rsp status[%d]}", pst_d2h_scan_rsp_info->en_scan_rsp_status);
...
OAL_STATIC oal_uint32 hmac_cac_abort_scan_check(hmac_device_stru *pst_hmac_device)
{
...
oal_uint32 hmac_scan_check_is_dispatch_scan_req(hmac_vap_stru *pst_hmac_vap,
    hmac_device_stru *pst_hmac_device)
{
    oal_uint32 ul_ret;
...
#ifdef _PRE_WLAN_FEATURE_DFS
    /* 4.判断当前是否正在执行CAC */
    if (OAL_SUCC == hmac_cac_abort_scan_check(pst_hmac_device)) {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
...
oal_uint32 hmac_scan_proc_scan_req_event_etc(hmac_vap_stru *pst_hmac_vap, oal_void *p_params)
{
    ...
    hmac_device_stru *pst_hmac_device = OAL_PTR_NULL;
    hmac_scan_record_stru *pst_scan_record = OAL_PTR_NULL;
    oal_uint32 ul_scan_timeout, ul_ret;
    ...
    /* 扫描停止模块测试 */
    temp_flag = (((g_en_bgscan_enable_flag_etc == HMAC_BGSCAN_DISABLE) &&
                  (pst_hmac_vap->st_vap_base_info.en_vap_state == MAC_VAP_STATE_UP)) ||
                  (g_en_bgscan_enable_flag_etc == HMAC_SCAN_DISABLE));
    if (temp_flag) {
        OAM_WARNING_LOG1(0, 0, "hmac_scan_proc_scan_req_event_etc:bgscan_en_flag:%d", g_en_bgscan_enable_flag_etc);
        return OAL_FAIL;
    }
    ...
    if (pst_hmac_device == OAL_PTR_NULL) {
        OAM_WARNING_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_scan_proc_scan_req_event_etc::hmac_device null.}");
        return OAL_ERR_CODE_MAC_DEVICE_NULL;
    }
    ...
    if (pst_hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_scan_proc_scan_req_event_etc::save last vap_state:%d}", pst_hmac_vap->st_vap_base_info.en_vap_state);
    ...
    if (pst_netbuf_scan_req == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_SCAN,
            "{hmac_scan_proc_scan_req_event_etc::pst_netbuf_scan_req alloc failed.}");
    ...
    FRW_TIMER_CREATE_TIMER(&(pst_hmac_device->st_scan_mgmt.st_scan_timeout), hmac_scan_proc_scan_timeout_fn,
