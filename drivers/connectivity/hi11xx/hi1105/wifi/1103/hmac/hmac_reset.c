...
OAL_STATIC oal_uint8 hmac_proc_query_response_flag_etc(hmac_vap_stru *hmac_vap,
...
oal_uint32 hmac_proc_query_response_event_etc(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    if (pst_query_station_reponse_event->query_event == OAL_QUERY_STATION_INFO_EVENT) {
        ...
        pst_hmac_vap->station_info.ul_delay =
            pst_query_station_reponse_event->us_asoc_id;
#endif
...
#else
            uc_flag |= hmac_proc_query_response_flag_etc(pst_hmac_vap, pst_query_station_reponse_event);
#endif
