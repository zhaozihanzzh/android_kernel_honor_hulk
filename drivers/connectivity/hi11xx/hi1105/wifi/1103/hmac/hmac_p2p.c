...
OAL_STATIC oal_void hmac_p2p_go_back_to_80m(mac_vap_stru *p_go_mac_vap)
{
    wlan_bw_cap_enum_uint8 bw_cap;
    wlan_channel_bandwidth_enum_uint8 new_80bw;

    bw_cap = mac_vap_bw_mode_to_bw(p_go_mac_vap->st_channel.en_bandwidth);
    if (bw_cap == WLAN_BW_CAP_160M) {
        new_80bw = mac_regdomain_get_support_bw_mode(WLAN_BAND_WIDTH_80M, p_go_mac_vap->st_channel.uc_chan_number);
        ...
        if (bw_cap == WLAN_BW_CAP_80M) {
            OAM_WARNING_LOG2(0, OAM_SF_QOS, "{hmac_p2p_go_back_to_80m::GO BW change from\
                [%d] to [%d]}", p_go_mac_vap->st_channel.en_bandwidth, new_80bw);
#ifdef _PRE_WLAN_FEATURE_DFS
            hmac_dfs_status_set(HMAC_BACK_80M);
#endif
            p_go_mac_vap->st_ch_switch_info.uc_ch_switch_cnt = 1;
            p_go_mac_vap->st_ch_switch_info.en_csa_mode      = WLAN_CSA_MODE_TX_ENABLE;
            hmac_chan_initiate_switch_to_new_channel(p_go_mac_vap,
                p_go_mac_vap->st_channel.uc_chan_number, new_80bw);
        }
    }
}


oal_void hmac_p2p_sta_join_go_back_to_80m_handle(mac_vap_stru *p_sta_mac_vap)
{
#ifdef _PRE_WLAN_FEATURE_160M
    mac_vap_stru *p_go_mac_vap;

    if (!IS_LEGACY_STA(p_sta_mac_vap)) {
        return;
    }

    if (OAL_TRUE == mac_vap_go_can_not_in_160M_check(p_sta_mac_vap, p_sta_mac_vap->st_channel.uc_chan_number)) {
        p_go_mac_vap = mac_vap_find_another_up_vap_by_mac_vap(p_sta_mac_vap);
        if (p_go_mac_vap != OAL_PTR_NULL && IS_P2P_GO(p_go_mac_vap)) {
            hmac_p2p_go_back_to_80m(p_go_mac_vap);
        }
    }
#endif
}
