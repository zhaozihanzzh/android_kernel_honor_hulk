...
oal_uint16 mac_get_center_freq1_from_freq_and_bandwidth(oal_uint16 freq,
    wlan_channel_bandwidth_enum_uint8 en_bandwidth)
{
    oal_uint16       us_center_freq1;
    ...
    ********************************************************/
    oal_int8        ac_center_chan_offset[WLAN_BAND_WIDTH_BUTT - 2] = {
                        0, 2, -2, 6, -2, 2, -6
...
oal_uint32 hmac_report_channel_switch_etc(hmac_vap_stru *pst_hmac_vap, mac_channel_stru *pst_channel)
{
    frw_event_mem_stru *pst_event_mem = OAL_PTR_NULL;
    frw_event_stru *pst_event = OAL_PTR_NULL;
    hmac_channel_switch_stru st_channel_switch = {0};

    if (OAL_ANY_NULL_PTR2(pst_hmac_vap, pst_channel)) {
        OAM_ERROR_LOG0(0, OAM_SF_CHAN, "{hmac_report_channel_switch_etc:: hmac_vap or pst_channel is NULL}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    if (OAL_SUCC != mac_is_channel_num_valid_etc(pst_channel->en_band, pst_channel->uc_chan_number)) {
        OAM_ERROR_LOG2(0, OAM_SF_CHAN, "{hmac_report_channel_switch_etc::invalid channel, en_band=%d, ch_num=%d",
            pst_channel->en_band, pst_channel->uc_chan_number);
        return OAL_FAIL;
    }

    st_channel_switch.center_freq = (pst_channel->en_band == WLAN_BAND_2G) ?
            (g_ast_freq_map_2g_etc[pst_channel->uc_chan_idx].us_freq) :
            (g_ast_freq_map_5g_etc[pst_channel->uc_chan_idx].us_freq);
    ...
    OAM_WARNING_LOG4(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHAN,
        "hmac_report_channel_switch_etc: center_freq=%d width=%d, center_freq1=%d center_freq2=%d",
        st_channel_switch.center_freq, st_channel_switch.width,
        st_channel_switch.center_freq1, st_channel_switch.center_freq2);

    pst_event_mem = FRW_EVENT_ALLOC(OAL_SIZEOF(st_channel_switch));
    if (pst_event_mem == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_CHAN,
            "{hmac_report_channel_switch_etc::FRW_EVENT_ALLOC fail! size[%d]}", OAL_SIZEOF(st_channel_switch));
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (EOK != memcpy_s((oal_uint8 *)frw_get_event_payload(pst_event_mem), OAL_SIZEOF(st_channel_switch),
                        (oal_uint8 *)&st_channel_switch, OAL_SIZEOF(st_channel_switch))) {
        OAM_ERROR_LOG0(0, OAM_SF_CHAN, "hmac_report_channel_switch_etc::memcpy fail!");
        FRW_EVENT_FREE(pst_event_mem);
        return OAL_FAIL;
    }
...
oal_uint32 hmac_chan_switch_to_new_chan_complete_etc(frw_event_mem_stru *pst_event_mem)
{
    ...
    OAM_WARNING_LOG0(pst_event->st_event_hdr.uc_vap_id, OAM_SF_2040, "hmac_chan_switch_to_new_chan_complete_etc");
    hmac_dump_chan_etc(pst_mac_vap, (oal_uint8 *)pst_set_chan);
    ...
    if (IS_STA(pst_mac_vap)) {
        ...
        hmac_report_channel_switch_etc(pst_hmac_vap, &(pst_mac_vap->st_channel));
    }
#endif
