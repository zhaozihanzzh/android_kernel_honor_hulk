...
oal_void hmac_dfs_status_set(oal_uint32 dfs_status);
OAL_STATIC oal_void hmac_dfs_go_cac_forbit_scan_flag_set(mac_vap_stru *pst_mac_vap,
...
oal_uint32 hmac_dfs_cac_timeout_fn_etc(oal_void *p_arg)
{
    ...
    hmac_dfs_go_cac_forbit_scan_flag_set(&pst_hmac_vap->st_vap_base_info, OAL_FALSE);
    ...
    if (pst_dfs_info->uc_timer_cnt != pst_dfs_info->uc_timer_end_cnt) {
        ...
        FRW_TIMER_CREATE_TIMER(&pst_mac_device->st_dfs.st_dfs_cac_timer,
                               ...
                               pst_hmac_vap, OAL_FALSE, OAM_MODULE_ID_HMAC,
...
oal_void hmac_dfs_cac_stop_etc(mac_device_stru *pst_mac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (g_st_dfs_result.en_dfs_status == HMAC_CAC_START ||
        ...
        en_dfs_status = (g_st_dfs_result.en_dfs_status == HMAC_CAC_START) ? HMAC_CAC_STOP : HMAC_INS_STOP;
        hmac_dfs_status_set(en_dfs_status);
    }
    hmac_dfs_go_cac_forbit_scan_flag_set(pst_mac_vap, OAL_FALSE);
    ...
    oal_bool_enum_uint8 en_flag)
{
    ...
    if (IS_P2P_GO(pst_mac_vap)) {
        g_st_dfs_result.bit_go_cac_forbit_scan = en_flag;
    }
}


OAL_STATIC oal_void hmac_dfs_cac_start_save_dfs_result(hmac_vap_stru *pst_hmac_vap)
{
    mac_vap_stru *pst_mac_vap;

    pst_mac_vap = &pst_hmac_vap->st_vap_base_info;
    g_st_dfs_result.us_freq = g_ast_freq_map_5g_etc[pst_mac_vap->st_channel.uc_chan_idx].us_freq;
    g_st_dfs_result.uc_bw = mac_vap_bw_mode_to_bw(pst_mac_vap->st_channel.en_bandwidth);

    if (EOK != memcpy_s(g_st_dfs_result.auc_name, OAL_IF_NAME_SIZE, \
        pst_hmac_vap->auc_name, OAL_IF_NAME_SIZE)) {
        OAM_ERROR_LOG0(0, OAM_SF_CFG, "hmac_dfs_cac_start_save_dfs_result::memcpy fail!");
    }
}


oal_uint32 hmac_dfs_go_cac_check(mac_vap_stru *pst_mac_vap)
{
    ...
    hmac_dfs_status_set(HMAC_INS_START);
    hmac_dfs_cac_start_save_dfs_result(pst_hmac_vap);

    if (!IS_LEGACY_VAP(pst_mac_vap) && g_go_cac == OAL_FALSE) {
        g_go_cac = OAL_TRUE;
        return OAL_FAIL;
    }
...
oal_void hmac_dfs_cac_start_etc(mac_device_stru *pst_mac_device, hmac_vap_stru *pst_hmac_vap)
{
    ...
    hmac_dfs_cac_start_save_dfs_result(pst_hmac_vap);
    hmac_dfs_go_cac_forbit_scan_flag_set(&pst_hmac_vap->st_vap_base_info, OAL_TRUE);
...
oal_void hmac_dfs_radar_wait_etc(mac_device_stru *pst_mac_device, hmac_vap_stru *pst_hmac_vap)
{
    ...
    if ((pst_hmac_vap == OAL_PTR_NULL) || (pst_hmac_vap->st_vap_base_info.st_channel.en_band != WLAN_BAND_5G)) {
        return;
    }
...
oal_uint32 hmac_dfs_ap_wait_start_radar_handler_etc(hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum) {
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_DFS, "{hmac_dfs_ap_wait_start_radar_handler_etc::\
            next ch(%d) and bw(%d) set by user.}", uc_new_channel, en_new_bandwidth);
                                                 ...
                                                 mac_vap_stru *pst_mac_vap)
{
    ...
    if (pst_mac_device->st_dfs.st_dfs_info.uc_custom_next_chnum) {
        ...
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_CHAN, "{hmac_dfs_switch_channel_for_radar_etc::\
            next ch(%d) and bw(%d) set by user.}", uc_new_channel, en_new_bandwidth);
                                       ...
                                       hmac_device_stru *pst_hmac_device)
{
    ...
    for (uc_vap_id = 0; uc_vap_id < pst_mac_device->uc_vap_num; uc_vap_id++) {
        ...
        if (pst_vap != OAL_PTR_NULL && pst_vap->en_vap_mode == WLAN_VAP_MODE_BSS_AP) {
