...
hisi_max_cpu_freq g_aul_cpumaxfreq_etc[OAL_BUS_MAXCPU_NUM];
struct pm_qos_request g_st_pmqos_requset_etc;
#endif
oal_bool_enum_uint8 g_feature_switch_etc[HMAC_FEATURE_SWITCH_BUTT] = {0};
...
OAL_STATIC oal_void hmac_event_fsm_tx_adapt_subtable_register_ext(oal_void)
{
#ifdef _PRE_WLAN_FEATURE_HIEX
    g_ast_dmac_wlan_ctx_event_sub_table_etc[DMAC_WLAN_CTX_EVENT_SUB_TYPE_HIEX_H2D_MSG].p_tx_adapt_func =
        hmac_hiex_h2d_msg_tx_adapt;
#endif
}
...
OAL_STATIC oal_void hmac_event_fsm_action_subtable_register_ext(oal_void)
{
#ifdef _PRE_WLAN_FEATURE_HIEX
    g_ast_hmac_wlan_crx_event_sub_table_etc[DMAC_WLAN_CRX_EVENT_SUB_TYPE_HIEX_D2H_MSG].p_func =
        hmac_hiex_rx_local_msg;
#endif
}
...
OAL_STATIC oal_void hmac_rxdata_netbuf_delist(oal_netdev_priv_stru *netdev_priv)
{
...
OAL_STATIC oal_void hmac_rxdata_napi_shedule_process(oal_netdev_priv_stru *netdev_priv)
{
#ifdef _PRE_CONFIG_HISI_110X_NAPI_DYNAMIC_SWITCH
...
OAL_STATIC oal_int32 hmac_rxdata_thread(oal_void *p_data)
{
    ...
    oal_set_thread_property_etc(current, SCHED_FIFO, &param, -10);
...
OAL_STATIC oal_uint32 hmac_hisi_cpufreq_init(oal_void)
{
    ...
    if (g_freq_lock_control_etc.uc_lock_max_cpu_freq == OAL_FALSE) {
        return OAL_SUCC;
    }

    memset_s(&g_aul_cpumaxfreq_etc, sizeof(g_aul_cpumaxfreq_etc), 0, sizeof(g_aul_cpumaxfreq_etc));
    ...
    for (uc_cpuid = 0; uc_cpuid < OAL_BUS_MAXCPU_NUM; uc_cpuid++) {
        ...
        g_aul_cpumaxfreq_etc[uc_cpuid].max_cpu_freq = hisi_cpufreq_get_maxfreq_etc(uc_cpuid);
        g_aul_cpumaxfreq_etc[uc_cpuid].valid = OAL_TRUE;
    }
...
OAL_STATIC oal_uint32 hmac_hisi_cpufreq_exit(oal_void)
{
    ...
    for (uc_cpuid = 0; uc_cpuid < OAL_BUS_MAXCPU_NUM; uc_cpuid++) {
        /* 未获取到正确的cpu频率则不设置 */
        if (g_aul_cpumaxfreq_etc[uc_cpuid].valid != OAL_TRUE) {
            continue;
        }

        hisi_cpufreq_exit_req(&g_ast_cpufreq_etc[uc_cpuid]);
        g_aul_cpumaxfreq_etc[uc_cpuid].valid = OAL_FALSE;
    }
...
#if defined(_PRE_WLAN_PROFLING_MIPS) || defined(_PRE_WLAN_RR_PERFORMENCE_DEBUG)

OAL_STATIC oal_void hmac_main_mips_and_rr_init_etc(oal_void) {
#ifdef _PRE_WLAN_PROFLING_MIPS
    oal_profiling_mips_init();
#endif

#ifdef _PRE_WLAN_RR_PERFORMENCE_DEBUG
    oal_profiling_rr_perform_timestamp_init();
#endif
}
#endif
...
#if defined(_PRE_WLAN_PROFLING_MIPS) || defined(_PRE_WLAN_RR_PERFORMENCE_DEBUG)
    hmac_main_mips_and_rr_init_etc();
#endif
...
oal_bool_enum_uint8 hmac_get_feature_switch(hmac_feature_switch_type_enum_uint8 feature_id)
{
    ...
    return g_feature_switch_etc[feature_id];
}

#ifdef _PRE_WLAN_FEATURE_DYN_CLOSED
oal_int32 is_hisi_insmod_hi110x_wlan(oal_void)
{
...
#endif
oal_int32 hmac_net_register_netdev(oal_net_device_stru *p_net_device)
{
#ifdef _PRE_WLAN_FEATURE_DYN_CLOSED
