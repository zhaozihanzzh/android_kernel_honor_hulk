...
#endif
        {
            pst_hmac_vap->bit_11k_enable = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_11k;
            pst_hmac_vap->bit_11v_enable = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_11v;
            pst_hmac_vap->bit_bcn_table_switch = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_11k;
        }
...
#endif
    {
        pst_hmac_vap->bit_11r_enable = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_11r;
        pst_hmac_vap->bit_11r_over_ds = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_11r_ds;
        pst_hmac_vap->bit_adaptive11r = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_adaptive11r;
        pst_hmac_vap->bit_nb_rpt_11k = g_st_mac_device_custom_cfg.st_voe_custom_cfg.en_nb_rpt_11k;
        pst_hmac_vap->bit_11r_private_preauth = pst_hmac_vap->bit_adaptive11r;
    }
#endif
    return;
}
                             ...
                             mac_cfg_add_vap_param_stru *pst_param)
{
    ...
    ul_ret = mac_vap_init_etc(&pst_hmac_vap->st_vap_base_info,
                              uc_chip_id, uc_device_id, uc_vap_id, pst_param);
...
#endif
#ifdef _PRE_WLAN_FEATURE_11AX
            hmac_wifi6_self_cure_init(pst_hmac_vap);
#endif
            break;
...
OAL_STATIC oal_uint32 hmac_vap_creat_netdev_set_net_device_etc(oal_net_device_stru *pst_net_device, oal_int8 *puc_mac_addr, mac_vap_stru *pst_vap)
{
    oal_int32 l_ret;
                                     ...
                                     oal_uint8 uc_netdev_name_len, oal_int8 *puc_mac_addr)
{
    ...
    if (OAL_UNLIKELY(OAL_ANY_NULL_PTR2(pst_hmac_vap, puc_netdev_name))) {
        OAM_ERROR_LOG2(0, OAM_SF_ASSOC,"{hmac_vap_creat_netdev_etc::param null %d %d.}",
    ...
    l_ret = hmac_vap_creat_netdev_set_net_device_etc(pst_net_device, puc_mac_addr, pst_vap);

#ifdef _PRE_CONFIG_HISI_110X_DISABLE_HISILICON0

#else
    ul_return = (oal_uint32)hmac_net_register_netdev(pst_net_device);
    if (OAL_UNLIKELY(ul_return != OAL_SUCC)) {
        MAC_WARNING_LOG(pst_vap->uc_vap_id, "mac_device_init_etc:hmac_net_register_netdev return fail.");
        OAM_WARNING_LOG0(pst_vap->uc_vap_id, OAM_SF_ANY,
                         "{hmac_vap_creat_netdev_etc::hmac_net_register_netdev failed.}");
                                                 ...
                                                 hmac_user_stru *pst_hmac_user_sta)
{
    ...
    if (pst_ht_hdl->en_ht_capable == OAL_FALSE) { /* STA不支持HT */
        ...
        hmac_amsdu_init_user_etc(pst_hmac_user_sta);
...
#endif /* _PRE_WLAN_FEATURE_P2P */ /*lint +e19*/
#ifdef __cplusplus
#if __cplusplus
}
#endif
