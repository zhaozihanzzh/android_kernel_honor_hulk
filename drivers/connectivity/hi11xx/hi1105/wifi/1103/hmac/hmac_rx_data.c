...
OAL_STATIC oal_uint32 hmac_rx_amsdu_check_frame_len(oal_netbuf_stru *pst_netbuf,
    dmac_msdu_proc_state_stru *pst_msdu_state, oal_uint16 us_submsdu_len, oal_uint8 uc_submsdu_pad_len)
{
...
oal_void hmac_pkt_mem_opt_cfg_etc(oal_uint32 ul_cfg_tpye, oal_uint32 ul_cfg_value)
{
    ...
    hmac_rx_dscr_opt_stru *pst_dscr_opt = OAL_PTR_NULL;
...
OAL_STATIC oal_void hmac_rx_transmit_msdu_to_lan_mips_and_time_info(oal_void)
{
                                                 ...
                                                 dmac_msdu_stru *pst_msdu)
{
    ...
    if (en_trace_pkt_type != PKT_TRACE_BUTT) {
        oam_report_eth_frame_etc(puc_mac_addr,
                                 ...
                                 OAM_OTA_FRAME_DIRECTION_TYPE_RX);
        OAM_WARNING_LOG1(pst_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
            "{hmac_rx_transmit_msdu_to_lan::type%d to eth \
            [0:dhcp 1:arp_req 2:arp_rsp 3:eapol 4:icmp 5:assoc_req 6:assoc_rsp 9:dis_assoc 10:auth 11:deauth]}\r\n",
            en_trace_pkt_type);
    }
#endif
    ...
    /* 接收流程mips和rr打点统计 */
    hmac_rx_transmit_msdu_to_lan_mips_and_time_info();
...
oal_void hmac_rx_vip_info(mac_vap_stru *pst_vap, oal_uint8 uc_data_type,
                               oal_netbuf_stru *pst_buf, mac_rx_ctl_stru *pst_rx_ctrl)
{
    ...
    if (uc_data_type == MAC_DATA_EAPOL) {
        ...
        OAM_WARNING_LOG3(pst_vap->uc_vap_id, OAM_SF_ANY,
                         "{hmac_rx_vip_info::EAPOL type=%u, len==%u, user[%d]}[1:1/4 2:2/4 3:3/4 4:4/4]",
                         en_eapol_type, OAL_NETBUF_LEN(pst_buf), MAC_GET_RX_CB_TA_USER_IDX(pst_rx_ctrl));
        ...
        if (pst_mac_header != OAL_PTR_NULL) {
            if (!ETHER_IS_BROADCAST(pst_mac_header->auc_address1) && !ETHER_IS_BROADCAST(pst_mac_header->auc_address2)) {
                hmac_ht_self_cure_event_set(pst_vap, pst_mac_header->auc_address2, HMAC_HT_SELF_CURE_EVENT_RX_DHCP_FRAME);
            }
        }
        uc_dhcp_type = mac_get_dhcp_frame_type_etc(pst_rx_ip_hdr);
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY,
            "{hmac_rx_vip_info::DHCP type=%d.[1:discovery 2:offer 3:request 4:decline 5:ack 6:nack 7:release 8:inform.]",
            uc_dhcp_type);
        OAM_WARNING_LOG4(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_rx_vip_info:: DHCP sip: %d.%d, dip: %d.%d.",
                         auc_ar_sip[2], auc_ar_sip[3], auc_ar_dip[2], auc_ar_dip[3]);
    } else {
        puc_arp_head = (oal_eth_arphdr_stru *)(pst_llc + 1);
        OAM_WARNING_LOG1(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_rx_vip_info:: ARP type=%d.[2:arp resp 3:arp req.]",
                         uc_data_type);
        OAM_WARNING_LOG4(pst_vap->uc_vap_id, OAM_SF_ANY, "{hmac_rx_vip_info:: ARP sip: %d.%d, dip: %d.%d",
    ...
    if (pst_mac_header != OAL_PTR_NULL) {
        OAM_WARNING_LOG4(pst_vap->uc_vap_id, OAM_SF_ANY,
                         "{hmac_rx_vip_info::send to wlan smac: %x:%x, dmac: %x:%x]",
...
OAL_STATIC oal_void hmac_rx_process_data_tcp_ack_opt_etc(hmac_vap_stru *hmac_vap, oal_netbuf_head_stru *netbuf_header)
{
    if (hmac_vap->st_vap_base_info.en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        hmac_rx_process_data_ap_tcp_ack_opt_etc(hmac_vap, netbuf_header);
    } else {
...
oal_uint32 hmac_rx_process_data_event(frw_event_mem_stru *pst_event_mem)
{
    ...
    hmac_rx_process_data_tcp_ack_opt_etc(pst_hmac_vap, &st_netbuf_header);
