...
OAL_STATIC OAL_INLINE oal_void hmac_mgmt_set_txbf_cap_para(oal_uint8 *puc_asoc_rsp)
{
...
oal_uint32 hmac_mgmt_encap_asoc_rsp_ap_etc(mac_vap_stru *pst_mac_ap, const unsigned char *puc_sta_addr,
    oal_uint16 us_assoc_id, mac_status_code_enum_uint16 en_status_code, oal_uint8 *puc_asoc_rsp, oal_uint16 us_type)
{
    ...
    if (OAL_ANY_NULL_PTR3(pst_mac_ap, puc_sta_addr, puc_asoc_rsp)) {
        OAM_ERROR_LOG3(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap_etc::pst_mac_ap[0x%x], puc_data[0x%x], \
    ...
    if (pst_mac_device == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_ap->uc_vap_id, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap_etc::pst_mac_device NULL}");
    }
    ...
    if (pst_hmac_user == OAL_PTR_NULL) {
        OAM_WARNING_LOG1(0, OAM_SF_ASSOC, "{hmac_mgmt_encap_asoc_rsp_ap_etc::pst_hmac_user(idx=%d) NULL}", us_assoc_id);
        return us_asoc_rsp_len;
    }
    ...
    puc_asoc_rsp += uc_ie_len;
#endif
    mac_set_hisi_cap_vendor_ie((oal_void *)pst_mac_ap, puc_asoc_rsp, &uc_ie_len);
    puc_asoc_rsp += uc_ie_len;
...
#ifdef _PRE_WLAN_FEATURE_HIEX
    puc_asoc_rsp += hmac_hiex_encap_ie((oal_void *)pst_mac_ap, OAL_PTR_NULL, puc_asoc_rsp);
#endif
    ...
    if (pst_hmac_user->en_user_vendor_novht_capable == OAL_TRUE) {
        mac_set_vendor_novht_ie((oal_void *)pst_mac_ap, puc_asoc_rsp, &uc_ie_len, OAL_FALSE);
        puc_asoc_rsp += uc_ie_len;
    }
...
OAL_STATIC oal_uint16 hmac_get_auth_rsp_timout(mac_vap_stru *pst_mac_vap)
{
    oal_uint16 us_auth_rsp_timeout;
                                   ...
                                   oal_netbuf_stru *pst_auth_req, oal_uint8 *puc_chtxt, oal_uint8 uc_chtxt_len)
{
    ...
    OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_AUTH,
                     "{hmac_encap_auth_rsp_etc::ul_auth_proc_rst:%d,us_auth_rsp_timeout:%d}",
