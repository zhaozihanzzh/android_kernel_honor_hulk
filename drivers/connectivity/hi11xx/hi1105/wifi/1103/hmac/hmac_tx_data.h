...
#ifdef _PRE_WLAN_FEATURE_HIEX
    if ((MAC_USER_HIMIT_ENABLED(&pst_hmac_user->st_user_base_info) == OAL_TRUE) &&
         MAC_GET_CB_IS_GAME_MARKED_FRAME(pst_tx_ctl) &&
        ((MAC_GET_CB_ACK_POLACY(pst_tx_ctl) == WLAN_TX_NORMAL_ACK) ||
        (MAC_GET_CB_ACK_POLACY(pst_tx_ctl) == WLAN_TX_BLOCK_ACK))) {
...
OAL_STATIC OAL_INLINE oal_void hmac_need_del_ba_check(hmac_vap_stru *pst_hmac_vap,
    hmac_user_stru *pst_hmac_user, oal_uint8 uc_tidno)
{
#ifdef _PRE_WLAN_FEATURE_AMPDU_TX_HW
