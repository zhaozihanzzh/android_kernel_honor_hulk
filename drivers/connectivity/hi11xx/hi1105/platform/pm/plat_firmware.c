...
/* 根据命令解析 */
static FIRMWARE_NUMBER_STRUCT number_type_etc[] =
{
...
int32 recv_device_mem_etc(OS_KERNEL_FILE_STRU *fp, uint8 *pucDataBuf, int32 len)
{
    ...
    if (len <= lenbuf) {
        oal_kernel_file_write_etc(fp, pucDataBuf, len);
    }
...
/*
 * 函 数 名  : download_dcxo_cali_data_etc
 * 功能描述  : 使用files命令加载dcxo的校准数据
 * 返 回 值  : -1表示失败，0表示成功
 */
static int32 download_dcxo_cali_data_etc(uint8 *key, uint8 *value)
{
    int32 l_ret;
    uint32 len;
    uint32 value_len;
    ...
    dcxo_buffer_addr = (uint8 *)get_dcxo_data_buf_addr_etc();
    ...
    /* 发送dcxo校准数据 */
    l_ret = msg_send_and_recv_except_etc(dcxo_buffer_addr, DCXO_CALI_DATA_BUF_LEN, MSG_FROM_DEV_FILES_OK);
    if (l_ret < 0) {
        PS_PRINT_ERR("send bfgx cali data fail\n");
        return -EFAIL;
    }
...
int32 sdio_read_mem_etc(uint8 *key, uint8 *value, bool is_wifi)
{
    ...
    while (size > 0) {
        ...
        l_ret = recv_device_mem_etc(fp, pucDataBuf, readlen);
        if (l_ret > 0) {
            size -= l_ret;
        } else {
...
static int32 exec_number_cali_dcxo_data(uint8 *key, uint8 *value)
{
    ...
    /* 发送TCXO 的校准参数到devcie */
    l_ret = download_dcxo_cali_data_etc(FILES_CMD_KEYWORD, value);
    if (l_ret < 0) {
...
/*
 * 函 数 名  : exec_number_rmem_cmd_etc
 * 功能描述  : 读device的内存
 * 返 回 值  : 0表示成功，其他值表示失败
 */
static int32 exec_number_rmem_cmd(uint8 *key, uint8 *value)
{
    return sdio_read_mem_etc(key, value, true);
}
...
int32 exec_number_type_cmd_etc(uint8 *Key, uint8 *Value)
{
    ...
    for(loop = 0; loop < OAL_ARRAY_SIZE(number_type_etc); loop++) {
        if (!OS_STR_CMP(Key, number_type_etc[loop].key)) {
            l_ret = (number_type_etc[loop].cmd_exec)(Key, Value);
            break;
        }
    }
...
static int32 firmware_parse_cur_cmd(uint32 index, uint32 pos)
{
    ...
    /* 获取配置版本号 */
    if (!OS_MEM_CMP(cfg_info_etc.apst_cmd[index][pos].cmd_name,
                    VER_CMD_KEYWORD,
                    OS_STR_LEN(VER_CMD_KEYWORD))) {
        cmd_para_len = OS_STR_LEN(cfg_info_etc.apst_cmd[index][pos].cmd_para);
        ret = memcpy_s(cfg_info_etc.auc_CfgVersion, sizeof(cfg_info_etc.auc_CfgVersion),
                       cfg_info_etc.apst_cmd[index][pos].cmd_para, cmd_para_len);
        if (ret != EOK) {
    ...
    } else if (!OS_MEM_CMP(cfg_info_etc.apst_cmd[index][pos].cmd_name,
                           CALI_DCXO_DATA_CMD_KEYWORD,
                           OS_STR_LEN(CALI_DCXO_DATA_CMD_KEYWORD))) {
        if (!test_bit(DCXO_PARA_READ_OK, &dcxo_info_etc.nv_init_flag)) {
            ret = read_dcxo_cali_data_etc();
            if (ret < 0) {
                PS_PRINT_ERR("read dcxo para from nv failed !\n");
                return -EFAIL;
            }
            set_bit(DCXO_PARA_READ_OK, &dcxo_info_etc.nv_init_flag);
        } else {
...
int32 firmware_cfg_init_etc(void)
{
    ...
    l_ret = dcxo_data_buf_malloc_etc();
    if (l_ret < 0) {
