...
oal_int32 hcc_bus_ip_pm_ctrl(hcc_bus *pst_bus, oal_uint32 enable)
{
    oal_int32 ret;
    oal_int32 cmd_len;
    oal_netbuf_stru *pst_netbuf = NULL;
    ...
    ret = memcpy_s((oal_void *)oal_netbuf_data(pst_netbuf), cmd_len, &enable, OAL_SIZEOF(enable));
    if (ret != EOK) {
        oal_netbuf_free(pst_netbuf);
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "memcpy_s error, destlen=%d, srclen=%u\n ",
            cmd_len, (oal_uint32)OAL_SIZEOF(enable));
        return -OAL_EFAIL;
    }
...
oal_int32 hi110x_hcc_ip_pm_ctrl(oal_uint32 enable)
{
...
irqreturn_t hcc_bus_wlan_gpio_irq(oal_int32 irq, oal_void *dev_id)
#else
...
static hcc_bus_dev *s_pst_bus_dev = NULL;

static oal_int32 hitv_wlan_wakeup_gpio_irq(unsigned int param)
{
    printk("%s\n", __FUNCTION__);
    hitv_gpio_clear_irq(get_hi110x_board_info_etc()->wlan_wakeup_host);
    hcc_bus_wlan_gpio_irq((oal_int32)(get_hi110x_board_info_etc()->wlan_irq), s_pst_bus_dev);
    return 0;
}
#endif
/*
...
#ifdef _PRE_HI_DRV_GPIO
    s_pst_bus_dev = pst_bus_dev;
    ret = hitv_gpio_request_irq(pst_board->wlan_wakeup_host, hitv_wlan_wakeup_gpio_irq, HI_UNF_GPIO_INTTYPE_UP, 0);
#else
...
oal_void oal_wlan_gpio_intr_enable_etc(hcc_bus_dev *pst_bus_dev, oal_uint32 ul_en)
{
    ...
    if (ul_en) {
...
#ifdef _PRE_HI_DRV_GPIO
            hitv_gpio_enable_irq(get_hi110x_board_info_etc()->wlan_wakeup_host);
#else
        ...
        if (!pst_bus_dev->ul_irq_stat) {
#ifdef _PRE_HI_DRV_GPIO
            hitv_gpio_disable_irq(get_hi110x_board_info_etc()->wlan_wakeup_host);
#else
