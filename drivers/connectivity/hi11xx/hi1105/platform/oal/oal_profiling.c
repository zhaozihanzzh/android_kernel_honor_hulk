...
/* 保存RR性能host打点 */
oal_perform_host_timestamp_t g_host_rr_perform_timestamp;
EXPORT_SYMBOL(g_host_rr_perform_timestamp);
#endif
...
#ifdef _PRE_WLAN_RR_PERFORMENCE_DEBUG
oal_void oal_profiling_rr_perform_timestamp_init(oal_void)
{
    memset_s(&g_host_rr_perform_timestamp, OAL_SIZEOF(oal_perform_host_timestamp_t),
             0, OAL_SIZEOF(oal_perform_host_timestamp_t));
}
EXPORT_SYMBOL(oal_profiling_rr_perform_timestamp_init);
#endif
