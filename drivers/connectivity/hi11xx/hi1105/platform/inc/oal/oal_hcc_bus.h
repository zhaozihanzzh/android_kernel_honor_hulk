...
typedef struct _hcc_bus_opt_ops {
    ...
    oal_void (*wlan_pm_vote)(hcc_bus *hi_bus, oal_uint8 uc_allow);
#endif
} hcc_bus_opt_ops;
...
extern irqreturn_t hcc_bus_wlan_gpio_irq(oal_int32 irq, oal_void *dev_id);
#endif
...
extern oal_int32 hi110x_hcc_ip_pm_ctrl(oal_uint32 enable);
#ifdef _PRE_CONFIG_S4_KIRIN_BUGFIX
...
#ifdef CONFIG_KIRIN_PCIE_L1SS_IDLE_SLEEP
OAL_STATIC OAL_INLINE oal_void hcc_bus_wlan_pm_vote(hcc_bus *hi_bus, oal_uint8 uc_allow)
{
    if (OAL_WARN_ON(hi_bus == NULL)) {
        return;
    }
