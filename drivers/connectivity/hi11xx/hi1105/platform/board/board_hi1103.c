...
static int32 hi1103_board_gpio_request_out(int32 physical_gpio, const char *gpio_name)
{
    ...
    gpio_direction_output(physical_gpio);
#endif
    return BOARD_SUCC;
}
#endif
...
int32 hitv_get_board_power_gpio(struct platform_device *pdev)
{
    ...
    board_info_etc.power_on_enable = POWER_ON_ENABLE_GPIO;
    board_info_etc.wlan_power_on_enable = WLAN_POWER_ON_ENABLE_GPIO;
    hitv_gpio_request(board_info_etc.power_on_enable, 0);
    hitv_gpio_direction_output(board_info_etc.power_on_enable, 1);
    hitv_gpio_request(board_info_etc.wlan_power_on_enable, 0);
    hitv_gpio_direction_output(board_info_etc.wlan_power_on_enable, 1);

    board_info_etc.bfgn_power_on_enable = BFGN_POWER_ON_ENABLE_GPIO;

    return BOARD_SUCC;
}

int32 hitv_board_wakeup_gpio_init_etc(struct platform_device *pdev)
{
    board_info_etc.wlan_wakeup_host = WLAN_WAKEUP_HOST_GPIO;
    hitv_gpio_request(board_info_etc.wlan_wakeup_host, 1);
    board_info_etc.bfgn_wakeup_host = BFGN_WAKEUP_HOST_GPIO;
    hitv_gpio_request(board_info_etc.bfgn_wakeup_host, 1);

    board_info_etc.host_wakeup_wlan = HOST_WAKEUP_WLAN_GPIO;
    hitv_gpio_request(board_info_etc.host_wakeup_wlan, 0);
    hitv_gpio_direction_output(board_info_etc.host_wakeup_wlan, 0);

#ifdef _PRE_PRODUCT_HI3751_PLATO
    board_info_etc.host_wakeup_bfg = HOST_WAKEUP_BFGN_GPIO_HEGEL;
    hitv_gpio_request(board_info_etc.host_wakeup_bfg, GPIO_LOWLEVEL); /* Req PCIE_rst GPIO */
    hitv_gpio_direction_output(board_info_etc.host_wakeup_bfg, 0);
    PS_PRINT_INFO("bfg plato wakeup gpio 20-6 \n");
#else
    ...
    if (PdmGetHwVer() == WIFI_PRODUCT_VER_OSCA_550 || PdmGetHwVer() == WIFI_PRODUCT_VER_OSCA_550A) {
        board_info_etc.host_wakeup_bfg = HOST_WAKEUP_BFGN_GPIO;
        hitv_gpio_request(board_info_etc.host_wakeup_bfg, GPIO_LOWLEVEL); /* Req PCIE_rst GPIO */
...
int32 hi1103_get_board_power_gpio(struct platform_device *pdev)
{
    ...
    board_info_etc.ssi_gpio_clk = (480 + 27);  // GPOA(480)+pad_gpio27;
    board_info_etc.ssi_gpio_data = (480 + 26); // GPOA(480)+pad_gpio26;
    ...
    board_info_etc.power_on_enable = desc_to_gpio(pdesc_power_on_enable);
    PS_PRINT_INFO("from gpiod CRS[0] power_on_enable:%d, gpio- ssi clk gpio:%d, ssi data gpio:%d\n",
                  board_info_etc.power_on_enable, board_info_etc.ssi_gpio_clk, board_info_etc.ssi_gpio_data);
    ...
    board_info_etc.bfgn_power_on_enable = desc_to_gpio(pdesc_bfgn_power_on_enable);
    PS_PRINT_INFO("from gpiod CRS[2] bfgn_power_on_enable:%d\n", board_info_etc.bfgn_power_on_enable);
    ...
    board_info_etc.wlan_power_on_enable = desc_to_gpio(pdesc_wlan_power_on_enable);
    PS_PRINT_INFO("from gpiod CRS[1] wlan_power_on_enable:%d\n", board_info_etc.wlan_power_on_enable);
#endif
    return BOARD_SUCC;
...
void hi1103_free_board_power_gpio_etc(struct platform_device *pdev)
{
    devm_gpiod_put(&pdev->dev, gpio_to_desc(board_info_etc.power_on_enable));
    devm_gpiod_put(&pdev->dev, gpio_to_desc(board_info_etc.bfgn_power_on_enable));

#ifndef BFGX_UART_DOWNLOAD_SUPPORT
    devm_gpiod_put(&pdev->dev, gpio_to_desc(board_info_etc.wlan_power_on_enable));
#endif
}

int32 hi1103_board_wakeup_gpio_init_etc(struct platform_device *pdev)
{
    ...
    board_info_etc.bfgn_wakeup_host = desc_to_gpio(pdesc_bfgn_wakeup_host);
    ...
    board_info_etc.wlan_wakeup_host = desc_to_gpio(pdesc_wlan_wakeup_host);
    ...
    board_info_etc.host_wakeup_wlan = desc_to_gpio(pdesc_host_wakeup_wlan);
...
void hi1103_free_board_wakeup_gpio_etc(struct platform_device *pdev)
{
    devm_gpiod_put(&pdev->dev, gpio_to_desc(board_info_etc.bfgn_wakeup_host));

#ifndef BFGX_UART_DOWNLOAD_SUPPORT
    devm_gpiod_put(&pdev->dev, gpio_to_desc(board_info_etc.wlan_wakeup_host));
    devm_gpiod_put(&pdev->dev, gpio_to_desc(board_info_etc.host_wakeup_wlan));
#endif
}
...
int32 hi1103_board_wakeup_gpio_init_etc(struct platform_device *pdev)
{
    ...
    hi1103_board_gpio_request_out(physical_gpio, PROC_NAME_GPIO_HOST_WAKEUP_BFG);
...
int32 hi1103_bfgx_dev_power_on(void)
{
    ...
    if (wlan_is_shutdown_etc()) {
        ret = firmware_download_function_etc(BFGX_CFG);
...
{
    board_info_etc.clk_32k_name = "clk_pmu32kb";
...
{
    board_info_etc.uart_port = "/dev/ttySC_SPI0";
...
int32 hi1103_check_pmu_clk_share(void)
{
    ...
    PS_PRINT_INFO("hi110x PMU clk request is %s\n", (board_info_etc.pmu_clk_share_enable ? "enable" : "disable"));
    return BOARD_SUCC;
}
...
int32 hi1103_check_wlan_wakeup_host(void)
{
    ...
    for (i = 0; i < ul_test_times; i++) {
        ...
        if (oal_gpio_get_value(board_info_etc.wlan_wakeup_host) == 0) {
...
void hi1103_suspend_power_gpio_etc(void)
{
    int physical_gpio;

    physical_gpio = board_info_etc.power_on_enable;
    if (physical_gpio == 0) {
    ...
    physical_gpio = board_info_etc.bfgn_power_on_enable;
    if (physical_gpio == 0) {
...
#ifndef BFGX_UART_DOWNLOAD_SUPPORT
    physical_gpio = board_info_etc.wlan_power_on_enable;
    if (physical_gpio == 0) {
...
void hi1103_suspend_wakeup_gpio_etc(void)
{
    int physical_gpio;

    physical_gpio = board_info_etc.bfgn_wakeup_host;
    if (physical_gpio == 0) {
    ...
    gpio_free(physical_gpio);
#ifndef BFGX_UART_DOWNLOAD_SUPPORT
    physical_gpio = board_info_etc.wlan_wakeup_host;
    if (physical_gpio == 0) {
    ...
    physical_gpio = board_info_etc.host_wakeup_wlan;
    if (physical_gpio == 0) {
...
void hi1103_suspend_gpio_etc(void)
{
    hi1103_free_board_wifi_tas_gpio_etc();
    hi1103_suspend_wakeup_gpio_etc();
    hi1103_suspend_power_gpio_etc();
}
...
int32 hi1103_resume_gpio_reuqest_in(int32 physical_gpio, const char *gpio_name)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO,
            " physical_gpio is %d, gpio_name is %s.\n", physical_gpio, gpio_name);
...
void hi1103_resume_power_gpio_etc(void)
{
    int32 ret;
    int32 physical_gpio = 0;
...
void hi1103_resume_wakeup_gpio_etc(void)
{
    int32 ret;
    int32 physical_gpio = 0;
...
#ifdef _PRE_H2D_GPIO_WKUP
    physical_gpio = board_info_etc.host_wakeup_bfg;
...
void hi1103_resume_gpio_etc(void)
{
    hi1103_resume_power_gpio_etc();
    hi1103_resume_wakeup_gpio_etc();
    hi1103_board_wifi_tas_gpio_init_etc();
}
#endif
