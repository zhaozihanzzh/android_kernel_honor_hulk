...
/* 函数定义 */
int32 hi1102_get_board_power_gpio(struct platform_device *pdev)
{
    int32 ret;
...
void hi1102_free_board_power_gpio_etc(struct platform_device *pdev)
{
    gpio_free(board_info_etc.power_on_enable);
}

int32 hi1102_board_wakeup_gpio_init_etc(struct platform_device *pdev)
{
    int32 ret;
    int32 physical_gpio = 0;
...
void hi1102_free_board_wakeup_gpio_etc(struct platform_device *pdev)
{
    gpio_free(board_info_etc.wlan_wakeup_host);
