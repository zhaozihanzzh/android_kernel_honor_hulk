...
{
    int ret = 0;
    bool fSnclk2 = (id == 2 || id == 3 || id == 4); // 2 , 3, 4 for different sensor
    struct device *dev;
...
static int hisi_sensor_power_config(sensor_t *s_ctrl, struct sensor_power_setting *power_setting, int on_off)
{
	int rc = 0;
...
int hw_sensor_power_down(sensor_t *s_ctrl)
{
	...
	for (index = 0; index < power_setting_array->size; index++) {
		power_setting = &power_setting_array->power_setting[index];
		rc = hisi_sensor_power_config(s_ctrl, power_setting, POWER_OFF);
	}
	...
	sensor_t *sensor)
{
	...
	rc = of_property_read_u32(of_node, "huawei,is_fpga",
		&is_fpga); /*lint !e64 */
	cam_info("%s huawei,is_fpga: %d, rc %d\n", __func__,
		is_fpga, rc);
	if (rc < 0) {
		cam_err("%s failed %d\n", __func__, __LINE__);
		goto fail;
	...
	cam_info("%s huawei,pd_valid %d, rc %d\n", __func__,
	...
	cam_info("%s huawei,reset_valid %d, rc %d\n", __func__,
	...
	cam_info("%s huawei,vcmpd_valid %d, rc %d\n", __func__,
	...
	rc = of_property_read_u32(of_node, "huawei,mclk",
		&sensor_info->mclk);
	cam_info("%s huawei,mclk 0x%x, rc %d\n", __func__,
		sensor_info->mclk, rc);
	if (rc < 0) {
		cam_err("%s failed %d\n", __func__, __LINE__);
		goto fail;
	...
	if (!rc && (sensor_info->phyinfo_valid == 1 || sensor_info->phyinfo_valid == 2)) { // 2 for phy sensor
