...
#define MICROSECOND_PER_SECOND 1000000
	...
	if (timestamp_h == NULL || timestamp_l == NULL) {
		cam_err("%s timestamp_h or timestamp_l is null", __FUNCTION__);
		return;
	}
	...
	*timestamp_l = (u32)(fw_micro_second & 0xFFFFFFFF);
	...
	cam_debug("%s h:0x%x l:0x%x", __FUNCTION__, *timestamp_h, *timestamp_l);
}
...
static int hisp210_unmap_a7isp_addr(void *cfg)
{
	...
	cam_info("func %s: a7 %x, size %x",
		__FUNCTION__, pcfg->param.moduleAddr, pcfg->param.size);
...
static int hisp210_init_r8isp_memory_pool(void *cfg)
{
	...
	cam_info("%s: pool cfg vaddr=0x%pK, iova=0x%x, size=0x%x", __FUNCTION__,
		...
		pcfg->param.iova,
		pcfg->param.size);
	cam_info("%s: type=%d, prot=0x%x sec=0x%x", __FUNCTION__,
		pcfg->param.type,
		pcfg->param.prot,
...
static int hisp210_mem_pool_pre_init(void)
{
	...
	for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
		...
		cam_info("%s  ipool %d prot 0x%x", __FUNCTION__, ipool, prot);
...
static int hisp210_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		...
		if (!remote_processor_up) {
			...
			cam_info("%s power on the hisp210", __FUNCTION__);
	...
	case HISP_CONFIG_POWER_OFF:
		mutex_lock(&hisp_power_lock_mutex);
		if (remote_processor_up) {
			cam_notice("%s power off the hisp210", __FUNCTION__);
			rc = hisp210_power_off(i);
		}
		mutex_unlock(&hisp_power_lock_mutex);
		break;
	...
	case HISP_CONFIG_ISP_ULTRALOW:
		cam_info("%s HISP_CONFIG_ISP_ULTRALOW", __FUNCTION__);
	...
	case HISP_CONFIG_R8_ULTRALOW:
		cam_info("%s HISP_CONFIG_R8_CLK", __FUNCTION__);
...
static int hisp210_open(hisp_intf_t *i)
{
	cam_info("%s hisp210 device open", __FUNCTION__);
	mutex_lock(&hisp_power_lock_mutex);
	g_hisp_ref++;
	mutex_unlock(&hisp_power_lock_mutex);
	return 0;
...
{
	int rc = 0;
	cam_info("%s hisp210 device close", __FUNCTION__);
	mutex_lock(&hisp_power_lock_mutex);
...
#pragma GCC visibility push(default)
	hisi_serv->ept =
		rpmsg_create_ept(hisi_serv->rpdev,
			hisp210_rpmsg_ept_cb, hisi_serv, chinfo);
