...
#endif
{
	...
	/* SMMU pgtable init */
	if (!ion_sgtable_init && secsg_heap->pgtable_phys) {
		if (!secsg_heap->TA_init ||
		    secsg_heap->heap_attr != HEAP_PROTECT) {
		...
		ret = ion_sg_pgtable_init(secsg_heap->session,
					  secsg_heap->pgtable_phys,
					  secsg_heap->pgtable_size);
		if (ret) {
			pr_err("%s:ion_sg_pgtable_init fail!\n", __func__);
			goto out;
		}
	}
