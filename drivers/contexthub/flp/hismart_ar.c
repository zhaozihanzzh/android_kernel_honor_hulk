	...
	context_iomcu_cfg_t *cur, context_iomcu_cfg_t *old_set)
{
	...
	(void)memset_s((void *)cur, sizeof(context_iomcu_cfg_t), 0, sizeof(context_iomcu_cfg_t));
...
static int ar_config_to_iomcu(ar_port_t *ar_port)
{
	...
	pr_info("hismart:[%s]:line[%d] interval[%u] size[%d]\n", __func__, __LINE__, devinfo->cfg.report_interval, devinfo->cfg_sz);
CFG_IOMCU_FIN:
	return ret;
}
/*lint -e715*/
/*
 * 将AR配置转化为IPC消息发送给contexthub
 */
