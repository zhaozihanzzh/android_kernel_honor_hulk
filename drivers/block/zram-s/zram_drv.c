 ...
 */
static size_t huge_class_size = (PAGE_SIZE / 4) * 3;
...
static inline struct bio_vec *zram_bio_first_bvec_all(struct bio *bio)
{
	WARN_ON_ONCE(bio_flagged(bio, BIO_CLONED));
	return bio->bi_io_vec;
}

static inline struct page *zram_bio_first_page_all(struct bio *bio)
{
	return zram_bio_first_bvec_all(bio)->bv_page;
}

static void zram_page_end_io(struct bio *bio)
{
	struct page *page = zram_bio_first_page_all(bio);
...
#ifdef CONFIG_ZRAM_S_MEMORY_TRACKING
			...
			int offset, bool is_write, struct bio *bio)
{
	...
	generic_start_io_acct(is_write, bvec->bv_len >> SECTOR_SHIFT,
			      &zram->disk->part0);
	...
	generic_end_io_acct(is_write, &zram->disk->part0, start_time);
