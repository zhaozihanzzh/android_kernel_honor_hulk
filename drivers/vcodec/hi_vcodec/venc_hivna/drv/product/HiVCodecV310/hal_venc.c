...
{
	pVeduReg->SMMU_ERR_RDADDR_NS                           = g_smmu_err_mem.RdAddr & 0xFFFFFFFF;//config alloc phyaddr,in order system don't dump
	pVeduReg->SMMU_ERR_ADDR_MSB_NS.bits.err_rd_addr_msb_ns = (g_smmu_err_mem.RdAddr >> 32) & 0xFFFF;
	pVeduReg->SMMU_ERR_WRADDR_NS                           = g_smmu_err_mem.WrAddr & 0xFFFFFFFF;
	pVeduReg->SMMU_ERR_ADDR_MSB_NS.bits.err_wr_addr_msb_ns = (g_smmu_err_mem.WrAddr >> 32) & 0xFFFF;
}
...
HI_VOID VeduHal_CfgReg_SMMUSet(HI_VOID)
{
	...
	pAllReg->SMMU_CB_TTBR_MSB.bits.cb_ttbr_msb = (phy_pgd_base >> 32) & 0xFFFF;
