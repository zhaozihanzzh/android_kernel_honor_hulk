...
err_wr_smmu_exit:
	DRV_MEM_KFree(&MEM_SMMU_WR_ADDR);
err_rd_smmu_exit:
...
HI_S32 DRV_MEM_KAlloc(const HI_CHAR* bufName, const HI_CHAR *zone_name, MEM_BUFFER_S *psMBuf)
{
	...
	psMBuf->u64StartPhyAddr = __pa((uintptr_t)virt_addr);/*lint !e648 !e834 !e712*/
			...
			long (*func)(struct file *file, HI_U32 cmd, unsigned long uarg))
{
	...
	if (_IOC_DIR(cmd) & _IOC_WRITE) {
		if (copy_from_user(parg, (void __user*)(uintptr_t)arg, _IOC_SIZE(cmd))) {/*lint !e747 */
	...
	/* call driver */
	err = func(file, cmd, (long)(uintptr_t)(parg)); /*lint !e732 !e712*/
	if (err == -ENOIOCTLCMD)
		err = -EINVAL;
	if (err < 0)
		goto out;
	...
	case (_IOC_WRITE | _IOC_READ):
		if (copy_to_user((void __user *)(uintptr_t)arg, parg, _IOC_SIZE(cmd))) {/*lint !e747 */
