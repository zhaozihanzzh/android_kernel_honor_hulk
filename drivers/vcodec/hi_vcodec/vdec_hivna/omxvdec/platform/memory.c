...
HI_S32 VDEC_MEM_GetMapInfo(HI_S32 share_fd, MEM_BUFFER_S * psMBuf)
{
	...
	iova_addr = hisi_iommu_map_dmabuf(vdec_entry.device, dmabuf, 0, &phy_size);
	if (!iova_addr) {
...
HI_S32 VDEC_MEM_PutMapInfo(MEM_BUFFER_S * psMBuf)
{
	...
	ret = hisi_iommu_unmap_dmabuf(vdec_entry.device, dmabuf,  psMBuf->startPhyAddr);
	if (ret != 0) {
		dprint(PRN_ERROR, "%s: hisi iommu unmap dmabuf failed \n", __func__);
		ret = HI_FAILURE;
		goto exit;
