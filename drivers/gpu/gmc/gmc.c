 ...
 * Authors: Huawei Technologies Co.
 * Create: 2019-2-24
 *
	...
	size_t len, loff_t *offp)
{
	...
	ret = snprintf_s(buf + out_offset, out_count, GMC_STORAGE_COUNT,
		"ComprSize:    %10llu kB\n", (unsigned long long)atomic64_read(
		&device->storage->stat.compr_data_size) / 1024);
	if (ret == -1) {
		kfree(buf);
		pr_err("GMC storage stat read snprintf_s failed\n");
		return -EINVAL;
	}
	out_offset += ret;
	/* Per page is 4KB */
	ret = snprintf_s(buf + out_offset, out_count - out_offset, GMC_STORAGE_COUNT,
		"StoredSize:   %10llu kB\n", (unsigned long long)atomic64_read(
...
int gmc_register_device(struct gmc_ops *gmc_operations, struct gmc_device *device)
{
	...
	for (i = 0; i < array_size; i++) {
		struct proc_dir_entry *dir_entry = proc_create_data(files[i].name, S_IWUSR | S_IRUSR | S_IWGRP,
