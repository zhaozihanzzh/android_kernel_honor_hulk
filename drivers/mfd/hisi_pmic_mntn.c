			...
			for (i = 0; i < pmic_mntn->data_width; i++) {/*lint !e574*/
				if ((pmic_mntn->record_event_buff[index] & BIT(i)) \
					...
					if (!dsm_client_ocuppy(power_dsm_get_dclient(POWER_DSM_PMU_OCP))) {
						...
						pr_err("pmic %s happened, please pay attention! vbatt:%d, batt_temp:%d, capacity:%d\n",
							pmic_mntn->record_exch_desc[index].event_bit_name[i], vbatt, batt_temp, capacity);
...
MODULE_AUTHOR("Huan Wang <wanghuan4@huawei.com>");
