...
static void ivp_release_iores(struct platform_device *plat_devp)
{
    ...
    if (NULL != pdev->io_res.noc_ivp_base_addr) {
...
static int ivp_init_reg_res(struct platform_device *pdev, struct ivp_device *ivp_devp)
{
    ...
    ivp_devp->io_res.noc_ivp_base_addr = devm_ioremap(&pdev->dev, mem_res->start, resource_size(mem_res)); /*lint -save -e747*/
    if (NULL == ivp_devp->io_res.noc_ivp_base_addr) {
