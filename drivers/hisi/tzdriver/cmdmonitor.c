...
static void cmd_monitor_tick(void)
{
	...
	list_for_each_entry_safe(monitor, tmp, &cmd_monitor_list, list) {
		...
		timedif = 1000 * nowtime.tv_sec -
			1000 * monitor->sendtime.tv_sec +
			nowtime.tv_nsec / 1000000 -
			monitor->sendtime.tv_nsec / 1000000;
		...
		if (timedif > 25 * 1000 && !monitor->isReported) {
			...
			/* print tee stask */
			tloge("[cmd_monitor_tick] pid=%d,pname=%s,tid=%d,tname=%s,lastcmdid=%d,timedif=%lld ms and report\n",
				monitor->pid, monitor->pname, monitor->tid,
...
void cmd_monitor_logend(tc_ns_smc_cmd *cmd)
{
	...
	list_for_each_entry(monitor, &cmd_monitor_list, list) {
		if (monitor->pid == pid && monitor->tid == tid &&
			monitor->returned == false) {
			struct timespec nowtime = current_kernel_time();
			long timedif = 1000000 * nowtime.tv_sec -
				1000000 * monitor->sendtime.tv_sec +
				nowtime.tv_nsec / 1000 -
				monitor->sendtime.tv_nsec / 1000;
