...
static void walk_callback_list(struct tc_ns_callback *callback_func_t,
...
static void tc_notify_timer_fn(struct notify_data_entry *notify_data_entry)
{
	...
	walk_callback_list(callback_func_t, tc_notify_data_timer);
		...
		if (temp_dev_file->dev_file_id == tc_notify_data_timer->dev_file_id) {
			...
			temp_svc =
				tc_find_service_in_dev(temp_dev_file,
					tc_notify_data_timer->uuid, false);
...
/*lint -esym(429,*)*/
int tc_ns_register_service_call_back_func(char *uuid, void *func,
	void *private_data)
{
...
int TC_NS_RegisterServiceCallbackFunc(char *uuid, void *func,
	void *private_data)
{
	return tc_ns_register_service_call_back_func(uuid, func, private_data);
}
...
int tc_ns_tst_cmd(tc_ns_dev_file *dev_id, void *argp)
{
	...
	if (copy_from_user(&cmd_id, (void *)client_context.params[0].value.a_addr,
		sizeof(cmd_id))) {
	...
	case TST_CMD_02:
		...
		if (copy_to_user((void *)client_context.params[1].value.a_addr,
...
int tz_spi_init(struct device *class_dev, struct device_node *np)
{
	...
	ret = devm_request_irq(class_dev, irq, tc_secure_notify, 0,
	...
	if (g_notify_data == NULL) {
		g_notify_data = (struct notify_data_struct *)__get_free_page(
