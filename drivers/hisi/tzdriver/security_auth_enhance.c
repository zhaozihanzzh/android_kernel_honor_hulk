...
static int32_t scrambling_timestamp(void *in, void *out,
	uint32_t data_len, void *key, uint32_t key_len)
{
	uint32_t i;
	...
	struct session_secure_info *secure_info, uint8_t flag)
{
	...
	if (scrambling_timestamp(&time_stamp,
		&in_token_buf[TIMESTAMP_BUFFER_INDEX], TIMESTAMP_LEN_DEFAULT,
		secure_info->scrambling, SCRAMBLING_KEY_LEN)) {
		tloge("descrambling_timestamp failed\n");
		return -EFAULT;
	}
...
int32_t update_timestamp(tc_ns_smc_cmd *cmd)
{
	...
	filter_flag = (cmd->agent_id != 0) || (cmd->ret_val == (int)TEEC_PENDING2_AGENT);
	if (filter_flag)
		return TEEC_SUCCESS;

	phy_addr = (phys_addr_t)(cmd->uuid_h_phys) << 32 | (cmd->uuid_phys);
	if (phy_addr == 0) {
		tloge("uuid phy is NULL!\n");
		return TEEC_ERROR_BAD_PARAMETERS;
	}
	...
	global = (uuid[0] == UUID_PREFIX_MEANS_GLOBAL) ? true : false;
	need_check_flag = (!global) && (cmd->agent_id == 0) &&
		(cmd->ret_val != (int)TEEC_PENDING2_AGENT);
	if (need_check_flag) {
		...
		session = tc_find_session2(cmd->dev_file_id, cmd->context_id, uuid + 1);
		if (session == NULL) {
			tloge("tc_find_session_key find session FAILURE.\n");
			return TEEC_ERROR_GENERIC;
		}
		secure_info = &session->secure_info;
		if (descrambling_timestamp(token_buffer, secure_info, INC) != EOK) {
...
int32_t sync_timestamp(tc_ns_smc_cmd *cmd, uint8_t *token, unsigned char *uuid,
	bool global)
{
	tc_ns_session *session = NULL;
	...
	if (token[SYNC_INDEX] == UN_SYNCED) {
		...
		session = tc_find_session2(cmd->dev_file_id, cmd->context_id, uuid);
		if (session == NULL) {
			tlogd("sync_timestamp find session FAILURE. \n");
			return TEEC_ERROR_GENERIC;
		}
		if (descrambling_timestamp(token, &session->secure_info, DEC) != EOK) {
			put_session_struct(session);
...
int32_t update_chksum(tc_ns_smc_cmd *cmd)
{
	...
	uuid = phys_to_virt((phys_addr_t)(cmd->uuid_h_phys) << 32 | (cmd->uuid_phys));
	if (uuid == NULL) {
		tloge("uuid is NULL, error!\n");
		return TEEC_ERROR_GENERIC;
	}
	...
	check_value = (!global) && (cmd->agent_id == 0) &&
		(cmd->ret_val != (int)TEEC_PENDING2_AGENT);
	if (check_value == true) {
		session = tc_find_session2(cmd->dev_file_id, cmd->context_id, uuid + 1);
		if (session != NULL) {
...
int32_t verify_chksum(tc_ns_smc_cmd *cmd)
{
	...
	/* cmd is invoke command */
	chk_flag = !global && cmd->cmd_id != GLOBAL_CMD_ID_CLOSE_SESSION &&
		cmd->cmd_id != GLOBAL_CMD_ID_KILL_TASK;
	if (chk_flag && (cmd->agent_id == 0)) {
		session = tc_find_session2(cmd->dev_file_id, cmd->context_id, uuid + 1);
		if (session) {
			secure_info = &session->secure_info;
			put_session_struct(session);
