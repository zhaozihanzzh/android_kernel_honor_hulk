...
static int get_hi6422v5_dieid(struct pmu_para *pmu_info, unsigned char *die_id)
{
	int i, ret;

	/*step1 : set Normal Read mode*/
	ret = hisi_subpmu_write(pmu_info, OTP_CTRL1_V5, 0x02);
	ret += hisi_subpmu_write(pmu_info, OTP_CTRL2_V5, 0x00);

	for (i = OTP_START_V5; i < OTP_END_V5; i += 2) {
		ret += hisi_subpmu_write(pmu_info, OTP_CTRL2_V5, i);
		ret += hisi_subpmu_write(pmu_info, OTP_CTRL0_V5, 0x04);
		udelay(45);

		ret += hisi_subpmu_write(pmu_info, OTP_CTRL0_V5, 0);
	...
	ret = hisi_subpmu_write(pmu_info, OTP_CTRL2_V5, 0x00);
	ret += hisi_subpmu_write(pmu_info, OTP_CTRL1_V5, 0x00);
