...
enum HISP_CPE_CLK_TYPE {
    CLK_HIGH_RATE    = 541000000,//541MHZ->540.67+0.5M
    CLK_NOMAL_RATE    = 406000000,//406M->405.50+0.5M
    CLK_LOW_RATE    = 325000000,//325M->324.40+0.5M=>384M
    CLK_ULTRA_LOW_RATE    = 271000000,//271M->270.34+0.5M
    MAX_CPE_CLK
};
...
static irqreturn_t hispcpe_irq_handler(int irq, void *devid)
{
    struct hispcpe_s *dev = NULL;
    unsigned int value = 0;
    U_CMDLST_ACPU_IRQ_REG2 reg2;
    U_CMDLST_ACPU_IRQ_REG2 regp;
    ...
    value = readl((volatile void __iomem *)(dev->reg[CPE_TOP]+SUB_CTRL_SLAM_IRQ_REG1_REG)); //get the mask
    pr_info("[%s] SLAM_IRQ. irq mask value = 0x%x.\n", __func__, value);  

    regp.u32 = readl((volatile void __iomem *)(dev->reg[CPE_TOP]+SUB_CTRL_CMDLST_ACPU_IRQ_REG2_REG)); //get irq state & mask
    ...
    if(reg2.bits.cmdlst_acpu_irq_state_raw != 0)
    {
       ...
       // writel(0x3FFF, (volatile void __iomem *)(dev->reg[CPE_TOP]+SUB_CTRL_SLAM_IRQ_REG0_REG));  //clear irq,should put in cmdlst
        writel(0x2, (volatile void __iomem *)(dev->reg[CPE_TOP]+SUB_CTRL_CMDLST_ACPU_IRQ_REG0_REG));  //clear ch1 ACPU irq
...
{
    U_CMDLST_CHN1 cfg_cmdlst_ch1_irq;
    //U_SLAM_IRQ_REG1 cfg_slam_irq;
    ...
    cfg_cmdlst_ch1_irq.bits.cmdlst_eof_mask_chn1 = 3;  //ch1 mask
...
//lint -restore
static int ipp_onoff_on_fpga(struct hispcpe_s *dev)
{
   unsigned int pctrl_ipp_onoff = 0x0;
   int ret = 0;
   pctrl_ipp_onoff = readl((volatile void __iomem*)(dev->reg[PCTRL] + IPP_PCTRL_PERI_STAT_ADDR));/*lint !e732 */
   ret = pctrl_ipp_onoff & (ISP_PCTRL_PERI_FLAG);
   if (ret == 0) {
       pr_err("[%s]  IPP May be not on board means no ipp logic on board\n", __func__);
   }
   pr_info("[%s]  IPP logic status on board , all_status.%d ipp_status.%d\n", __func__, pctrl_ipp_onoff, ret);
   return ret;
}
