	...
	unsigned int val1, unsigned int val2)
{
	...
	if (val1 != val2) {
		...
		audio_dsm_report_info(AUDIO_CODEC, DSM_CODEC_SSI_READ_ONCE,
			...
			val1, val2);
#endif
		pr_warning("[%s:%d] ssi read once err, val1 is 0x%x, val2 is 0x%x\n",
								 ...
								 unsigned int reg_addr)
{
	...
	if (reg_addr >= priv->regaddr8_begin && reg_addr <= priv->regaddr8_end) {
		ret1 = priv->reg_ops.read8(reg_addr);
		ret = priv->reg_ops.read8(reg_addr);
	} else {
		ret1 = priv->reg_ops.read32(reg_addr);
		ret = priv->reg_ops.read32(reg_addr);
	}

	ssi_check_dsm_report(&priv->cdc_ctrl, ret1, ret);
