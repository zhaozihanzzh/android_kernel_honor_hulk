...
static  void hifi_om_work_smartpa_dft_report_process_error(struct work_struct *work);
static  void hifi_om_work_smartpa_dft_report_f0_error(struct work_struct *work);
static  void hifi_om_work_smartpa_dft_report_resistance_error(struct work_struct *work);
static  void hifi_om_work_smartpa_dft_report_overheat_error(struct work_struct *work);
...
static struct hifi_om_work_info work_info[] = {
	...
	{HIFI_OM_WORK_SMARTPA_PROCESS_ERROR, "hifi_om_work_smartpa_dft_report_process_error",
		hifi_om_work_smartpa_dft_report_process_error, {0}},
	{HIFI_OM_WORK_SMARTPA_F0_ERROR, "hifi_om_work_smartpa_dft_report_f0_error",
		hifi_om_work_smartpa_dft_report_f0_error, {0}},
	{HIFI_OM_WORK_SMARTPA_RESISTANCE_ERROR, "hifi_om_work_smartpa_dft_report_resistant_error",
		hifi_om_work_smartpa_dft_report_resistance_error, {0}},
	{HIFI_OM_WORK_SMARTPA_OVERHEAT_ERROR, "hifi_om_work_smartpa_dft_report_overheat_error",
		hifi_om_work_smartpa_dft_report_overheat_error, {0}},
...
static void hifi_om_work_smartpa_dft_report_process_error(struct work_struct *work)
{
	...
	spin_lock_bh(&work_info[HIFI_OM_WORK_SMARTPA_PROCESS_ERROR].ctl.lock);
	if (!list_empty(&work_info[HIFI_OM_WORK_SMARTPA_PROCESS_ERROR].ctl.list)) {
		om_work = list_entry(
			work_info[HIFI_OM_WORK_SMARTPA_PROCESS_ERROR].ctl.list.next,
			struct hifi_om_work, om_node);/*lint !e826*/
		memcpy(data, om_work->data, om_work->data_len);/*lint !e747*/ /* unsafe_function_ignore: memcpy */
		list_del(&om_work->om_node);
		kzfree(om_work);
	}
	spin_unlock_bh(&work_info[HIFI_OM_WORK_SMARTPA_PROCESS_ERROR].ctl.lock);

	hifi_to_ap_event_count++;
	if (hifi_to_ap_event_count < HIFI_AP_MESG_CNT) {
		smartpa_dft_report_process_error(data);
	}
}
...
static void hifi_om_work_smartpa_dft_report_f0_error(struct work_struct *work)
{
	...
	spin_lock_bh(&work_info[HIFI_OM_WORK_SMARTPA_F0_ERROR].ctl.lock);
	if (!list_empty(&work_info[HIFI_OM_WORK_SMARTPA_F0_ERROR].ctl.list)) {
		om_work = list_entry(
			work_info[HIFI_OM_WORK_SMARTPA_F0_ERROR].ctl.list.next,
			struct hifi_om_work, om_node);/*lint !e826*/
		memcpy(data, om_work->data, om_work->data_len);/*lint !e747*/ /* unsafe_function_ignore: memcpy */
		list_del(&om_work->om_node);
		kzfree(om_work);
	}
	spin_unlock_bh(&work_info[HIFI_OM_WORK_SMARTPA_F0_ERROR].ctl.lock);

	hifi_to_ap_event_count++;
	if (hifi_to_ap_event_count < HIFI_AP_MESG_CNT) {
		smartpa_dft_report_f0_error(data);
	}
}
...
static void hifi_om_work_smartpa_dft_report_resistance_error(struct work_struct *work)
{
	...
	spin_lock_bh(&work_info[HIFI_OM_WORK_SMARTPA_RESISTANCE_ERROR].ctl.lock);
	if (!list_empty(&work_info[HIFI_OM_WORK_SMARTPA_RESISTANCE_ERROR].ctl.list)) {
		om_work = list_entry(
			work_info[HIFI_OM_WORK_SMARTPA_RESISTANCE_ERROR].ctl.list.next,
			struct hifi_om_work, om_node);/*lint !e826*/
		memcpy(data, om_work->data, om_work->data_len);/*lint !e747*/ /* unsafe_function_ignore: memcpy */
		list_del(&om_work->om_node);
		kzfree(om_work);
	}
	spin_unlock_bh(&work_info[HIFI_OM_WORK_SMARTPA_RESISTANCE_ERROR].ctl.lock);

	hifi_to_ap_event_count++;
	if (hifi_to_ap_event_count < HIFI_AP_MESG_CNT) {
		smartpa_dft_report_resistance_error(data);
	}
}
...
static void hifi_om_work_smartpa_dft_report_overheat_error(struct work_struct *work)
{
	...
	spin_lock_bh(&work_info[HIFI_OM_WORK_SMARTPA_OVERHEAT_ERROR].ctl.lock);
	if (!list_empty(&work_info[HIFI_OM_WORK_SMARTPA_OVERHEAT_ERROR].ctl.list)) {
		om_work = list_entry(
			work_info[HIFI_OM_WORK_SMARTPA_OVERHEAT_ERROR].ctl.list.next,
			struct hifi_om_work, om_node);/*lint !e826*/
		data_len = om_work->data_len;
		memcpy(data, om_work->data, om_work->data_len);/*lint !e747*/ /* unsafe_function_ignore: memcpy */
		list_del(&om_work->om_node);
	...
	spin_unlock_bh(&work_info[HIFI_OM_WORK_SMARTPA_OVERHEAT_ERROR].ctl.lock);

	hifi_to_ap_event_count++;
	if (hifi_to_ap_event_count < HIFI_AP_MESG_CNT) {
		smartpa_dft_report_overheat_error(data);
	}
}
