				...
				int flag)
{
	...
	if (flag || (batt_init_level != di->basp_level)) {
		if (get_basp_nondc_volt_dec(&nondc_volt_dec) < 0) {
			coul_core_err("get basp nondc_volt_dec fail ret -1\n");
			return -1;
		}
		if (!nondc_volt_dec)
			return 0;
...
static void resume_polar_calc(struct smartstar_coul_device *di, int temp)
{
	...
	delta_temp = abs(temp - sr_suspend_temp);
...
static int iscd_check_cc_drift(struct smartstar_coul_device *di)
{
	...
	if (!dmd_reported && abn_cc_cnt >= ISCD_DMD_REPORT_TIME_FOR_ABN_RECHARGE) {
		...
		if (snprintf_s(dsm_buff, ISCD_DSM_LOG_SIZE_MAX,
			ISCD_DSM_LOG_SIZE_MAX - 1, "Abnormal cc drift detected\n") < 0)
