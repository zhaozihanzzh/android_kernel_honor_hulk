...
static void hisi_reset_capacity_dec_fifo(struct hisi_bci_device_info *di)
{
	...
	for (i = 0; i < CAPACITY_DEC_WINDOW_LEN; i++) {
		capacity_dec_filter[i] = (unsigned int)curr_capacity;
		capacity_dec_sum += capacity_dec_filter[i];
	}

	bci_info("%s capacity_filter=%d\n", __func__, capacity_dec_filter[0]);

	capacity_dec_init_value = (unsigned int)curr_capacity;
	capacity_dec_cnt = 0;
}
#endif
...
static int get_capacity_decimal(struct hisi_bci_device_info *di)
{
	...
	bci_info("%s times=%d curr_cap_dec=%d rep_cap_dec=%d prev_cap=%d rep_cap=%d capacity_remain =%d\n", __func__ ,\
		 capacity_dec_cnt, curr_capacity, rep_capacity, capacity_prev, capacity_round, capacity_remain);
...
static int hisi_bci_battery_get_scp_status(struct hisi_bci_device_info *di)
{
	...
	if (uvdm_charge_get_super_charging_flag() && di->ac_online)
		return 1;
...
MODULE_AUTHOR("Jack Lu <jack.lujianglin@hisilicon.com>");
