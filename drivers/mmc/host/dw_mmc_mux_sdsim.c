...
void switch_gpio_change(int set_side_status)
{
	...
	if (switch_gpio_number_0 != SWITCH_GPIO_DEFAULT_NUMBER) {
		switch (set_side_status) {
		case SWITCH_GPIO_SD_SIDE:
			/* sd mode: switch gpio should be set to output low power level */
			set_sd_sim_gpio_direction(switch_gpio_number_0, "nanocard_switch_gpio", 1, 0);
			break;
		case SWITCH_GPIO_SIM_SIDE:
			/* sim mode: switch gpio should be set to output high power level */
			set_sd_sim_gpio_direction(switch_gpio_number_0, "nanocard_switch_gpio", 1, 1);
			break;
		default:
			pr_err("%s %s the mode is illegal\n", MUX_SDSIM_LOG_TAG, __func__);
		}
	}
#else
