...
void __iomem *mmc1_sys_ctrl_base;
...
static int dw_mci_hs_get_resource(void)
{
	...
	if (!pctrl_base) {
		...
		if (!np) {
			pr_err("can't find pctrl!\n");
			return -1;
		}

		pctrl_base = of_iomap(np, 0);
		if (!pctrl_base) {
			pr_err("pctrl_base iomap error!\n");
			return -1;
		}
		pr_err("pctrl base iomap success\n");
	}
...
static int dw_mci_hs_parse_dt(struct dw_mci *host)
{
	...
	value = 0;
	if (of_property_read_u32(np, "print-reg", &value)) {
		dev_info(host->dev, "print-reg property not found, using value of 0 as default\n");
		value = 0;
	}
	priv->print_reg = value;
	dev_info(host->dev, "dts print-reg = %u\n", priv->print_reg);
...
void dw_mci_hi3xxx_reset_defalut_regist(struct dw_mci *host,
