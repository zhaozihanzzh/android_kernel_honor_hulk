...
int sec_rproc_boot(struct rproc *rproc)
{
	...
	if(ret != EOK) {
		pr_err("%s: memset_s loaded_table fail ret.%d\n",__func__, ret);
	}
...
#define PART_ISPFW_SIZE 0x00E00000
	...
	if ((NULL != hisi_rproc) && (NULL != hisi_rproc->firmware)) {
		snprintf_s(pathname, sizeof(pathname), sizeof(pathname)-1, "/system/vendor/firmware/%s", hisi_rproc->firmware);
	} else {
		snprintf_s(pathname, sizeof(pathname), sizeof(pathname)-1, "/system/vendor/firmware/isp_fw.elf");
	}
...
static struct hisi_rproc_data *hisi_rproc_data_dtget(struct device *pdev)
{
	...
	if (use_default_isp()) {
		...
		memset(va, 0, share_size);/*lint !e668 */
		iounmap(va);
	}
