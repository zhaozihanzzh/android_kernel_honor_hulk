		...
		int offset, int avail)
{
	...
	printk("[%s] Firmware_version: build_time = %s.\n",
...
int _stat_poll(void __iomem *addr, unsigned int value,
		const void __iomem *stat_addr, unsigned int cond)
{
	unsigned int stat;
	int          timeout;
...
void dump_vivobus_regs(void)
{
	void __iomem *media1_base = NULL;
	void __iomem *pmc_base = NULL;
	void __iomem *pctrl_addr = NULL;
	void __iomem *sctrl_addr = NULL;
	struct rproc_boot_device *dev = &rproc_boot_dev;
	int ret = 0;

	if (dev->isppd_adb_flag == 0)
		return;
	...
	sctrl_addr = get_regaddr_by_pa(SCTRL);
	if (sctrl_addr == NULL) {
		RPROC_ERR("Failed : ioremap sctrl_addr\n");
		return;
	}

	pr_alert("MEDIA1_POWER:PERCLKEN0.0x%x, PERSTAT0.0x%x, CLKDIV9.0x%x\n",
				__raw_readl(media1_base + MEDIA1_PERCLKEN0_ADDR),
				__raw_readl(media1_base + MEDIA1_PERSTAT0_ADDR),
				__raw_readl(media1_base + MEDIA1_CLKDIV9_ADDR));
	pr_alert("PMC:PMC_IDLEREQ.0x%x, PMC_IDLEACK.0x%x, PMC_IDLE.0x%x\n",
				__raw_readl(pmc_base + PMC_NOC_POWER_IDLEREQ_0),
				__raw_readl(pmc_base + PMC_NOC_POWER_IDLEACK_0),
				__raw_readl(pmc_base + PMC_NOC_POWER_IDLE_0));
	pr_alert("SC:SC_DATA11.0x%x\n",
				__raw_readl(sctrl_addr + SC_BAKDATA11));
...
int set_debug_isp_clk_freq(unsigned int type, unsigned long value)
{
	...
	if (type >= ISP_CLK_MAX || ((1 << type) & dev->hisi_isp_clk.freqmask)) {
...
static struct hisi_rproc_data *hisi_rproc_data_dtget(struct device *pdev)
{
	...
	if (ret < 0) {
		RPROC_ERR("Failed: logb-flag.0x%x of_property_read_u32.%d\n", dev->use_logb_flag, ret);
		return NULL;
	}
	RPROC_INFO("use_logb_flag.%d\n", dev->use_logb_flag);
...
static int hisi_rproc_probe(struct platform_device *pdev)
{
	...
	rproc_dev->hisi_isp_clk.freqmask |= (1 << ISPI3C_CLK);
