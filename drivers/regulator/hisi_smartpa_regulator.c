...
static struct hisi_regulator_reg_ops g_hisi_regulator_reg_ops;
...
int hisi_smartpa_flash_init()
{
	...
	for (i = 0; i < ilen; i++) {
		ret = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(I2C_ADDR,
...
int hisi_smartpa_flash_irq_enable(int enable)
{
	...
	if (enable == 1) {
		...
		for (i = 0; i < ilen1; i++) {
			ret = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(
				I2C_ADDR,
...
static int hisi_regulator_smartpa_is_enabled(struct regulator_dev *dev)
{
	int ret;
	int value = 0;
	struct hisi_smartpa_regulator *sreg = rdev_get_drvdata(dev);
	...
	struct hisi_smartpa_regulator_ctrl_regs *ctrl_regs = &(sreg->ctrl_regs);
	struct hisi_smartpa_regulator_ctrl_data *ctrl_data = &(sreg->ctrl_data);

	ret = g_hisi_regulator_reg_ops.hisi_regulator_reg_read(
		I2C_ADDR, ctrl_regs->enable_reg2, &value);
	if (ret) {
		pr_err("[%s]: hisi_regulator_reg_read error", __func__);
		return ret;
	}
...
static int hisi_regulator_smartpa_enable(struct regulator_dev *dev)
{
	int ret;
	int ret1, ret2;
	struct hisi_smartpa_regulator *sreg = rdev_get_drvdata(dev);
	...
	struct hisi_smartpa_regulator_ctrl_regs *ctrl_regs = &(sreg->ctrl_regs);
	struct hisi_smartpa_regulator_ctrl_data *ctrl_data = &(sreg->ctrl_data);
	struct hisi_smartpa_regulator_time_out_regs *time_out_regs = &(sreg->time_out_regs);
	struct hisi_smartpa_regulator_vbat_protect_regs *vbat_protect_regs = &(sreg->vbat_protect_regs);
	...
	ret1 = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(I2C_ADDR,
		ctrl_regs->enable_reg1, ctrl_data->mask1, ctrl_data->shift1);
	ret2 = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(I2C_ADDR,
...
static int hisi_regulator_smartpa_disable(struct regulator_dev *dev)
{
	int ret;
	int ret1, ret2;
	struct hisi_smartpa_regulator *sreg = rdev_get_drvdata(dev);
	...
	ret1 = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(
		I2C_ADDR, ctrl_regs->enable_reg2, ctrl_data->mask2, 0);
	ret2 = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(
		I2C_ADDR, ctrl_regs->enable_reg1, ctrl_data->mask1, 0);
	ret = (ret1 || ret2);
	return ret;
}

static int hisi_regulator_smartpa_list_voltage_linear(
	struct regulator_dev *rdev, unsigned int selector)
{
	struct hisi_smartpa_regulator *sreg = rdev_get_drvdata(rdev);
...
static int hisi_regulator_smartpa_get_voltage(struct regulator_dev *dev)
{
	int index;
	int ret1, ret2;
	int value1 = 0;
	...
	struct hisi_smartpa_regulator_vset_reg1 *vset_reg1 = &(sreg->vset_reg1);
	struct hisi_smartpa_regulator_vset_reg2 *vset_reg2 = &(sreg->vset_reg2);
	struct hisi_smartpa_regulator_vset_data1 *vset_data1 = &(sreg->vset_data1);
	struct hisi_smartpa_regulator_vset_data2 *vset_data2 = &(sreg->vset_data2);

	ret1 = g_hisi_regulator_reg_ops.hisi_regulator_reg_read(
		I2C_ADDR, vset_reg1->vset_reg, &value1);
	ret2 = g_hisi_regulator_reg_ops.hisi_regulator_reg_read(
...
static int hisi_regulator_smartpa_set_voltage(
	struct regulator_dev *dev, int min_uv, int max_uv, unsigned int *selector)
{
	...
	*selector = vsel;
	...
	ret3 = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(
		I2C_ADDR, vset_reg1->vset_reg, 0xFF, value1);
	ret4 = g_hisi_regulator_reg_ops.hisi_regulator_reg_write(
		I2C_ADDR, vset_reg2->vset_reg, 0xFF, value2);
	ret = (ret1 || ret2 || ret3 || ret4);
	return ret;
}

static int hisi_regulator_smartpa_set_current_limit(
...
static int hisi_regulator_smartpa_get_current_limit(struct regulator_dev *dev)
{
	int i;
	int index;
	int ret;
...
static struct regulator_ops hisi_regulator_smartpa_rops = {
	.is_enabled = hisi_regulator_smartpa_is_enabled,
	.enable = hisi_regulator_smartpa_enable,
	.disable = hisi_regulator_smartpa_disable,
	.list_voltage = hisi_regulator_smartpa_list_voltage_linear,
	.get_voltage = hisi_regulator_smartpa_get_voltage,
	.set_voltage = hisi_regulator_smartpa_set_voltage,
	.get_current_limit = hisi_regulator_smartpa_get_current_limit,
	.set_current_limit = hisi_regulator_smartpa_set_current_limit,
};

static const struct hisi_smartpa_regulator hisi_smartpa_regulator = {
	.rdesc = {
			.ops = &hisi_regulator_smartpa_rops,
			.type = REGULATOR_VOLTAGE,
			.owner = THIS_MODULE,
	},
};

static struct of_device_id of_hisi_regulator_smartpa_match_tbl[] = {
	{
		.compatible = "hisilicon,hisi-regulator-smartpa",
		.data = &hisi_smartpa_regulator,
	},
	{}
};
...
static int fake_of_get_regulator_sreg(struct hisi_smartpa_regulator *sreg,
	struct device *dev, struct device_node *np)
{
	...
	(void)of_property_read_u32_array(np, "hisilicon,vset-reg1",
		(unsigned int *)(&sreg->vset_reg1), 0x1);
	(void)of_property_read_u32_array(np, "hisilicon,vset-data1",
		(unsigned int *)(&sreg->vset_data1), 0x4);
	(void)of_property_read_u32_array(np, "hisilicon,vset-reg2",
		(unsigned int *)(&sreg->vset_reg2), 0x1);
	(void)of_property_read_u32_array(np, "hisilicon,vset-data2",
	...
	ret = of_property_read_u32_array(np, "hisilicon,vset-table",
...
int hisi_regulator_reg_ops_register(struct hisi_regulator_reg_ops *ops)
{
	if (ops != NULL && ops->hisi_regulator_reg_read != NULL \
		&& ops->hisi_regulator_reg_write != NULL) {
		g_hisi_regulator_reg_ops.hisi_regulator_reg_read =
			ops->hisi_regulator_reg_read;

		g_hisi_regulator_reg_ops.hisi_regulator_reg_write =
			ops->hisi_regulator_reg_write;
		return 0;
	} else {
...
EXPORT_SYMBOL_GPL(hisi_regulator_reg_ops_register);

static int hisi_smartpa_regulator_probe(struct platform_device *pdev)
{
	int ret;
	...
	struct hisi_smartpa_regulator *sreg = NULL;
	struct regulator_init_data *initdata = NULL;
	const struct of_device_id *match = NULL;
	const struct hisi_smartpa_regulator *template = NULL;
	struct regulator_config config = {};
	/* Because read and write interface has not been successfully enabled */
	struct hisi_regulator_reg_ops ops = {
		.hisi_regulator_reg_read = fake_read,
		.hisi_regulator_reg_write = fake_write,
	};
	...
	match = of_match_device(
		of_hisi_regulator_smartpa_match_tbl, &pdev->dev);
	if (!match) {
	...
	if (g_hisi_regulator_reg_ops.hisi_regulator_reg_read ==	NULL) {
		ret = hisi_regulator_reg_ops_register(&ops);
		if (ret) {
			kfree(sreg);
			return -ENOMEM;
		};
...
static int hisi_smartpa_regulator_remove(struct platform_device *pdev)
{
	struct regulator_dev *rdev = NULL;
...
static struct platform_driver hisi_smartpa_regulator_driver = {
	.driver = {
			.name = "hisi_smartpa_regulator",
			.owner = THIS_MODULE,
			.of_match_table = of_hisi_regulator_smartpa_match_tbl,
	},
	.probe = hisi_smartpa_regulator_probe,
	.remove = hisi_smartpa_regulator_remove,
};

static int __init hisi_smartpa_regulator_init(void)
{
	/*if there is some adapt layers so must be add its init-function*/
	return platform_driver_register(&hisi_smartpa_regulator_driver);
}

static void __exit hisi_smartpa_regulator_exit(void)
{
	platform_driver_unregister(&hisi_smartpa_regulator_driver);
}

fs_initcall(hisi_smartpa_regulator_init);
module_exit(hisi_smartpa_regulator_exit);
