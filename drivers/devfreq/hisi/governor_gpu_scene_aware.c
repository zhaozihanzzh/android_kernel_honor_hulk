		...
		unsigned int *para, unsigned int ntokens)
{
	...
	while (i < ntokens) {
		if (sscanf_s(cp, "%u", &para[i++]) != 1) /*  [false alarm] */
			return -1;
...
static int policy_para_parse(struct devfreq *devfreq, const char *buf, size_t count)
{
	struct devfreq_gpu_scene_aware_data *data = NULL;
...
static void gpufreq_scene_aware_policy_init(struct devfreq *devfreq)
{
	...
	int ret, count, i;
	unsigned int cl_boost_freq = 0;
	struct device_node *np = NULL;
	const char *scene_para = NULL;
	struct devfreq_gpu_scene_aware_data *data = devfreq->data;

	if (data == NULL) {
		pr_err("%s: null data of devfreq\n", __func__);
		return;
	}
