...
#define UFS_BKOPS_1861_FLAG                                                    \
		(BKOPS_FLAG_BKOPS_CHK_TIME_INTERVAL |                          \
		BKOPS_FLAG_BKOPS_CHK_ACCU_WRITE |                              \
		BKOPS_FLAG_BKOPS_CHK_ACCU_DISCARD)
...
static char *bkops_status_str_1861[BKOPS_STATUS_MAX_HI1861] = {
	"bkops none", "bkops normal", "bkops urgent count", "bkops wl",
	"bkops not care", "bkops slc cache",
};

#define BKOPS_STATUS_MAX_HYNIX 2
static char *bkops_status_str_hynix[BKOPS_STATUS_MAX_HYNIX] = {
				  ...
				  struct hisi_bkops *ufs_bkops)
{
	if (hba->manufacturer_id == UFS_VENDOR_HI1861) {
		...
		hisi_bkops_set_status_info(ufs_bkops,
