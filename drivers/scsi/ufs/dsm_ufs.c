...
int dsm_ufs_update_fastboot_info(struct ufs_hba *hba)
{
	...
	if (1 != sscanf(pstr, "fastbootdmd=%d", &err_code)) {  // unsafe_function_ignore: sscanf
...
int dsm_ufs_get_log(struct ufs_hba *hba, unsigned long code, char *err_msg)
{
	...
	memset((void*)product_name, 0, 32);  // unsafe_function_ignore: memset
	...
	memset(g_ufs_dsm_log.dsm_log, 0, buff_size); // unsafe_function_ignore: memset
	...
	if ((hba->sdev_ufs_device != NULL)
		...
		snprintf(rev, 5, "%.4s", hba->sdev_ufs_device->rev); //unsafe_function_ignore: snprintf

	ret = snprintf(dsm_log_buff, buff_size, "fw:%s\n", rev); //unsafe_function_ignore: snprintf
	dsm_log_buff += ret;
	buff_size -= ret;
	...
	case DSM_UFS_INLINE_CRYPTO_ERR:
		...
		if(g_ufs_dsm_adaptor.err_type&(1 << UFS_INLINE_CRYPTO_ERR)) {
			...
			for(i = 0; i < 32; i++) {
				if(crypto_conf_index[i]) {
					ret = snprintf(dsm_log_buff, buff_size, "UTP_DESC[%d]: 0x%x\n", i, crypto_conf_index[i]); //unsafe_function_ignore: snprintf
					dsm_log_buff += ret;
					buff_size-= ret;
		...
		ret = snprintf(dsm_log_buff, buff_size, "UECDL:%08x,UECN:%08x\n", g_ufs_dsm_adaptor.uic_uecdl, g_ufs_dsm_adaptor.uic_uecn); //unsafe_function_ignore: snprintf
		dsm_log_buff += ret;
		buff_size -= ret;
		ret = snprintf(dsm_log_buff, buff_size, "UECT:%08x,UEDME:%08x\n", g_ufs_dsm_adaptor.uic_uect, g_ufs_dsm_adaptor.uic_uedme); //unsafe_function_ignore: snprintf
		dsm_log_buff += ret;
		buff_size -= ret;
		break;
	...
	case DSM_UFS_TIMEOUT_SERIOUS:
		...
		for (i = 0; i < COMMAND_SIZE(g_ufs_dsm_adaptor.timeout_scmd[0]); i++) {
			ret = snprintf(dsm_log_buff, buff_size, "%02x", g_ufs_dsm_adaptor.timeout_scmd[i]);/*[false alarm]: buff_size is nomal*/  //unsafe_function_ignore: snprintf
			dsm_log_buff += ret;
			buff_size -= ret;
		}
		break;
...
void dsm_ufs_init(struct ufs_hba *hba)
{
	...
	memset((void*)&uic_err_history, 0, sizeof(uic_err_history)); //unsafe_function_ignore: memset
