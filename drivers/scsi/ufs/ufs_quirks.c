	...
	uint8_t *str_desc_buf, uint8_t *desc_buf, const char *product_name)
{
	...
	memset(&hba->unique_number, 0, sizeof(hba->unique_number)); /* unsafe_function_ignore: memset */
	memset(snum_buf, 0, sizeof(snum_buf)); /* unsafe_function_ignore: memset */
	get_boardid(boardid);
	...
	case UFS_VENDOR_SAMSUNG:
		if (is_old_product_name(product_name, boardid)) {
			...
			memcpy(snum_buf, str_desc_buf + QUERY_DESC_HDR_SIZE, 12); /* unsafe_function_ignore: memcpy */
		} else {
	...
	case UFS_VENDOR_MICRON:
		memcpy(snum_buf, str_desc_buf + QUERY_DESC_HDR_SIZE, 4); /* unsafe_function_ignore: memcpy */
		for(i = 4; i < 12; i++) {
			snum_buf[i] = 0;
		}
		break;
	...
	memcpy(hba->unique_number.serial_number, snum_buf, SERIAL_NUM_SIZE); /* unsafe_function_ignore: memcpy */
}
			       ...
			       struct ufs_device_info *card_data)
{
	...
	memset(str_desc_buf, 0, QUERY_DESC_STRING_MAX_SIZE); /* unsafe_function_ignore: memset */
	...
	if (get_bootdevice_type() == BOOT_DEVICE_UFS) {
		u32 cid[4];
		int i;
		memcpy(cid, (u32 *)&hba->unique_number, sizeof(cid)); /* unsafe_function_ignore: memcpy */
		for (i = 0; i < 3; i++)
